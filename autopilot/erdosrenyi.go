...
// ConstrainedErdosRenyi is an implementation of the AttachmentHeuristic
// interface that implement a constrained Erdos Renyi heuristic. This means that
// given a threshold to allocate to automatic channel establishment, the
// heuristic will attempt to randomly connecting to nodes, selected by sampling
// from a uniform  distribution.
type ConstrainedErdosRenyi struct {
...
// NewConstrainedErdosRenyi creates a new instance of a
// ConstrainedErdosRenyi heuristics given bounds on allowed channel sizes,
// and an allocation amount which is interpreted as a percentage of funds that
// is to be committed to channels at all times.
func NewConstrainedErdosRenyi(minChanSize, maxChanSize btcutil.Amount,
	chanLimit uint16, allocation float64) *ConstrainedErdosRenyi {

	prand.Seed(time.Now().Unix())

	return &ConstrainedErdosRenyi{
...
// A compile time assertion to ensure ConstrainedErdosRenyi meets the
// AttachmentHeuristic interface.
var _ AttachmentHeuristic = (*ConstrainedErdosRenyi)(nil)
...
// TODO: Refactor since this is exactly the same as in prefattach.go
func (p *ConstrainedErdosRenyi) NeedMoreChans(channels []Channel,
...
// TODO:(renepickhardt) move this code from prefAttach to a more general place
// NodeID is a simple type that holds an EC public key serialized in compressed
// format.
// type NodeID [33]byte // already defined in prefattach.go
...
//	return n
//} // already defined in prefattach.go
...
// available.
// Other than the above contrains this will just be a random choice sampled
// from a uniform distribution
//
// NOTE: This is a part of the AttachmentHeuristic interface.
func (p *ConstrainedErdosRenyi) Select(self *btcec.PublicKey, g ChannelGraph,
	...
	for i := uint32(0); i < numNewChans; i++ {
		...
		if err := g.ForEachNode(func(node Node) error {
			...
			// Only include nodes that already have at least 1 channel
			// It does not make sense to automatically connect to a node
			// which is not improving the network by having no channels.
			...
			// TODO:(renepickhardt) make the 1 a constant or configurable consider
			// increasing this threshold but will also need to addept test cases
			if numChannels >= 1 {
