...
func TestConstrainedErdosRenyiNeedMoreChan(t *testing.T) {
	t.Parallel()
	...
	erdosRenyi := NewConstrainedErdosRenyi(minChanSize, maxChanSize,
		chanLimit, threshold)

	for i, testCase := range testCases {
		amtToAllocate, numMore, needMore := erdosRenyi.NeedMoreChans(
...
// TestConstrainedErdosRenyiSelectEmptyGraph ensures that when passed en
...
// infinite loop.
func TestConstrainedErdosRenyiSelectEmptyGraph(t *testing.T) {
	const (
		minChanSize = 0
		maxChanSize = btcutil.Amount(btcutil.SatoshiPerBitcoin)
		chanLimit   = 3
	...
	for _, graph := range chanGraphs {
		success := t.Run(graph.name, func(t1 *testing.T) {
			...
			directives, err := erdosRenyi.Select(self, graph,
...
// TestConstrainedErdosRenyiSelectTwoVertexes ensures that when passed a
...
func TestConstrainedErdosRenyiSelectTwoVertexes(t *testing.T) {
	t.Parallel()
...
// TestConstrainedErdosRenyiSelectInsufficientFunds ensures that if the
// balance of the backing wallet is below the set min channel size, then it
// never recommends candidates to attach to.
func TestConstrainedErdosRenyiSelectInsufficientFunds(t *testing.T) {
	t.Parallel()
	...
	for _, graph := range chanGraphs {
		success := t.Run(graph.name, func(t1 *testing.T) {
			...
			erdosRenyi := NewConstrainedErdosRenyi(
			...
			directives, err := erdosRenyi.Select(self, graph, 0,
				0, skipNodes)
			if err != nil {
				t1.Fatalf("unable to select attachment "+
					"directives: %v", err)
...
// TestConstrainedErdosRenyiSelectGreedyAllocation tests that if upon
...
// max channel size).
func TestConstrainedErdosRenyiSelectGreedyAllocation(t *testing.T) {
	t.Parallel()
...
// TestConstrainedErdosRenyiSelectSkipNodes ensures that if a node was
// already select for attachment, then that node is excluded from the set of
// candidate nodes.
func TestConstrainedErdosRenyiSelectSkipNodes(t *testing.T) {
	t.Parallel()
