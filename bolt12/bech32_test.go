...
func TestPropertyBech32RoundTrip(t *testing.T) {
	...
	rapid.Check(t, func(t *rapid.T) {
		...
		// The two limits have their own tests: the empty payload
		// in TestEncodeEmptyPayload and the upper limit in
		// TestSizeBudgetIsSymmetric. The upper bound here stays
		// far below the limit, so rapid works on the content of
		// the payload and not on its length.
