...
type InvoiceRequest struct {
	...
	// InvreqMetadata is a blob of unpredictable bytes provided by the payer.
	// It serves multiple roles under the BOLT 12 specification:
	// 1. Nonce: Ensures cryptographic signature entropy.
	// 2. Idempotency Key: If the metadata is identical to a previous request,
	//    the receiver may reply with the previously generated invoice instead
	//    of minting a new one.
	// 3. Message Marker: As Type 0, it canonically sits at the start of the
	//    invoice_request TLV stream.
