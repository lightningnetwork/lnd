...
func TestNonceHash(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			got := nonceHash(firstTLV, tc.tlvType)
			require.Equal(t, expected, got[:])
		})
	}
}
...
// sequence the Merkle root depends on the input order. The receiver-to-sender
// invoice flow signs a tree built over a type-sorted stream. If the root were
// order-insensitive, an attacker could permute fields without invalidating the
// signature.
