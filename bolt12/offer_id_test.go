...
// TestOfferIDMatchesCoreLightning pins the id against the offer Core Lightning
// publishes in its own RPC schema, together with the offer_id it reports for
// it.
func TestOfferIDMatchesCoreLightning(t *testing.T) {
	t.Parallel()

	// Offer and expected offer_id taken from Core Lightning's
	// doc/schemas/offer.json.
	const (
		clnOffer = "lno1qgsqvgnwgcg35z6ee2h3yczraddm72xrfua9uve2rlr" +
			"m9deu7xyfzrcgqgn3qzs2ge5hx6pqwdskcefpzcssxwz9sqk" +
			"jtd8qwnx06lxckvu6g8w8t0ue0zsrfqqygj636s4sw7v6"

		clnOfferID = "c5cde0292d56941940f8b10a4c9bdd1f8846d6041a61b" +
			"3f7e0f87e105aa88121"
	)

	_, tlvBytes, err := decodeBech32(clnOffer)
	require.NoError(t, err)
	...
	require.Equal(t, clnOfferID, hex.EncodeToString(id[:]))
}
