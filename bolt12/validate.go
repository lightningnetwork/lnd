...
// signatureTLVType is the TLV type carrying the BIP-340 Schnorr signature in an
// invoice_request. Per BOLT 12 it sits in the 240-1000 signature range,
// deliberately outside the allowed message ranges, so the range scans skip it
// explicitly rather than rejecting it as out-of-range.
const signatureTLVType tlv.Type = 240
...
func ValidateInvoiceRequestRead(ir *InvoiceRequest,
	...
	//
	// The same pass also rejects unknown even (must-understand) types per
	// the BOLT 1 TLV rule, matching the offer reader.
	for _, t := range sortedTypes(ir.decodedTLVs) {
		if t == signatureTLVType {
			continue
		}
	...
	if isResponse {
		...
		if ir.OfferChains.IsSome() || ir.OfferFeatures.IsSome() ||
			...
			return fmt.Errorf("%w: offer fields present on "+
				"non-offer response", ErrOutOfRangeType)
		}
