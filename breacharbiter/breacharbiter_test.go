	...
	breachedOutputs = []BreachedOutput{
		...
		},
		{
			BaseOutput: *lnwallet.NewBaseOutput(
				btcutil.Amount(3e4),
				breachOutPoints[2],
				lnwallet.CommitmentDelayOutput,
				lnwallet.SignDescriptor{
					SingleTweak: []byte{
						0x02, 0x02, 0x02, 0x02, 0x02, 0x02,
						0x02, 0x02, 0x02, 0x02, 0x02, 0x02,
...
func TestBreachHandoffFail(t *testing.T) {
	...
	aliceKeyPriv, _ := btcec.PrivKeyFromBytes(btcec.S256(),
		alicesPrivKey)
	aliceSigner := test.MakeMockSigner(aliceKeyPriv)
...
func createTestArbiter(t *testing.T, contractBreaches chan *ContractBreachEvent,
	...
	aliceKeyPriv, _ := btcec.PrivKeyFromBytes(btcec.S256(),
		alicesPrivKey)
	signer := test.MakeMockSigner(aliceKeyPriv)

	// Assemble our test arbiter.
	notifier := test.MakeMockSpendNotifier()
...
func createInitChannels(revocationWindow int) (*lnwallet.LightningChannel, *lnwallet.LightningChannel, func(), error) {
	...
	pCache := &test.MockPreimageCache{
		// hash -> preimage
		PreimageMap: make(map[[32]byte][]byte),
	}

	aliceSigner := test.MakeMockSigner(aliceKeyPriv)
	bobSigner := test.MakeMockSigner(bobKeyPriv)
