...
// TestTlsNoInterface checks that setting the --tlsnointerface flag
// does not add interface ip addresses to the cert
func TestTlsNoInterface(t *testing.T) {
	tempDir, err := ioutil.TempDir("", "certtest")
	if err != nil {
		t.Fatal(err)
	}
	...
	if shouldNotBeOutdated {
		t.Fatal("TLS Certificate was marked as outdated when it should not be")
	}
	...
	if !shouldBeOutdated {
		t.Fatal("TLS Certificate was not marked as outdated when it should be")
	}
}
