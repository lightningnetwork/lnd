...
type blockEpoch chainnotif.BlockEpoch
...
func (blockEpoch) Generate(r *rand.Rand, size int) reflect.Value {
	...
	return reflect.ValueOf(blockEpoch(chainnotif.BlockEpoch{
		Hash:   &chainHash,
...
func TestBestBlockTracker(t *testing.T) {
	...
	chainNotifierI := chainnotif.ChainNotifier(notifier)

	tracker := chainnotif.NewBestBlockTracker(chainNotifierI)
	...
	correctness := func(epochRand blockEpoch) bool {
		epoch := chainnotif.BlockEpoch(epochRand)
