...
type BtcdNotifier struct {
	...
	memNotifier *chainnotif.MempoolNotifier
...
var _ chainnotif.ChainNotifier = (*BtcdNotifier)(nil)

// Ensure BtcdNotifier implements the MempoolWatcher interface at compile time.
var _ chainnotif.MempoolWatcher = (*BtcdNotifier)(nil)
...
func New(config *rpcclient.ConnConfig, chainParams *chaincfg.Params,
	...
	notifier := &BtcdNotifier{
		...
		memNotifier: chainnotif.NewMempoolNotifier(),
...
func (b *BtcdNotifier) Stop() error {
	...
	chainnotif.Log.Info("btcd notifier shutting down...")
	defer chainnotif.Log.Debug("btcd notifier shutdown complete")
...
func (b *BtcdNotifier) startNotifier() error {
	...
	b.txNotifier = chainnotif.NewTxNotifier(
		uint32(currentHeight), chainnotif.ReorgSafetyLimit,
		b.confirmHintCache, b.spendHintCache,
	)

	b.bestBlock = chainnotif.BlockEpoch{
		...
		case item := <-b.chainUpdates.ChanOut():
			...
			if update.connect {
				...
				if blockHeader.PrevBlock != *b.bestBlock.Hash {
					...
					// backend
					chainnotif.Log.Infof("Missed blocks, " +
				...
				newBlock := chainnotif.BlockEpoch{
...
func (b *BtcdNotifier) handleRelevantTx(tx *btcutil.Tx,
	...
	if mempool {
		...
		if err != nil {
			chainnotif.Log.Errorf("Unable to process transaction "+
				"%v: %v", tx.Hash(), err)
		}
	...
	if err != nil {
		chainnotif.Log.Errorf("Unable to process transaction %v: %v",
			tx.Hash(), err)
...
func (b *BtcdNotifier) historicalConfDetails(confRequest chainnotif.ConfRequest,
	...
	if confRequest.TxID == chainnotif.ZeroHash {
	...
	txConf, txStatus, err := chainnotif.ConfDetailsFromTxIndex(
	...
	// scanning manually.
	case err != nil:
		chainnotif.Log.Debugf("Unable to determine confirmation of %v "+
	...
	case txStatus == chainnotif.TxFoundIndex:

	// The transaction was not found within the node's mempool or txindex.
	case txStatus == chainnotif.TxNotFoundIndex:
...
func (b *BtcdNotifier) confDetailsManually(confRequest chainnotif.ConfRequest,
	startHeight, endHeight uint32) (*chainnotif.TxConfirmation,
	chainnotif.TxConfStatus, error) {
	...
	for height := endHeight; height >= startHeight && height > 0; height-- {
		...
		select {
		case <-b.quit:
			return nil, chainnotif.TxNotFoundManually,
				chainnotif.ErrChainNotifierShuttingDown
		default:
		}
		...
		for txIndex, tx := range block.Transactions {
			...
			return &chainnotif.TxConfirmation{
				...
				Block:       block,
			}, chainnotif.TxFoundManually, nil
		}
	}

	// If we reach here, then we were not able to find the transaction
	// within a block, so we avoid returning an error.
	return nil, chainnotif.TxNotFoundManually, nil
}
...
// them.
func (b *BtcdNotifier) handleBlockConnected(epoch chainnotif.BlockEpoch) error {
	...
	chainnotif.Log.Infof("New block: height=%v, sha=%v", epoch.Height,
		epoch.Hash)
...
func (b *BtcdNotifier) RegisterSpendNtfn(outpoint *wire.OutPoint,
	...
	if outpoint == nil || *outpoint == chainnotif.ZeroOutPoint {
	...
	go func() {
		if rescanErr := asyncResult.Receive(); rescanErr != nil {
			chainnotif.Log.Errorf("Rescan to determine the spend "+
...
func (b *BtcdNotifier) SubscribeMempoolSpent(
	outpoint wire.OutPoint) (*chainnotif.MempoolSpendEvent, error) {
...
func (b *BtcdNotifier) CancelMempoolSpendEvent(
	sub *chainnotif.MempoolSpendEvent) {
