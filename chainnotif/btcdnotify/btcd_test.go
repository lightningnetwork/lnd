...
func TestHistoricalConfDetailsTxIndex(t *testing.T) {
	...
	txid, pkScript, err := chainnotif.GetTestTxidAndScript(harness)
	require.NoError(t, err, "unable to create tx")
	if err := chainnotif.WaitForMempoolTx(harness, txid); err != nil {
		t.Fatalf("unable to find tx in the mempool: %v", err)
	}
	confReq, err := chainnotif.NewConfRequest(txid, pkScript)
	require.NoError(t, err, "unable to create conf request")
	...
	// within the mempool.
	switch txStatus {
	case chainnotif.TxFoundMempool:
	default:
...
func TestHistoricalConfDetailsNoTxIndex(t *testing.T) {
	...
	// Since it has yet to be included in a block, it should have been found
	// within the mempool.
	if txStatus != chainnotif.TxFoundMempool {
		t.Fatal("should have found the transaction within the " +
			"mempool, but did not")
	}
	...
	// the chain manually.
	if txStatus != chainnotif.TxFoundManually {
