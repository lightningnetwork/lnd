...
type NeutrinoNotifier struct {
	...
	bestBlock    chainnotif.BlockEpoch
	...
	txNotifier *chainnotif.TxNotifier
	...
	spendHintCache chainnotif.SpendHintCache

	// confirmHintCache is a cache used to query the latest height hints for
	// a transaction. Each height hint represents the earliest height at
	// which the transaction could have confirmed within the chain.
	confirmHintCache chainnotif.ConfirmHintCache
...
func New(node *neutrino.ChainService, spendHintCache chainnotif.SpendHintCache,
	confirmHintCache chainnotif.ConfirmHintCache,
...
func (n *NeutrinoNotifier) Stop() error {
	...
	chainnotif.Log.Info("neutrino notifier shutting down...")
	defer chainnotif.Log.Debug("neutrino notifier shutdown complete")
...
func (n *NeutrinoNotifier) startNotifier() error {
	...
	n.txNotifier = chainnotif.NewTxNotifier(
		uint32(n.bestBlock.Height), chainnotif.ReorgSafetyLimit,
...
func (n *NeutrinoNotifier) connectFilteredBlock(update *filteredBlock) {
	...
	if update.height != uint32(n.bestBlock.Height+1) {
		chainnotif.Log.Infof("Missed blocks, attempting to catch up")

		_, missedBlocks, err := chainnotif.HandleMissedBlocks(
...
func (n *NeutrinoNotifier) disconnectFilteredBlock(update *filteredBlock) {
	...
	if update.height != uint32(n.bestBlock.Height) {
		chainnotif.Log.Infof("Missed disconnected blocks, attempting" +
			" to catch up")
	}
	newBestBlock, err := chainnotif.RewindChain(n.chainConn, n.txNotifier,
		n.bestBlock, int32(update.height-1),
	)
	if err != nil {
		chainnotif.Log.Errorf("Unable to rewind chain from height %d"+
...
func (n *NeutrinoNotifier) notificationDispatcher() {
	...
	for {
		...
		case registerMsg := <-n.notificationRegistry:
			...
			case *chainnotif.HistoricalConfDispatch:
				...
				//nolint:lll
				go func(msg *chainnotif.HistoricalConfDispatch) {
					defer n.wg.Done()
		...
		case txUpdate := <-n.txUpdates.ChanOut():
			...
			if err != nil {
				chainnotif.Log.Errorf("Unable to process "+
					"transaction %v: %v", update.tx.Hash(),
					err)
			}

		case err := <-n.rescanErr:
			chainnotif.Log.Errorf("Error during rescan: %v", err)
...
func (n *NeutrinoNotifier) historicalConfDetails(
	...
	for h := endHeight; h >= startHeight && h > 0; h-- {
		...
		select {
		case <-n.quit:
			return nil, chainnotif.ErrChainNotifierShuttingDown
		default:
		}
...
func (n *NeutrinoNotifier) handleBlockConnected(newBlock *filteredBlock) error {
	...
	chainnotif.Log.Infof("New block: height=%v, sha=%v", newBlock.height,
		newBlock.hash)
...
func (n *NeutrinoNotifier) RegisterSpendNtfn(outpoint *wire.OutPoint,
	...
	go func() {
		...
		if err != nil && !strings.Contains(err.Error(), "not found") {
			chainnotif.Log.Errorf("Failed getting UTXO: %v", err)
			return
		}
...
func (n *NeutrinoNotifier) RegisterBlockEpochNtfn(
	...
	reg := &blockEpochRegistration{
		...
		epochChan:  make(chan *chainnotif.BlockEpoch, 20),
...
// to satisfy the chainnotif.ChainConn interface.
