...
type mockHintCache struct {
	mu         sync.Mutex
	confHints  map[chainnotif.ConfRequest]uint32
	spendHints map[chainnotif.SpendRequest]uint32
}

var _ chainnotif.SpendHintCache = (*mockHintCache)(nil)
var _ chainnotif.ConfirmHintCache = (*mockHintCache)(nil)
...
func (c *mockHintCache) QuerySpendHint(spendRequest chainnotif.SpendRequest) (uint32, error) {
	...
	if !ok {
		return 0, chainnotif.ErrSpendHintNotFound
	}
...
func (c *mockHintCache) QueryConfirmHint(confRequest chainnotif.ConfRequest) (uint32, error) {
	...
	if !ok {
		return 0, chainnotif.ErrConfirmHintNotFound
	}
...
func newMockHintCache() *mockHintCache {
	return &mockHintCache{
		confHints:  make(map[chainnotif.ConfRequest]uint32),
		spendHints: make(map[chainnotif.SpendRequest]uint32),
	}
}
...
func TestTxNotifierRegistrationValidation(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			numConfs:   0,
			heightHint: 1,
			err:        chainnotif.ErrNumConfsOutOfRange,
		},
		{
			name:       "exceed max num confs",
			pkScript:   testRawScript,
			numConfs:   chainnotif.MaxNumConfs + 1,
			heightHint: 1,
			err:        chainnotif.ErrNumConfsOutOfRange,
		},
		{
			...
			heightHint: 0,
			checkSpend: true,
			err:        chainnotif.ErrNoHeightHint,
		},
	}
	...
	for _, testCase := range testCases {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			_, err := n.RegisterConf(
				&chainnotif.ZeroHash, testCase.pkScript,
			...
			_, err = n.RegisterSpend(
				&chainnotif.ZeroOutPoint, testCase.pkScript,
				testCase.heightHint,
			)
			if err != testCase.err {
...
func TestTxNotifierHistoricalConfDispatch(t *testing.T) {
	...
	txConf1 := chainnotif.TxConfirmation{
		BlockHash:   &chainnotif.ZeroHash,
		BlockHeight: 9,
		TxIndex:     1,
		Tx:          &tx1,
	}
	...
	txConf2 := chainnotif.TxConfirmation{
		BlockHash:   &chainnotif.ZeroHash,
		BlockHeight: 9,
		TxIndex:     2,
		Tx:          &tx2,
...
func TestTxNotifierFutureSpendDispatch(t *testing.T) {
	...
	expectedSpendDetails := &chainnotif.SpendDetail{
		SpentOutPoint:     &op,
...
func TestTxNotifierHistoricalSpendDispatch(t *testing.T) {
	...
	hintCache := newMockHintCache()
	n := chainnotif.NewTxNotifier(
		startingHeight, chainnotif.ReorgSafetyLimit, hintCache,
		hintCache,
	)
...
func TestTxNotifierMultipleHistoricalConfRescans(t *testing.T) {
	...
	ntfn1, err := n.RegisterConf(&chainnotif.ZeroHash, testRawScript, 1, 1)
	require.NoError(t, err, "unable to register spend ntfn")
	...
	ntfn2, err := n.RegisterConf(&chainnotif.ZeroHash, testRawScript, 1, 1)
	require.NoError(t, err, "unable to register spend ntfn")
...
func TestTxNotifierMultipleHistoricalSpendRescans(t *testing.T) {
	...
	spendDetails := &chainnotif.SpendDetail{
		SpentOutPoint:     &op,
		SpenderTxHash:     &chainnotif.ZeroHash,
		SpendingTx:        msgTx,
		SpenderInputIndex: 0,
		SpendingHeight:    startingHeight - 1,
	}
...
func TestTxNotifierMultipleHistoricalNtfns(t *testing.T) {
	...
	confNtfns := make([]*chainnotif.ConfRegistration, numNtfns)
	...
	expectedConfDetails := &chainnotif.TxConfirmation{
		BlockHeight: startingHeight - 1,
		Tx:          wire.NewMsgTx(1),
	}
	...
	op := wire.OutPoint{Index: 1}
	spendNtfns := make([]*chainnotif.SpendRegistration, numNtfns)
	for i := uint64(0); i < numNtfns; i++ {
		ntfn, err := n.RegisterSpend(&op, testRawScript, 1)
		if err != nil {
...
func TestTxNotifierCancelConf(t *testing.T) {
	...
	const startingHeight = 10
	hintCache := newMockHintCache()
	n := chainnotif.NewTxNotifier(startingHeight, 100, hintCache, hintCache)
	...
	tx1ConfDetails := &chainnotif.TxConfirmation{
		BlockHeight: startingHeight + 1,
...
func TestTxNotifierConfReorg(t *testing.T) {
	...
	hintCache := newMockHintCache()
	n := chainnotif.NewTxNotifier(
		7, chainnotif.ReorgSafetyLimit, hintCache, hintCache,
	)
...
func TestTxNotifierSpendReorg(t *testing.T) {
	...
	expectedSpendDetails1 := &chainnotif.SpendDetail{
		SpentOutPoint:     &op1,
	...
	spendTx2.AddTxIn(&wire.TxIn{
		PreviousOutPoint: chainnotif.ZeroOutPoint,
		SignatureScript:  testSigScript,
	})
	spendTx2.AddTxIn(&wire.TxIn{
	...
	expectedSpendDetails2BeforeReorg := chainnotif.SpendDetail{
		SpentOutPoint:     &op2,
...
func TestTxNotifierTearDown(t *testing.T) {
	...
	_, err = n.RegisterConf(&chainnotif.ZeroHash, testRawScript, 1, 1)
	if err == nil {
		t.Fatal("expected confirmation registration to fail")
	}
	_, err = n.RegisterSpend(&chainnotif.ZeroOutPoint, testRawScript, 1)
	if err == nil {
