...
type NotifierOptions struct {
	...
	// HistoricalScanFrom, if non-nil, requests a historical scan starting
	// at the given block height. A nil value means future-only watching.
...
// WithHistoricalScanFrom is an optional argument that requests a historical
// pkScript scan from the given block height through the current tip. Omitting
// this option means future-only watching. Passing height 0 is allowed and means
// an explicit scan from genesis.
func WithHistoricalScanFrom(height uint32) NotifierOption {
	return func(o *NotifierOptions) {
		o.HistoricalScanFrom = &height
	}
}
...
// script matches.
//
// AddPkScripts acknowledgments only mean the scripts were accepted and any
// requested historical scan was queued; they do not mean historical backfill has
// completed. Confirmation notifications are only sent once the requested
// confirmation depth is reached. Reorg invalidations are sent on the same stream
...
const (
	...
	// PkScriptNotificationHistoricalScanComplete signals that a historical
	// scan requested by an AddPkScripts call has completed or failed.
	PkScriptNotificationHistoricalScanComplete
)
...
// PkScriptHistoricalScan describes the completion status of a historical
// pkScript scan.
type PkScriptHistoricalScan struct {
	ScanID          uint64
	StartHeight     uint32
	EndHeight       uint32
	CompletedHeight uint32
	Error           string
}

// PkScriptAddResult describes the result of adding pkScripts to a registration.
type PkScriptAddResult struct {
	HistoricalScanID          uint64
	HistoricalScanQueued      bool
	HistoricalScanStartHeight uint32
	HistoricalScanEndHeight   uint32
	NumAdded                  uint32
}
