...
type TxNotifier struct {
	...
	// numPkScriptMatches tracks the total number of matched outputs
	// retained across all active pkScript subscriptions.
	numPkScriptMatches int

	// numPkScriptMatchBytes tracks the approximate memory footprint of all
	// matched outputs retained across active pkScript subscriptions.
	numPkScriptMatchBytes uint64
	...
	// pkScriptConfirmedByHeight tracks confirmation notifications by the
	// block height at which they were dispatched so they can be invalidated
	// during reorgs.
...
func (n *TxNotifier) ConnectTip(block *btcutil.Block,
	...
	var pendingPkScriptSpends []pkScriptPendingSpend
	if block != nil {
	...
	err = n.dispatchPendingPkScriptSpends(pendingPkScriptSpends)
	if err != nil {
		return err
	}
