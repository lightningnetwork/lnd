...
type chanAuxData struct {
	...
	initialLocalBalance tlv.RecordT[tlv.TlvType1, uint64]

	// initialRemoteBalance is the initial remote balance of the channel.
	initialRemoteBalance tlv.RecordT[tlv.TlvType2, uint64]

	// realScid is the real short channel ID of the channel corresponding to
	// the on-chain outpoint.
	realScid tlv.RecordT[tlv.TlvType3, lnwire.ShortChannelID]
...
func newChanAuxDataFromChan(openChan *OpenChannel) *chanAuxData {
	c := &chanAuxData{
		revokeKeyLoc: tlv.NewRecordT[tlv.TlvType0](
			keyLocRecord{openChan.RevocationKeyLocator},
		),
		initialLocalBalance: tlv.NewPrimitiveRecord[tlv.TlvType1](
			uint64(openChan.InitialLocalBalance),
		),
		initialRemoteBalance: tlv.NewPrimitiveRecord[tlv.TlvType2](
			uint64(openChan.InitialRemoteBalance),
		),
		realScid: tlv.NewRecordT[tlv.TlvType3](
