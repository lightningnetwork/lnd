...
// TestCloseChannelPersistsResponses asserts that closing a channel retains
// eligible responses and removes the forwarding package in the same
// transaction.
func TestCloseChannelPersistsResponses(t *testing.T) {
	...
	require.NoError(t, ch.CloseChannel(summary))
	...
	require.Equal(t, ch.ShortChanID(), response.OutKey.ChanID)
	require.Equal(t, settle.ID, response.OutKey.HtlcID)

	msg := response.Message
	persisted, ok := msg.(*lnwire.UpdateFulfillHTLC)
	require.True(t, ok)
	require.Equal(t, settle.PaymentPreimage, persisted.PaymentPreimage)
}
