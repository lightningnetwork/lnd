...
func TestGraphTraversal(t *testing.T) {
	...
	const numNodes = 20
	const numChannels = 5
	chanIndex, firstNode, secondNode := fillTestGraph(
...
func TestGraphCacheTraversal(t *testing.T) {
	...
	err = graph.graphCache.ForEachChannel(
		firstNode.PubKeyBytes, func(d *DirectedChannel) error {
			...
			if !bytes.Equal(
				inPolicyNodeKey[:], firstNode.PubKeyBytes[:],
			) {
				return fmt.Errorf("wrong outgoing edge")
			}
	...
	require.NoError(t, err)
	err = graph.graphCache.ForEachChannel(
		secondNode.PubKeyBytes, func(d *DirectedChannel) error {
			delete(chanIndex, d.ChannelID)
			...
			inPolicyNodeKey := d.InPolicy.ToNodePubKey()
			if !bytes.Equal(
				inPolicyNodeKey[:], secondNode.PubKeyBytes[:],
			) {
				return fmt.Errorf("wrong outgoing edge")
			}
	...
	// amount now.
	require.Equal(t, numChannels*2, numNodeChans)
}

func fillTestGraph(t *testing.T, graph *ChannelGraph, numNodes,
	numChannels int) (map[uint64]struct{}, *LightningNode, *LightningNode) {
	...
	return chanIndex, firstNode, secondNode
}
