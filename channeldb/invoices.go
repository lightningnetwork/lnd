...
type ContractTerm struct {
	...
	// Canceled indicates that this invoice has been canceled or otherwise
	// invalidated by the recipient
	Canceled bool
}
...
// returned, skipping all invoices that are fully settled or canceled.
func (d *DB) FetchAllInvoices(pendingOnly bool) ([]*Invoice, error) {
	...
	err := d.View(func(tx *bolt.Tx) error {
		...
		return invoiceB.ForEach(func(k, v []byte) error {
			...
			if pendingOnly && (invoice.Terms.Settled || invoice.Terms.Canceled) {
				return nil
			}
...
// CancelInvoice attempts to mark an invoice corresponding to the passed
// payment hash as canceled. If an invoice matching the passed payment
// hash doesn't existing within the database, then the action will fail with a
// "not found" error.
func (d *DB) CancelInvoice(paymentHash [32]byte) error {
	return d.Update(func(tx *bolt.Tx) error {
		...
		return cancelInvoice(invoices, invoiceNum)
	})
}
...
func serializeInvoice(w io.Writer, i *Invoice) error {
	...
	return binary.Write(w, byteOrder, i.Terms.Canceled)
}
...
func deserializeInvoice(r io.Reader) (*Invoice, error) {
	...
	if err := binary.Read(r, byteOrder, &invoice.Terms.Canceled); err != nil {
		return nil, err
	}
...
func cancelInvoice(invoices *bolt.Bucket, invoiceNum []byte) error {
	invoice, err := fetchInvoice(invoiceNum, invoices)
	if err != nil {
		return err
	}

	// Add idempotency to duplicate cancellations, return here to avoid
	// redundant writes
	if invoice.Terms.Canceled {
		return nil
	}

	invoice.Terms.Canceled = true
