...
func TestPaymentControlMultiShard(t *testing.T) {
	...
	runSubTest := func(t *testing.T, test testCase) {
		...
		if test.settleLast {
			...
			// If the first htlc is failed, settled the payment
			...
			// If the first htlc is settled, fail the payment would
			// give us an error.
			if test.settleFirst {
				require.ErrorIs(t, err,
					errPaymentConflictStatus)
		...
		// terminal error, this will result in an error state as
		...
		case !test.settleFirst && test.settleLast:
			finalStatus = StatusInFlight
			registerErr = ErrPaymentPendingFailed
