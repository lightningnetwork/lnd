...
func TestWitnessCacheRetrieval(t *testing.T) {
	...
	err = wCache.AddWitness(Sha256HashWitness, witness, 0)
	if err != nil {
...
func TestWitnessCacheDeletion(t *testing.T) {
	...
	if err := wCache.AddWitness(Sha256HashWitness, witness1, 0); err != nil {
		t.Fatalf("unable to add witness: %v", err)
	}

	witness2 := key[:]
	witness2Key := sha256.Sum256(witness2)
	if err := wCache.AddWitness(Sha256HashWitness, witness2, 0); err != nil {
		t.Fatalf("unable to add witness: %v", err)
	}
...
func TestWitnessCacheUnknownWitness(t *testing.T) {
	...
	err = wCache.AddWitness(234, key[:], 0)
