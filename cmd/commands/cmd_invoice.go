...
var deleteInvoicesCommand = cli.Command{
	...
	ArgsUsage: "--all --hash_list hash1,hash2",
	Description: `
	...
	If a --hash_list is specified, those invoices are deleted,
	...
	Flags: []cli.Flag{
		...
		cli.StringFlag{
			Name: "hash_list",
			Usage: "delete a list of invoices identified by its " +
				"payment hash",
		},
	},
}
...
func deleteInvoices(ctx *cli.Context) error {
	...
	var (
		hashArray     [][]byte
		rHash         []byte
		hashList      = ctx.String("hash_list")
		all           = ctx.Bool("all")
		hashListIsSet = ctx.IsSet("hash_list")
		err           error
		resp          proto.Message
	)
	...
	if all && hashListIsSet {
		return fmt.Errorf("cannot use --all and --hash_list at " +
			"the same time")
	}
