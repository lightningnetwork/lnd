...
func create(ctx context.Context, cmd *cli.Command) error {
	ctxc := getContext()
	client, cleanUp := getWalletUnlockerClient(cmd)
	defer cleanUp()
	...
	backups, err := parseChanBackups(cmd)
	...
	statelessInit := cmd.Bool(statelessInitFlag.Name)
	if !statelessInit && cmd.IsSet(saveToFlag.Name) {
...
mnemonicCheck:
	...
	if cmd.IsSet(macRootKeyFlag.Name) {
		macRootKey, err = hex.DecodeString(
			cmd.String(macRootKeyFlag.Name),
		)
		if err != nil {
	...
	if statelessInit {
		return storeOrPrintAdminMac(cmd, response.AdminMacaroon)
	}
...
func unlock(ctx context.Context, cmd *cli.Command) error {
	...
	switch {
	case cmd.IsSet("recovery_window"):
		recoveryWindow = int32(cmd.Int("recovery_window"))
	case len(args) > 0:
...
func changePassword(ctx context.Context, cmd *cli.Command) error {
	ctxc := getContext()
	client, cleanUp := getWalletUnlockerClient(cmd)
	defer cleanUp()
	...
	req := &lnrpc.ChangePasswordRequest{
		...
		NewPassword:        newPw,
		StatelessInit:      statelessInit,
		NewMacaroonRootKey: cmd.Bool("new_mac_root_key"),
	}
...
func createWatchOnly(ctx context.Context, cmd *cli.Command) error {
	ctxc := getContext()
	client, cleanUp := getWalletUnlockerClient(cmd)
	defer cleanUp()

	if cmd.NArg() != 1 {
		return cli.ShowCommandHelp(ctx, cmd, "createwatchonly")
	}
	...
	if statelessInit {
		return storeOrPrintAdminMac(cmd, initResp.AdminMacaroon)
	}
...
func storeOrPrintAdminMac(cmd *cli.Command, adminMac []byte) error {
	// The user specified the optional --save_to parameter. We'll save the
	// macaroon to that file.
	if cmd.IsSet(saveToFlag.Name) {
		macSavePath := lncfg.CleanAndExpandPath(cmd.String(
			saveToFlag.Name,
		))
