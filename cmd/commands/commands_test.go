...
func TestParseBlockHeightInputs(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			flagSet := flag.NewFlagSet("listchaintxns",
				flag.ContinueOnError)

			var startHeight, endHeight int64
			flagSet.Int64Var(&startHeight, "start_height",
				startDefault, "")
			flagSet.Int64Var(&endHeight, "end_height",
				endDefault, "")

			err := flagSet.Set("start_height",
				strconv.Itoa(int(tc.expectedStart)))
			if err != nil {
				t.Fatalf("failed to set start_height flag: %v",
					err)
			}
			err = flagSet.Set("end_height",
