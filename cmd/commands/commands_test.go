...
func TestParseBlockHeightInputs(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			flagSet1 := flag.NewFlagSet("listchaintxns",
				flag.ContinueOnError)

			var startHeight, endHeight int64
			flagSet1.Int64Var(&startHeight, "start_height",
				startDefault, "")
			flagSet1.Int64Var(&endHeight, "end_height",
				endDefault, "")

			err := flagSet1.Set("start_height",
				strconv.Itoa(int(tc.expectedStart)))
			if err != nil {
				t.Fatalf("failed to set start_height flag: %v",
					err)
			}
			err = flagSet1.Set("end_height",
				strconv.Itoa(int(tc.expectedEnd)))
			if err != nil {
				t.Fatalf("failed to set end_height flag: %v",
					err)
			}

			ctx := cli.NewContext(app, flagSet1, nil)
