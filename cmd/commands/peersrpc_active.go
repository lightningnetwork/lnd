...
func getPeersClient(cmd *cli.Command) (peersrpc.PeersClient, func()) {
...
func updateNodeAnnouncement(ctx context.Context, cmd *cli.Command) error {
	ctxc := getContext()
	client, cleanUp := getPeersClient(cmd)
	defer cleanUp()
	...
	if cmd.IsSet("address_add") {
		change = true
		for _, addr := range cmd.StringSlice("address_add") {
	...
	if cmd.IsSet("address_remove") {
		change = true
		for _, addr := range cmd.StringSlice("address_remove") {
			action := &peersrpc.UpdateAddressAction{
	...
	if cmd.IsSet("alias") {
		change = true
		req.Alias = cmd.String("alias")
	}

	if cmd.IsSet("color") {
		change = true
		req.Color = cmd.String("color")
	}

	if cmd.IsSet("feature_bit_add") {
		change = true
		for _, feature := range cmd.IntSlice("feature_bit_add") {
	...
	if cmd.IsSet("feature_bit_remove") {
		change = true
		for _, feature := range cmd.IntSlice("feature_bit_remove") {
			action := &peersrpc.UpdateFeatureAction{
				Action:     peersrpc.UpdateAction_REMOVE,
				FeatureBit: lnrpc.FeatureBit(feature),
			}
