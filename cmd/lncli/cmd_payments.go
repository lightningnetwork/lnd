...
var deletePaymentsCommand = cli.Command{
	...
	Description: `
	By default this command deletes all failed payments from the database to
	reclaim disk space.

	If so desired, _ALL_ payments (even the successful ones) can be deleted
	by specifying --include_non_failed.
...
func deletePayments(ctx *cli.Context) error {
	...
	// We pack two RPCs into the same CLI so there is a non-valid
	// combination of the flags we need to filter out.
	if singlePayment && includeNonFailed {
