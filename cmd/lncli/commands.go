...
var addFundCommand = cli.Command{
	Name:  "addfund",
	Usage: "Add fund to an existing channel",
	Description: `
	Attempt to add some fund to an existing channel. This will make the old funding tx
	as a Vin, which will be added to the new funding tx and will create a new channel 
	and remove the old one.
	`,
	ArgsUsage: "funding_txid output_index add_funding_amount target_conf",
	Flags: []cli.Flag{
		cli.StringFlag{
			Name:  "funding_txid",
...
func addFund(ctx *cli.Context) error {
	...
	// Show command help if no arguments provided
	if ctx.NArg() == 0 && ctx.NumFlags() == 0 {
		cli.ShowCommandHelp(ctx, "addfund")
		return nil
	}

	req := &lnrpc.AddFundRequest{
	...
	switch {
	case ctx.IsSet("add_amt"):
		req.AddFundingAmount = ctx.Int64("add_amt")
	case args.Present():
		addFundingAmount, err := strconv.ParseInt(args.First(), 10, 64)
		if err != nil {
			return fmt.Errorf("unable to decode add amount: %v", err)
		}
		req.AddFundingAmount = addFundingAmount
		args = args.Tail()
	default:
		req.ChannelPoint.OutputIndex = 0
	}
