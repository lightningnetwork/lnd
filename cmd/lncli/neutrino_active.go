...
func addNeutrinoPeer(ctx *cli.Context) error {
	...
	ips, err := resolveName(ctx.Args().First())
	if err != nil {
		return err
	}

	req := &neutrinorpc.AddPeerRequest{
		PeerAddrs: ips[0].String(),
	}
...
func disconnectNeutrinoPeer(ctx *cli.Context) error {
	...
	// number of arguments/flags.
	if ctx.NArg() != 1 || ctx.NumFlags() > 0 {
		return cli.ShowCommandHelp(ctx, "disconnetpeer")
	}
...
func isBanned(ctx *cli.Context) error {
	...
	if ctx.NArg() != 1 {
		return fmt.Errorf("address argument is missing")
	}
	client, cleanUp := getNeutrinoKitClient(ctx)
	defer cleanUp()
...
func getBlockHeader(ctx *cli.Context) error {
	...
	if !args.Present() {
		return fmt.Errorf("block hash argument is missing")
	}
...
func getCFilter(ctx *cli.Context) error {
	...
	if !args.Present() {
		return fmt.Errorf("block hash argument missing")
	}
...
func resolveName(host string) ([]net.IP, error) {
