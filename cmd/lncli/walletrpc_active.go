...
var bumpFeeCommand = cli.Command{
	...
	ArgsUsage: "outpoint",
	Description: `
	BumpFee attempts to bump an existing input (e.g., an anchor sweep) or a
	new input (e.g., an unconfirmed wallet utxo). This input is sent to
	lnd’s sweeper and registered for potential fee bumping. Depending on
	the params chosen, this can be either an RBF or CPFP.
	...
	lnd has a central batching engine in which inputs with same deadline
	are batched together to save on transaction fees. Due to this, we
	cannot rely on bumping the fee on a specific transaction, since
	transactions can change at any point with the addition of new inputs.
	The list of inputs that currently exist within lnd's central batching
	engine can be retrieved through the cli pendingsweeps.
