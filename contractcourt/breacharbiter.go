...
func updateBreachInfo(breachInfo *retributionInfo, spends []spend) (
	...
	for _, s := range spends {
		...
		case input.HtlcOfferedRevoke:
			...
			if err != nil {
				log.Errorf("Unable to determine if "+
					"revoke spend: %v", err)
				break
			}

			if ok {
				log.Debugf("HTLC spend was our own " +
					"revocation spend")
				break
			}
...
justiceTxBroadcast:
	...
	log.Debugf("Broadcasting justice tx: %v", newLogClosure(func() string {
...
Loop:
	for {
		...
		case spends := <-spendChan:
			...
			log.Infof("%v spends from breach tx for "+
			...
			if len(breachInfo.breachedOutputs) == 0 {
				log.Infof("Justice for ChannelPoint(%v) "+
		...
		case epoch, ok := <-newBlockChan.Epochs:
			...
			log.Warnf("Block height %v arrived without "+
			...
			if justiceTxs.spendCommitOuts != nil {
				...
				log.Debugf("Broadcasting justice tx "+
