...
func TestEarlyDispatchForceCloseNotInvoked(t *testing.T) {
	...
	// Wait briefly so any spurious early dispatch would have landed.
...
func TestEarlyDispatchSkippedOnFastPath(t *testing.T) {
	...
	// Give any spurious early-dispatch goroutine time to fire so the
	// assertion below isn't racing against a delayed invocation.
	time.Sleep(100 * time.Millisecond)
	require.Equal(t, 0, harness.earlyCoopCloseCount(),
