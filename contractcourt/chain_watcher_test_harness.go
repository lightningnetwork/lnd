...
// testReporter is a minimal interface for test reporting that is satisfied by
// both *testing.T and *rapid.T, allowing the harness to work with property-based
// tests.
...
type chainWatcherTestHarness struct {
	...
	// blockbeatProcessed is a channel that signals when a blockbeat has been processed.
...
// withRequiredConfs sets the number of confirmations required for channel closes.
...
// newChainWatcherTestHarness creates a new test harness for chain watcher tests.
func newChainWatcherTestHarness(t *testing.T, opts ...harnessOpt) *chainWatcherTestHarness {
	return newChainWatcherTestHarnessFromReporter(t, t, opts...)
}

// newChainWatcherTestHarnessFromReporter creates a test harness that works with
// both *testing.T and *rapid.T. The testingT parameter is used for operations
// that specifically require *testing.T (like CreateTestChannels), while reporter
// is used for all test reporting (Helper, Fatalf).
func newChainWatcherTestHarnessFromReporter(testingT *testing.T,
	...
	aliceChannel, bobChannel, err := lnwallet.CreateTestChannels(
		testingT, channeldb.SingleFunderTweaklessBit,
	)
	if err != nil {
	...
	chainWatcher, err := newChainWatcher(chainWatcherConfig{
		...
		contractBreach: func(retInfo *lnwallet.BreachRetribution) error {
			// In tests, we just need to accept the breach notification.
			// The actual breach handling is tested elsewhere.
			return nil
		},
	})
	if err != nil {
	...
	// Register cleanup. We use the testingT for Cleanup since rapid.T
...
func (h *chainWatcherTestHarness) createCoopCloseTx(outputValue int64) *wire.MsgTx {
	return &wire.MsgTx{
		TxIn: []*wire.TxIn{{
			PreviousOutPoint: h.aliceChannel.State().FundingOutpoint,
			Sequence:         wire.MaxTxInSequenceNum,
		}},
		TxOut: []*wire.TxOut{{
			Value:    outputValue,
			PkScript: []byte{byte(outputValue % 255)}, // Unique script.
		}},
	}
}

// createRemoteUnilateralCloseTx creates a remote unilateral close transaction.
// From Alice's perspective, this is Bob's local commitment transaction.
func (h *chainWatcherTestHarness) createRemoteUnilateralCloseTx() *wire.MsgTx {
...
func (h *chainWatcherTestHarness) createBreachCloseTx() *wire.MsgTx {
	...
	if err := lnwallet.ForceStateTransition(h.aliceChannel, h.bobChannel); err != nil {
...
// sendBlockBeat sends a blockbeat to the chain watcher.
// Note: This is not used for cooperative close tests since the chain watcher
// blocks synchronously waiting for confirmations and can't process blockbeats.
func (h *chainWatcherTestHarness) sendBlockBeat() {
	h.t.Helper()

	// Create mock blockbeat exactly as the other tests do.
	...
	mockBeat.On("Height").Return(h.currentHeight).Maybe()
	
	// Create a channel to signal when blockbeat is processed.
	processed := make(chan struct{})
	
	// Mock `NotifyBlockProcessed` to signal when done.
	mockBeat.On("NotifyBlockProcessed",
		nil, h.chainWatcher.quit).Return().Run(func(args mock.Arguments) {
			close(processed)
		}).Once()

	// Send the blockbeat.
	select {
	case h.chainWatcher.BlockbeatChan <- mockBeat:
	case <-time.After(5 * time.Second):
		h.t.Fatalf("unable to send blockbeat")
	}
	
	// Wait for the blockbeat to be processed.
	select {
	case <-processed:
		// Blockbeat processed.
	case <-time.After(5 * time.Second):
		h.t.Fatalf("blockbeat not processed")
	}
}
...
func (h *chainWatcherTestHarness) confirmTx(tx *wire.MsgTx, height int32) {
	...
	if confEvent == nil {
		// The chain watcher might not have registered for confirmations yet.
		// This is not necessarily an error in some test scenarios.
		return
	}
...
func (h *chainWatcherTestHarness) triggerReorg(tx *wire.MsgTx, reorgDepth int32) {
	h.t.Helper()
...
func (h *chainWatcherTestHarness) waitForCoopClose(timeout time.Duration) *CooperativeCloseInfo {
	h.t.Helper()
...
func (h *chainWatcherTestHarness) runCoopCloseFlow(
	...
	// This happens inside waitForCoopCloseConfirmation.
	h.waitForConfRegistration()

	if shouldReorg {
		// Trigger reorg to unblock waitForCoopCloseConfirmation.
		...
		// Normal confirmation flow - confirm to unblock waitForCoopCloseConfirmation.
		h.mineBlocks(1)
...
// createChannelCapacity returns a channel capacity suitable for testing
// scaled confirmations.
func createChannelCapacity(scale float64) btcutil.Amount {
	// Use the maximum channel size as base.
	maxSize := btcutil.Amount(16777215) // From lnwallet constants.
	return btcutil.Amount(float64(maxSize) * scale)
}