...
func (c *ChannelArbitrator) relaunchResolvers(commitSet *CommitSet,
	...
	for i := range unresolvedContracts {
		...
		if !htlc.Incoming {
			deadline := c.findOutgoingHTLCDeadline(htlc.RHash)
	...
	for htlcAction, htlcs := range htlcActions {
		...
		case HtlcTimeoutAction:
			for _, htlc := range htlcs {
				...
				// HTLC.
				deadline := c.findOutgoingHTLCDeadline(
					htlc.RHash,
				)
...
// by the timeout height of its incoming HTLC - this is the expiry height the
// that remote peer can spend his/her outgoing HTLC via the timeout path.
func (c *ChannelArbitrator) findOutgoingHTLCDeadline(
	rHash chainhash.Hash) fn.Option[int32] {

	// Iterate all the known HTLCs to find the targeted incoming HTLC.
	...
	log.Infof("ChannelArbitrator(%v): incoming HTLC not found for "+
		"rHash=%v, using default deadline instead", c.cfg.ChanPoint,
		rHash)
