...
func (c *ChannelArbitrator) dispatchBlockbeatToSub(
	...
	case <-timeout.C:
		// The beat was never delivered, so this subscription cannot
		// safely stay active for later heights. Cancel it and let the
		// resolver resubscribe/re-check the best height.
		c.cancelBlockbeatSubscription(sub)
