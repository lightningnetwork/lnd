...
type BudgetConfig struct {
	...
	AnchorCPFPRatio float64        `long:"anchorcpfpratio" description:"The ratio of the value in time-sensitive HTLCs to allocate as the budget to pay fees when CPFPing a force close tx using the anchor output."`

	DeadlineHTLC      btcutil.Amount `long:"deadlinehtlc" description:"The amount in satoshis to allocate as the budget to pay fees when sweeping a time-sensitive HTLC. If set, the budget calculated using the ratio (if set) will be capped at this value."`
	DeadlineHTLCRatio float64        `long:"deadlinehtlcratio" description:"The ratio of the value in a time-sensitive HTLC to allocate as the budget to pay fees when sweeping it."`

	NoDeadlineHTLC      btcutil.Amount `long:"nodeadlinehtlc" description:"The amount in satoshis to allocate as the budget to pay fees when sweeping a non-time-sensitive HTLC. If set, the budget calculated using the ratio (if set) will be capped at this value."`
	NoDeadlineHTLCRatio float64        `long:"nodeadlinehtlcratio" description:"The ratio of the value in a non-time-sensitive HTLC to allocate as the budget to pay fees when sweeping it."`
}
