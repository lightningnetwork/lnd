...
// success by PublishTx).
//
// Once the broadcast succeeds, sweepAnchor is invoked to offer the tx's CPFP
// anchor to the sweeper, and the goroutine keeps supervising that offer: the
// pre-signed parent cannot be RBF'd, so the anchor is its only fee-bumping
// path and a lost offer must not go unnoticed. An immediate SweepInput error
// is retried on the next block, and a failed sweep result (reported on the
// returned channel) re-offers the anchor on the next block. The loop ends
// when the sweep succeeds, the anchor is spent by another tx (which includes
// the anyone-can-spend path after the parent confirms), or the resolver
// shuts down; a resolver restart re-publishes and re-offers from scratch, so
// the retry boundary also survives restarts.
//
...
func publishPreSignedHtlcTx(tx *wire.MsgTx, label string,
	...
	wg *sync.WaitGroup, log btclog.Logger,
	sweepAnchor func() (<-chan sweep.Result, error)) error {
	...
	// waitBlock blocks until the next block epoch. It returns false when
	// the epoch stream ends or the resolver shuts down.
	waitBlock := func() bool {
		select {
	...
	go func() {
		...
		// Anchor supervision phase: keep the CPFP offer alive until
		// it concludes.
		var resultChan <-chan sweep.Result
		for {
			if resultChan == nil {
				rc, err := sweepAnchor()
				if err != nil {
					log.Errorf("unable to offer CPFP "+
						"anchor of tx=%v to the "+
						"sweeper, retrying on next "+
						"block: %v", tx.TxHash(), err)
				...
				// A nil channel without an error means there
				// is no anchor to sweep (or no key material
				// to sweep it with): nothing to supervise.
				if rc == nil {
					return
				}
				resultChan = rc
			...
			case result, ok := <-resultChan:
				...
				switch {
				// Someone else spent the anchor. Once the
				// parent has been confirmed for 16 blocks the
				// output is anyone-can-spend, so this is the
				// expected terminal outcome when our own
				// child never confirmed. Either way CPFP is
				// moot now.
				case errors.Is(
					result.Err, sweep.ErrRemoteSpend,
				):

					log.Debugf("second-level anchor of "+
						"tx=%v spent by remote party",
						tx.TxHash())

					return

				// Any other failed outcome (budget exhausted,
				// persistent publish failure, sweeper
				// restart) leaves the immutable parent
				// without a fee bump, so re-offer on the
				// next block.
				case result.Err != nil:
					log.Errorf("second-level anchor "+
						"CPFP sweep of tx=%v failed, "+
						"re-offering on next block: "+
						"%v", tx.TxHash(), result.Err)
...
// floor fee).
//
// The returned channel reports the sweep's final outcome and is nil (with a
// nil error) when there is no anchor to sweep. The caller is responsible for
// consuming it and re-offering on failure: the pre-signed parent cannot be
// RBF'd, so a silently lost CPFP offer would leave it stuck at its baked-in
// floor fee.
func offerSecondLevelAnchorToSweeper(
	req *secondLevelAnchorSweepReq) (<-chan sweep.Result, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("build anchor script tree: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("offer second-level anchor: %w", err)
	}
