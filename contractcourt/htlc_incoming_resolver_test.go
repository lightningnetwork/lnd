...
func (i *incomingResolverTestContext) waitForResult(expectSuccessRes bool) {
	...
	if !ok {
		i.t.Fatal("expected succesresolver")
	}
	...
	if successResolver.htlcResolution.SignedSuccessTx != nil &&
		!bytes.Equal(
			successResolver.htlcResolution.SignedSuccessTx.TxIn[0].
				Witness[3], testResPreimage[:],
		) {
		i.t.Fatal("invalid preimage")
	}
}
