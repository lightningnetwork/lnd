...
func TestProcessAnnouncement(t *testing.T) {
	...
	err = mustProcess(t, tCtx.gossiper.ProcessRemoteAnnouncement(ctx, ca, nodePeer))
	...
	err = mustProcess(t, tCtx.gossiper.ProcessRemoteAnnouncement(ctx, ua, nodePeer))
	...
	err = mustProcess(t, tCtx.gossiper.ProcessRemoteAnnouncement(ctx, na, nodePeer))
	require.NoError(t, err, "can't process remote announcement")
...
func TestPrematureAnnouncement(t *testing.T) {
	...
	mustProcess(t, tCtx.gossiper.ProcessRemoteAnnouncement(ctx, ca, nodePeer))
...
func TestSignatureAnnouncementLocalFirst(t *testing.T) {
	...
	// between two nodes.
	err = mustProcess(t, tCtx.gossiper.ProcessLocalAnnouncement(batch.chanAnn))
	...
	err = mustProcess(t, tCtx.gossiper.ProcessLocalAnnouncement(batch.chanUpdAnn1))
	...
	err = mustProcess(t, tCtx.gossiper.ProcessLocalAnnouncement(batch.nodeAnn1))
...
func TestForwardPrivateNodeAnnouncement(t *testing.T) {
	...
	err = mustProcess(t, tCtx.gossiper.ProcessLocalAnnouncement(nodeAnn))
...
func TestReceiveRemoteChannelUpdateFirst(t *testing.T) {
	...
	// Pretending that we receive local channel announcement from funding
	// manager, thereby kick off the announcement exchange process.
	err = mustProcess(t, tCtx.gossiper.ProcessLocalAnnouncement(batch.localProofAnn))
	if err != nil {
		t.Fatalf("unable to process :%v", err)
	}
...
func TestOptionalFieldsChannelUpdateValidation(t *testing.T) {
	...
	err = mustProcess(t, processRemoteAnnouncement(ctx, chanUpdAnn, nodePeer))
...
func TestPrematureAnnouncementProcessing(t *testing.T) {
	...
	case <-tCtx.broadcastedMessage:
	case <-time.After(2 * trickleDelay):
		t.Fatal("premature announcement was not broadcast after block " +
			"height advanced")
	}
