...
func (m *SyncManager) syncerHandler() {
	...
	for {
		...
		case newSyncer := <-m.newSyncers:
			...
			// The graph isn't synced and no initial historical sync is
			// in flight, e.g. the prior attempt failed and no eligible
			// replacement was found at the time. This new peer is a
			// healthy candidate, so attempt a historical sync with it rather
			// than waiting for the next hourly ticker to pick one.
			case !m.IsGraphSynced() && initialHistoricalSyncer == nil:
				attemptHistoricalSync =
					m.cfg.NumActiveSyncers > 0
				fallthrough
