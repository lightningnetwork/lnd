...
func TestDeriveRateLimitReservation(t *testing.T) {
	...
	t.Run("sequential messages have increasing delays", func(t *testing.T) {
		...
		delay1, err := sm.deriveRateLimitReservation(ctx, peer, msg)
		require.NoError(t, err)
		require.Equal(
		...
		delay2, err := sm.deriveRateLimitReservation(ctx, peer, msg)
		require.NoError(t, err)
		require.True(
		...
		delay3, err := sm.deriveRateLimitReservation(ctx, peer, msg)
		require.NoError(t, err)
	...
	t.Run("propagates serialization errors", func(t *testing.T) {
		...
		_, err := sm.deriveRateLimitReservation(ctx, peer, msg)
		require.Error(t, err)
		require.Equal(
	...
	t.Run("larger messages have longer delays", func(t *testing.T) {
		...
		_, err := sm.deriveRateLimitReservation(ctx, peer, initialMsg)
		require.NoError(t, err)
		...
		smallDelay, err := sm.deriveRateLimitReservation(
			ctx, peer, smallMsg,
		)
		require.NoError(t, err)
		...
		largeDelay, err := sm.deriveRateLimitReservation(
			ctx, peer, largeMsg,
		)
		require.NoError(t, err)
