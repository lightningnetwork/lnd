	...
	case msgs := <-msgChan2:
		for _, msg := range msgs {
			...
			case syncer1.gossipMsgs <- replyEnvelope{
				gen: syncer1.chanRangeQueryGen.Load(), msg: msg,
			}:
			}
		}
	}
	...
	case msgs := <-msgChan1:
		for _, msg := range msgs {
			...
			case syncer2.gossipMsgs <- replyEnvelope{
				gen: syncer2.chanRangeQueryGen.Load(), msg: msg,
			}:
...
func TestGossipSyncerDoesNotReuseQueuedRangeReply(t *testing.T) {
	...
	// The second reply is also out of range for any valid query, so if
	// it were wrongly consumed as the response to the new attempt below
	// it would fail that attempt, which the test detects through the
	// failure callback.
	...
	// attempt or completing it. The syncer must still be waiting for
	// the response to the new query, and no failure may be reported for
	// it.
	select {
	case err := <-failureErr:
