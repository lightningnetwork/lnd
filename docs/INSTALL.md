...
`lnd` is written in Go, with a minimum version of `1.23.8` (or, in case this
...
<details>
  ...
  sha256sum go1.23.8.linux-amd64.tar.gz | awk -F " " '{ print $1 }'
  ```
...
<details>
  ...
  ```
  wget https://dl.google.com/go/go1.23.8.linux-armv6l.tar.gz
  sha256sum go1.23.8.linux-armv6l.tar.gz | awk -F " " '{ print $1 }'
  ```
  ...
  this version of Go. If it matches, then proceed to install Go:
  ```
  sudo rm -rf /usr/local/go && tar -C /usr/local -xzf go1.23.8.linux-armv6l.tar.gz
  export PATH=$PATH:/usr/local/go/bin
  ```  
