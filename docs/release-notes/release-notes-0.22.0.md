...
* The fundee now [rejects absurdly high channel
  ...
  turn away sane channels), or if its `to_local` and `to_remote` amounts both
  land at or under the `channel_reserve_satoshis` it proposed. Both amounts
  are compared against that one reserve, as BOLT-02 specifies, rather than
  against the reserve we name in our own `accept_channel`. Because all
