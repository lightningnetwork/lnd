...
type (
	...
	// boolean indicating if this features depdencies have already been
	// verified. This allows us to short circuit verification if multiple
	// features have common depdencies, or map traversal starts verifying
	// from the bottom up.
...
// feature is not present in the depDesc it is assumed to have no depdencies.
//
...
// but its depdency is optional, that is sufficient.
...
func validateDeps(features featureSet, supported supportedFeatures) error {
	for bit := range features {
		...
		// vector is invalid. Alternatively, if we know that this
		// dependencies are valid, we can short circuit and continue
...
// depdendency graph is described using only optional feature bits, which
