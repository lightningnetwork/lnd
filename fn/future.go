...
func (p *promiseImpl[T]) Complete(result Result[T]) bool {
	var success bool
...
func (f *futureImpl[T]) Await(ctx context.Context) Result[T] {
...
func (f *futureImpl[T]) ThenApply(ctx context.Context,
	...
	go func() {
		...
		originalResult.WhenOk(func(res T) {
			newValue := fApply(res)
			transformedPromise.Complete(Ok(newValue))
		})
	}()
