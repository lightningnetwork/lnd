...
func (r *reader) readStartParams(lockedWumbo *bool) (startParams, bool) {
	...
	// Resolve the channel-type preset (mostly a default type; a minority pick
	// an explicit one). The two feature bytes only matter for the default
...
const (
	...
	// OpenChannel carrying no channel_type at all, so the SUT (fundee) must
	// reject it with ErrChanTypeRequired.
	tamperOpenStripType
...
func resolveChanType(sel, localByte, remoteByte byte) chanTypeConfig {
	...
	case 8:
		...
		// The same missing channel_type, from a peer that does not
		// signal option_channel_type. The SUT keeps the bit, lnd
		// advertising it unconditionally, so a node without it is one
		// lnd cannot be. The requirement does not hinge on the bit, so
		// she must still reject the open as fundee, and still send a
		// channel_type herself as funder, where the injection never
		// runs.
		default:
	...
	// No type requested: the funder picks a default out of the features
	// both sides signal and sends it explicitly, whether or not the pair
	// signals option_channel_type. A 1-in-8 minority models a peer pair
	// without the bit.
	default:
...
func (f *fuzzFSM) funderInitReq(flow *flowState, peer *testNode,
	...
	return &InitFundingMsg{
		...
		// which case the manager picks its own default and still sends
		// that explicitly.
...
func (n *testNode) handleOpenChannel(f *fuzzFSM, params startParams) {
	...
	f.assertFunderChanType(flow, open)
...
// hands its peer a message the spec obliges the peer to fail on. The
// requirement holds whatever either side signals.
func (f *fuzzFSM) assertFunderChanType(flow *flowState,
	open *lnwire.OpenChannel) {

	if open.ChannelType == nil {
		f.t.Fatalf("flow %d: SUT sent an OpenChannel with no "+
...
func (f *fuzzFSM) assertFundeeChanTypeEcho(flow *flowState,
	...
	f.t.Fatalf("flow %d: SUT's AcceptChannel does not echo the "+
