...
func (f *Manager) addToGraph(completeChan *channeldb.OpenChannel,
	peerAlias *lnwire.ShortChannelID, shouldAnnounce bool) error {

	chanAnn, err := f.newChanAnnouncement(
		f.cfg.IDKey, completeChan.IdentityPub,
		&completeChan.LocalChanCfg.MultiSigKey,
		completeChan.RemoteChanCfg.MultiSigKey.PubKey,
		completeChan.ShortChanID(), completeChan.ChanType,
	)
	if err != nil {
	...
	chanUpdateAnn, err := f.newChanUpdate(completeChan, nil, shouldAnnounce)
	if err != nil {
...
func (f *Manager) annAfterSixConfs(completeChan *channeldb.OpenChannel,
	confirmedScid *lnwire.ShortChannelID) error {
	...
	if !announceChan {
		...
		log.Debugf("Will announce channel %v after ChannelPoint"+
			...
			confirmedScid.ToUint64(), completeChan.FundingOutpoint,
			numConfs)
		...
		// we need to update the msg flags of the channel policies so
		// that the __dont_forward__ bit is unset.
		...
		log.Debugf("Channel with ChannelPoint(%v), short_chan_id=%v "+
			"sent to gossiper", &fundingPoint, confirmedScid)
	}
...
func (f *Manager) waitForZeroConfChannel(c *channeldb.OpenChannel) error {
	...
	err = c.MarkRealScid(confirmedScid)
	if err != nil {
	...
	if isPublic {
		err = f.cfg.AliasManager.DeleteSixConfs(aliasScid)
		if err != nil {
		...
		err = f.sendChanUpdate(c, ourPolicy, true)
		if err != nil {
...
func (f *Manager) handleChannelReadyReceived(channel *channeldb.OpenChannel,
	...
	if channel.IsZeroConf() {
		...
		// In case of a zero-conf channel, we don't want to announce it
		// before it has its obligatory 6 confirmations.
		shouldAnnounce = false
		peerAlias = &foundAlias
	}

	err := f.addToGraph(channel, peerAlias, shouldAnnounce)
	if err != nil {
...
func (f *Manager) newChanUpdate(completeChan *channeldb.OpenChannel,
	ourPolicy *models.ChannelEdgePolicy,
	shouldAnnounce bool) (*lnwire.ChannelUpdate1, error) {
...
func (f *Manager) newChanProof(chanAnn *lnwire.ChannelAnnouncement1,
...
func (f *Manager) announceChannel(completeChan *channeldb.OpenChannel,
	...
	case err := <-errChan:
		if graph.IsError(err, graph.ErrOutdated, graph.ErrIgnored) {
			log.Debugf("Graph rejected AnnounceSignatures: %v", err)
		}
	...
	case err := <-errChan:
		if graph.IsError(err, graph.ErrOutdated, graph.ErrIgnored) {
			log.Debugf("Graph rejected NodeAnnouncement: %v", err)
		}
...
func (f *Manager) sendChanUpdate(completeChan *channeldb.OpenChannel,
	ourPolicy *models.ChannelEdgePolicy, shouldAnnounce bool) error {

	chanUpdate, err := f.newChanUpdate(
		completeChan, ourPolicy, shouldAnnounce,
	)
	if err != nil {
		return fmt.Errorf("error generating channel update: %w", err)
	}
