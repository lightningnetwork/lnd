...
// EstimateFeePerKW returns the rate configured on the stub, or its
// error.
func (f feeEstimatorStub) EstimateFeePerKW(
	confTarget uint32,
...
// anchorOpenChannel builds a complete, valid OpenChannel that proposes
// the given feerate, funding amount, and reserve. It is a helper for the
// tests below so the deeply nested struct literal can sit at a level of
// indentation the 80-column limit can accommodate.
func anchorOpenChannel(
	feeRate chainfee.SatPerKWeight,
	fundingAmt btcutil.Amount,
	...
	return &lnwire.OpenChannel{
		...
		CsvDelay:       4,
		// The BOLT-02 limit on accepted HTLCs.
...
func TestFundingManagerCommitFeeRateTooLarge(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			estimator := feeEstimatorStub{
				feeRate: test.feeRate,
				err:     test.err,
			}
			alice, bob := setupFundingManagers(
				t, func(cfg *Config) {
					cfg.FeeEstimator = estimator
				},
			)
			t.Cleanup(func() {
				tearDownFundingManagers(t, alice, bob)
			...
			// 702500 sat, and its two anchor outputs add 660 sat,
			// so the channel is sized to leave the initiator well
			...
			// newOpenChanMsg builds the OpenChannel the
			// initiator sends, with feerate_per_kw overridden.
			// Every other field is a complete, valid open,
			// built by the helper above, so the positive case
			// below cannot pass on a later failure.
			newOpenChanMsg := func(
				feeRate chainfee.SatPerKWeight,
			) *lnwire.OpenChannel {

				return anchorOpenChannel(
					feeRate, fundingAmt, initiatorReserve,
					&channelType,
				)
			}
			...
			errMsg, ok := msg.(*lnwire.Error)
			require.True(t, ok,
				"expected *lnwire.Error, got %T", msg)

			// The message carries the exact bound we expect,
			// and its text is derived from the same constructor
			// the daemon uses, so this pins the rate and the bound
			// reported as well as the wording the remote peer
			// sees.
			...
			require.ErrorIs(t, expected,
				lnwallet.ErrCommitFeeRateTooLargeBase,
			)
			...
			require.True(t, ok, "expected *lnwire.AcceptChannel, "+
				"got %T", accept)
		})
	}
}
...
func TestFundingManagerBalancesBelowReserve(t *testing.T) {
	...
	initiatorBalance := capacity - commitFee -
