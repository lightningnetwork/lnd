...
// representation. v2 entries are still serialized so that previously stored
// or peer-announced v2 addresses round-trip faithfully through the database
// and through Node.NodeAnnouncement re-broadcasts; lnd does not produce v2
// addresses itself anymore.
...
// FilterReachableAddrs returns the subset of addrs that lnd can actually
// reach today. Legacy Tor v2 onion entries are stored faithfully (so we can
// reproduce the signed bytes of peer announcements that included them) but
// callers that intend to dial or surface addresses should drop them, since
// Tor stopped serving v2 services in October 2021.
func FilterReachableAddrs(addrs []net.Addr) []net.Addr {
