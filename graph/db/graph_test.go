...
func TestNodeUpdatesInHorizonBoundaryConditions(t *testing.T) {
	...
	for _, batchSize := range batchSizes {
		t.Run(fmt.Sprintf("BatchSize%d", batchSize), func(t *testing.T) {
			...
			var nodeAnns []models.LightningNode

			for i := 0; i < 25; i++ {
				nodeAnn := createTestVertex(t)
				nodeAnn.LastUpdate = startTime.Add(time.Duration(i) * time.Hour)
				nodeAnns = append(nodeAnns, *nodeAnn)
				require.NoError(t, testGraph.AddLightningNode(ctx, nodeAnn))
			}
			...
			testCases := []struct {
				...
				},
				{
					...
					start: startTime,
					end:   startTime.Add(time.Duration(min(batchSize, 25)-1) * time.Hour),
					want:  min(batchSize, 25),
				},
				{
					name:  "cross batch boundary",
					start: startTime,
					end:   startTime.Add(time.Duration(min(batchSize, 24)) * time.Hour),
					want:  min(batchSize+1, 25),
				},
				{
					name: "exact boundary",
					start: func() time.Time {
						// Test querying exactly at a batch boundary.
						if batchSize <= 25 {
							return startTime.Add(time.Duration(batchSize-1) * time.Hour)
						}

						// For batch sizes > 25, test beyond our data range.
						return startTime.Add(time.Duration(25) * time.Hour)
					}(),
					end: func() time.Time {
						if batchSize <= 25 {
							return startTime.Add(time.Duration(batchSize-1) * time.Hour)
					...
					want: func() int {
						...
						// No nodes exist at hour 25 or beyond
						return 0
					}(),
				},
				{
			...
			for _, tc := range testCases {
				t.Run(tc.name, func(t *testing.T) {
					...
					for i := 1; i < len(nodes); i++ {
						require.True(t,
							nodes[i-1].LastUpdate.Before(nodes[i].LastUpdate) ||
								nodes[i-1].LastUpdate.Equal(nodes[i].LastUpdate),
...
func TestNodeUpdatesInHorizonEarlyTermination(t *testing.T) {
	...
	for _, stopAt := range terminationPoints {
		t.Run(fmt.Sprintf("StopAt%d", stopAt), func(t *testing.T) {
			...
			var collected []models.LightningNode
			count := 0
			for node := range iter {
				if count >= stopAt {
					break
...
func TestChanUpdatesInHorizonBoundaryConditions(t *testing.T) {
	...
	for _, batchSize := range batchSizes {
		t.Run(fmt.Sprintf("BatchSize%d", batchSize), func(t *testing.T) {
			...
			for i := 0; i < numChans; i++ {
				updateTime := startTime.Add(time.Duration(i) * time.Hour)
