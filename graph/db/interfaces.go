...
type Store interface { //nolint:interfacebloat
	...
	// graph for the given gossip version, executing the passed callback
	// with each node encountered. If the callback returns an error, then
	// the transaction is aborted and the iteration stops early.
	...
	NodeUpdatesInHorizon(v lnwire.GossipVersion, r NodeUpdateRange,
	...
	FilterChannelRange(v lnwire.GossipVersion, startHeight,
		endHeight uint32, withTimestamps bool) ([]BlockChannelRange,
		error)
