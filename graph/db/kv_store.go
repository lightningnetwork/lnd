...
func (c *KVStore) fetchNextChanUpdateBatch(
	...
	err := kvdb.View(c.db, func(tx kvdb.RTx) error {
		...
		default:
			// TODO(roasbeef): throwing away the value here?
			indexKey, _ = updateCursor.Seek(startTimeBytes[:])
		}

		// TODO(roasbeef): iterate the channel graph cache instead w/ a treap
		// ordering?
		...
		for len(batch) < state.batchSize && indexKey != nil {
			...
			// The edge wasn't in the cache, so we'll fetch it along w/ the
			// edge policies and nodes.
			edgeInfo, err := fetchChanEdgeInfo(edgeIndex, chanID)
			if err != nil {
				return fmt.Errorf("unable to fetch info "+
					"for edge with chan_id=%v: %v", chanIDInt, err)
			}
...
func (c *KVStore) ChanUpdatesInHorizon(startTime, endTime time.Time,
	...
	return func(yield func(ChannelEdge) bool) {
		...
		for {
			...
			if err != nil {
				// These errors just mean the graph is empty, which is OK.
				if !isEmptyGraphError(err) {
					log.Errorf("ChanUpdatesInHorizon batch "+
						"error: %v", err)
					return
				}
...
func (c *KVStore) fetchNextNodeBatch(
	state *nodeUpdatesIterator) ([]models.LightningNode, bool, error) {

	var (
		nodeBatch []models.LightningNode
		hasMore   bool
	)
	...
	err := kvdb.View(c.db, func(tx kvdb.RTx) error {
		...
		if ourPubKey == nil && state.publicNodesOnly {
			// If we're filtering for public nodes only but don't have
			// a source node set, we can't determine if nodes are public.
			// A node is considered public if it has at least one channel
			// with our node (the source node).
			return ErrSourceNodeNotSet
		}
		...
		for len(nodeBatch) < state.batchSize && indexKey != nil {
			// Extract the timestamp from the index key (first 8 bytes).
			// Only compare timestamps, not the full key with pubkey.
		...
		if lastProcessedKey != nil {
			if state.lastSeenKey == nil {
				state.lastSeenKey = make([]byte, len(lastProcessedKey))
			}
