...
var (
	// ErrNoParentJobs is returned when we expect a parent job ID for a
	// child job, but there aren't any.
	ErrNoParentJobs = fmt.Errorf("unable to find parent job ids")
)
...
func (v *ValidationBarrier) InitJobDependencies(job interface{}) (JobID,
	...
	updateOrCreateJobInfo := func(annID any, annJobID JobID) {
		...
		if ok {
			// If an entry already exists for annID, thne a job
...
func (v *ValidationBarrier) WaitForParents(childJobID JobID,
	...
	case *lnwire.ChannelUpdate1:
		...
		if !ok {
			// If ok is false, some consistency guarantee has been
			// broken.
			v.Unlock()
			return ErrNoParentJobs
		}
...
func (v *ValidationBarrier) SignalDependents(job interface{}, id JobID) error {
	...
	case *lnwire.NodeAnnouncement:
		...
		removeJob(route.Vertex(msg.NodeID), id, true)
		return nil
	case *lnwire.ChannelUpdate1:
		// Remove child job info.
		removeJob(msg.ShortChannelID, id, true)
		return nil
