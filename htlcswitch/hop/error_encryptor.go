...
// timestamp and passes it to the underlying EncryptError method.
func (s *SphinxErrorEncrypter) encryptWithHoldTime(initial bool,
...
func (s *SphinxErrorEncrypter) EncryptFirstHop(
	...
	return s.encryptWithHoldTime(true, b.Bytes(), nil)
}
...
func (s *SphinxErrorEncrypter) EncryptMalformedError(
	...
	return s.encryptWithHoldTime(true, reason, nil)
}
...
func (s *SphinxErrorEncrypter) IntermediateEncrypt(
	...
	encrypted, attrData, err := s.encryptWithHoldTime(
		false, reason, attrData,
	)
...
func (s *SphinxErrorEncrypter) Encode(w io.Writer) error {
	...
	creationTime := uint64(s.CreatedAt.UnixNano())
