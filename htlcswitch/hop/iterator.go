...
func deriveBlindedRouteForwardingInfo(r *sphinxHopIterator,
	...
	// blinded routes, by the next node's ID (next_node_id). We pass a
	// node-ID next hop through verbatim; the switch's non-strict forwarding
	// logic resolves it to one of our channels with that peer. The dummy
	// hop check above has already handled a next_node_id that points at us.
