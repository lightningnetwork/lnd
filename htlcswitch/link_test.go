...
func TestExitNodeTimelockPayloadExceedsHTLC(t *testing.T) {
	...
	var rtErr ClearTextError
	ok := errors.As(err, &rtErr)
	require.True(t, ok)
	require.IsType(
		t, rtErr, err,
		"expected a ClearTextError, instead got: %T", err,
	)
...
func TestExitNodeHTLCUnderpaysPayloadAmount(t *testing.T) {
	...
	f := func(underpayment_rand uint64) bool {
		underpayment := lnwire.MilliSatoshi(
			underpayment_rand%(btcutil.SatoshiPerBitcoin-1) + 1,
		)
		channels, _, err := createClusterChannels(
...
func TestExitNodeHTLCExceedsAmountPayload(t *testing.T) {
	...
	f := func(overpayment_rand uint64) bool {
		overpayment := lnwire.MilliSatoshi(
			overpayment_rand%(btcutil.SatoshiPerBitcoin-1) + 1,
		)
		channels, _, err := createClusterChannels(
			t, btcutil.SatoshiPerBitcoin*5,
			btcutil.SatoshiPerBitcoin*5,
