...
// recorded via the StoreResult method and should not use FailAttempt.
func (store *networkResultStore) FailAttempt(attemptID uint64,
	...
	// First, verify that the attempt exists and is in the pending state,
	// otherwise we cannot fail it.
	err := store.verifyAttemptIsPending(attemptID)
	if err != nil {
		return fmt.Errorf("cannot fail attempt %d: %w", attemptID, err)
	}

	// The attempt to send the htlc failed before it was ever dispatched.
	// We will write a failure result to the store to unblock any potential
	...
	if err := store.storeResult(attemptID, failureResult); err != nil {
		return err
	}
...
// verifyAttemptIsPending checks that an attempt exists in the store and is
// currently in a pending state. It returns an error otherwise.
func (store *networkResultStore) verifyAttemptIsPending(
	...
	if existingResult.msg.MsgType() != pendingHtlcMsgType {
		return fmt.Errorf("not in pending state")
	}
