...
func (r *mockReputationManager) OnForward(in, out CircuitKey, inAmt,
	outAmt lnwire.MilliSatoshi, cltv uint32, accountable bool) {

	r.mu.Lock()
	defer r.mu.Unlock()
	r.forwards = append(r.forwards, repForward{
		in: in, out: out, inAmt: inAmt, outAmt: outAmt, cltv: cltv,
...
func (p *panicReputationManager) OnForward(_, _ CircuitKey, _,
	_ lnwire.MilliSatoshi, _ uint32, _ bool) {
...
func TestGuardedReputationManagerRecovers(t *testing.T) {
	...
	mustNotPanic(t, func() {
		guard.OnForward(in, out, 1, 1, 100, false)
	})
