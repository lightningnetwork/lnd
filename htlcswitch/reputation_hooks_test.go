...
//
// This matters because the manager tracks the pending HTLC against its outgoing
// channel: if the add and the resolve reported different channels, the
// resolution would never match the add, the HTLC would never be scored, and the
// pending entry would be left behind.
func TestSwitchReputationNonStrictForward(t *testing.T) {
	...
	// The resolution must report the same channel as the add, so that the
	// manager can match the two.
	_, settles, _ := repMgr.snapshot()
	require.Len(t, settles, 1)
	require.Equal(t, forwards[0].out, settles[0].out,
