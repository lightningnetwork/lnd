...
func (s *Switch) extractResult(deobfuscator ErrorDecrypter, n *networkResult,
	...
	case *lnwire.UpdateFailHTLC:
		...
		paymentErr, encryptedErr := s.parseFailedPayment(
			deobfuscator, attemptID, paymentHash, n.unencrypted,
			n.isResolution, htlc,
		)

		// ElimOption allows us to safely handle both cases of the
		// optional error.
		result := fn.ElimOption(
			paymentErr,
...
func (s *Switch) parseFailedPayment(deobfuscator ErrorDecrypter,
	...
	isResolution bool, htlc *lnwire.UpdateFailHTLC) (fn.Option[error],
	[]byte) {
	...
	case unencrypted:
		...
		if err != nil {
			...
			return fn.Some[error](linkError), nil
		}

		// If we successfully decoded the failure reason, return it.
		return fn.Some[error](NewLinkError(failureMsg)), nil
	...
	default:
		...
		if deobfuscator == nil {
			return fn.None[error](), htlc.Reason
		}
		...
		if err != nil {
			...
			return fn.Some[error](ErrUnreadableFailureMessage), nil
		}

		return fn.Some[error](failure), nil
	}
}
