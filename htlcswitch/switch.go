...
func (s *Switch) handlePacketForward(packet *htlcPacket) error {
	...
	// forwards it. Thus there's no need to catch `UpdateFailMalformedHTLC`
	// here.
	case *lnwire.UpdateFailHTLC:
...
func (s *Switch) handlePacketAdd(packet *htlcPacket,
	...
	if len(destinations) == 0 {
		...
		log.Tracef("incoming HTLC(%x) violated target outgoing link "+
			"(id=%v) policy: %v", htlc.PaymentHash[:],
			packet.outgoingChanID, linkErr)
	...
	// Evaluate whether this HTLC would increase our exposure to dust on
	// the incoming link. If it does, fail it backwards.
	...
	// Also evaluate whether this HTLC would increase our exposure to dust
	// on the destination link. If it does, fail it back.
	if s.evaluateDustThreshold(
...
func (s *Switch) handlePacketSettle(packet *htlcPacket) error {
	...
	// received, which invokes `processRemoteSettleFails` in its link.
	if circuit == nil {
	...
	// Check to see that the source link is online before removing the
	// circuit.
