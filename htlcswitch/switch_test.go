...
// TestHTLCPersistence...
func TestHTLCPersistence(t *testing.T) {
	...
	htlcSwitch, err := NewHTLCSwitch(db)
	if err != nil {
		t.Fatalf("can't initialize htlc switch: %v", err)
	}
	htlcSwitch.Start()
	...
	newHtlcSwitch, err := NewHTLCSwitch(db)
	if err != nil {
		t.Fatalf("can't initialize htlc switch: %v", err)
	}
	newHtlcSwitch.Start()
	newHtlcSwitch.Add(aliceHltcManager)
	newHtlcSwitch.Add(bobHtlcManager)

	if newHtlcSwitch.circuits.pending() != 1 {
		t.Fatal("wrong amount of circuits")
	}
	...
	// Handle the request and checks that payment circuit works properly.
	if err := newHtlcSwitch.Forward(request); err != nil {
		t.Fatal(err)
	}
	...
	if newHtlcSwitch.circuits.pending() != 0 {
		t.Fatal("wrong amount of circuits")
	}
}
