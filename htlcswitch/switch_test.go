...
//  1. A successful payment (UpdateFulfillHTLC) correctly returns the preimage.
//  2. A failed payment (UpdateFailHTLC) with no deobfuscator correctly returns
//     the opaque, encrypted error reason.
//  3. A failed payment with a deobfuscator correctly decrypts and returns the
//     underlying failure message.
//  4. An unencrypted, local failure is correctly decoded and returned.
//  5. An on-chain resolution failure is correctly identified and returned.
//  6. An unknown message type results in an error.
func TestExtractResult(t *testing.T) {
	...
	err := lnwire.EncodeFailure(&unencryptedReasonBytes, unencryptedFailure, 0)
	require.NoError(t, err)
