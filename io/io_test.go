...
// TestWriteFileToDisk uses same scenario of ioutil asserting the content
// created and stored on new file with the original one.
func TestWriteFileToDisk(t *testing.T) {
	...
	if err := io.WriteFileToDisk(
		filename, []byte(data2), 0644, io.Default); err == nil {
		t.Fatalf(
			"WriteFileToDisk expected permission error %s",
			filename,
		)
	}
...
func ensureTempfile(t *testing.T) (*os.File, func()) {
	t.Helper()
	f, err := os.CreateTemp("", "io-test-TestWriteFileToDisk")
	if err != nil {
		t.Fatal(err)
	}
