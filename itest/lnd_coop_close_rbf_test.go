...
// It also verifies the blocks_til_closed field correctly tracks remaining
// confirmations and resets appropriately after a reorg.
func testCoopCloseRBFWithReorg(ht *lntest.HarnessTest) {
	...
	// Verify blocks_til_closed equals requiredConfs when tx is unconfirmed.
	waitingClose := ht.AssertNumWaitingClose(alice, 1)
	require.Equal(
		ht, uint32(requiredConfs), waitingClose[0].BlocksTilClosed,
	...
	// Verify blocks_til_closed decremented to requiredConfs - 1 = 2.
	...
	// Verify blocks_til_closed decremented to 1.
	err = wait.NoError(func() error {
		resp := alice.RPC.PendingChannels()
		if len(resp.WaitingCloseChannels) != 1 {
			return fmt.Errorf("expected 1 waiting close channel, "+
	...
	err = wait.NoError(func() error {
		...
		blocks := resp.WaitingCloseChannels[0].BlocksTilClosed
		if blocks != uint32(requiredConfs) {
			return fmt.Errorf("expected blocks_til_closed=%d "+
	...
	// Verify blocks_til_closed resumes countdown after re-mining.
	err = wait.NoError(func() error {
		resp := alice.RPC.PendingChannels()
		if len(resp.WaitingCloseChannels) != 1 {
			return fmt.Errorf("expected 1 waiting close channel, "+
