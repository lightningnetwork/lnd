...
func testDeleteCanceledInvoice(ht *lntest.HarnessTest) {
	...
	// Now that the channel is open, create 2 invoices for Bob.
	const numInvoices = 2
	...
	bob.RPC.DeleteCanceledInvoiceAssertErr(
		&lnrpc.DelCanceledInvoiceReq{InvoiceHash: "bb02fbfa62983b6b62" +
			"1376bf8230732dd3a6dcea9f5df803c0935ae6ce7440dg"},
		"encoding/hex: invalid byte",
	)

	// Let's delete one canceled invoice.
	resp := bob.RPC.DeleteCanceledInvoice(&lnrpc.DelCanceledInvoiceReq{
		InvoiceHash: lntypes.Hash(invoiceResp[0].RHash).String(),
	})

	require.Contains(ht, resp.String(), "canceled invoice deleted "+
		"successfully")
	...
	bob.RPC.DeleteCanceledInvoiceAssertErr(&lnrpc.DelCanceledInvoiceReq{
		InvoiceHash: lntypes.Hash(invoiceResp[0].RHash).String()},
		invoices.ErrInvoiceNotFound.Error(),
	)

	// Let's assert an error while deleting a non canceled invoice.
	notCanInv := lntypes.Hash(invoiceResp[numInvoices-1].RHash).String()
	bob.RPC.DeleteCanceledInvoiceAssertErr(&lnrpc.DelCanceledInvoiceReq{
		InvoiceHash: notCanInv},
