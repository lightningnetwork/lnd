...
func testDeleteCanceledInvoices(ht *lntest.HarnessTest) {
	...
	hashStrs = []string{lntypes.Hash(invoiceResp[4].RHash).String()}
	...
	hashStrs = []string{
		"invalidhash",
		lntypes.Hash(invoiceResp[4].RHash).String(),
		lntypes.Hash(invoiceResp[0].RHash).String(),
		lntypes.Hash(invoiceResp[1].RHash).String(),
	}
