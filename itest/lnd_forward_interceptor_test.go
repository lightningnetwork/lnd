...
func testForwardInterceptorRestart(
	...
	if !resumeBeforeRestart {
		...
		// custom records intact. The expected number of records depends
		// on whether the endorsement experiment is still active.
		packet = ht.ReceiveHtlcInterceptor(bobInterceptor)
		expectedLen := 1
		...
		require.Equal(
			ht,
			lntest.CustomRecordsWithUnendorsed(customRecords),
			packet.InWireCustomRecords,
		)
	...
	// Forward the payment at Carol, expecting only an endorsement signal in
	// the incoming custom records (if the experiment is still active).
	expectedCarolLen := 0
	if lntest.ExperimentalEndorsementActive() {
		expectedCarolLen = 1
	}
	require.Len(ht, carolPacket.InWireCustomRecords, expectedCarolLen)
