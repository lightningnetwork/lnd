...
func runMultiHopHtlcAggregation(ht *lntest.HarnessTest,
	...
	if c == lnrpc.CommitmentType_SIMPLE_TAPROOT {
		...
		carolRouteHints = append(carolRouteHints, &lnrpc.RouteHint{
			HopHints: []*lnrpc.HopHint{hopHint},
		})

		aliceChans := alice.RPC.ListChannels(
			&lnrpc.ListChannelsRequest{},
		)

		aliceChan := aliceChans.Channels[0]

		hopHint = &lnrpc.HopHint{
			NodeId: aliceChan.RemotePubkey,
			ChanId: aliceChan.ChanId,
			FeeBaseMsat: uint32(
				chainreg.DefaultBitcoinBaseFeeMSat,
			),
			FeeProportionalMillionths: uint32(
