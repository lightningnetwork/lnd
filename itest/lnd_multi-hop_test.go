...
func runMultiHopReceiverChainClaim(ht *lntest.HarnessTest,
	...
	// If we need to mine an extra block to account for the fact that we
	// want to mine the zero conf channel before closing, then we need to
	// tack on an extra block to the cltv expiry, otherwise the invoices we
	// cancel early due to expiry.
	var zeroConfDelta uint64
	...
	invoiceReq := &invoicesrpc.AddHoldInvoiceRequest{
		Value:      invoiceAmt,
		CltvExpiry: finalCltvDelta + zeroConfDelta,
		Hash:       payHash[:],
		RouteHints: routeHints,
	}
...
func runMultiHopLocalForceCloseOnChainHtlcTimeout(ht *lntest.HarnessTest,
	...
	req := &routerrpc.SendPaymentRequest{
		...
		Amt:            int64(htlcAmt),
		PaymentHash:    payHash,
		FinalCltvDelta: finalCltvDelta + int32(zeroConfDelta),
		TimeoutSeconds: 60,
		FeeLimitMsat:   noFeeLimitMsat,
		RouteHints:     routeHints,
	}
	...
	numBlocks := padCLTV(uint32(finalCltvDelta + zeroConfDelta))
...
func runMultiHopRemoteForceCloseOnChainHtlcTimeout(ht *lntest.HarnessTest,
	...
	numBlocks := padCLTV(uint32(invoiceReq.CltvExpiry) - 1)
	ht.MineBlocks(numBlocks)
...
func runMultiHopHtlcAggregation(ht *lntest.HarnessTest,
	...
	for i := 0; i < numInvoices; i++ {
		...
		invoiceReq := &invoicesrpc.AddHoldInvoiceRequest{
			Value:      invoiceAmt,
			CltvExpiry: thawHeightDelta - 4 + zeroConfDelta,
			Hash:       payHash[:],
