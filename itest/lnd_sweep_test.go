...
func testSweepCommitOutputAndAnchor(ht *lntest.HarnessTest) {
	...
	// config.
	deadlineA, deadlineB := uint32(1000), uint32(1000)
	...
	cfg := []string{
		"--protocol.anchors",
		fmt.Sprintf("--sweeper.nodeadlineconftarget=%v", deadlineA),
		fmt.Sprintf("--bitcoin.defaultremotedelay=%v", toLocalCSV),
	}
	openChannelParams := lntest.OpenChannelParams{
		Amt:     fundAmt,
	...
	// requests.
	pendingSweepAlice := ht.AssertNumPendingSweeps(alice, 2)[0]
	...
	// `blockbeat`.
	_, currentHeight = ht.Miner.GetBestBlock()
	actualDeadline = int32(pendingSweepAlice.DeadlineHeight) - currentHeight
	...
	alicePosition, bobPosition := uint32(0), uint32(2)
