...
func testSweepAnchorCPFPLocalForceClose(ht *lntest.HarnessTest) {
	...
	// Send two utxos to Alice - she will need two utxos to sweep the
	// anchor output living on the local and remote commits.
	ht.FundCoins(btcutil.SatoshiPerBitcoin, alice)
	ht.FundCoins(btcutil.SatoshiPerBitcoin, alice)

	// Send one utxo to Bob.
	...
	// Mine a block to confirm Alice's force closing tx.
	...
	// block.
	sweepTx := ht.Miner.GetNumTxsFromMempool(1)[0]
	...
	// Bob should has no sweeping txns as he doesn't have any balance.
	//
	// TODO(yy): it looks like there's a to remote anchor being created in
	// Alice's commitment tx. However, since there's no to_remote balance
	// there, it should not be created? Double check if this is a bug in
	...
	for i := uint32(1); i < deadline; i++ {
		...
		// Alice should still has this CPFP pending sweep.
	...
	// Alice should now have two pending sweeps - one for the HTLC and one
	// for her anchor output.
	ht.AssertNumPendingSweeps(alice, 2)
	...
	// Mine a block to confirm Alice's sweeping tx, this is needed to clean
	// up the mempool.
...
func testSweepCommitOutputAndAnchor(ht *lntest.HarnessTest) {
	...
	fundAmt := btcutil.Amount(1000_000)
	...
	// Send one utxo to Alice - she will only need this utxo for funding.
	// For Bob, he doesn't need any utxos as his sweeping txns will only
	// use the inputs from the force close tx.
	ht.FundCoins(btcutil.SatoshiPerBitcoin, alice)
	...
	// sweeping tx being RBFed. For Bob, he will only publish a tx every 4
	// blocks mined. Since his fee function is already at position 1,
