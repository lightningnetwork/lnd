...
func testZeroConfCoopCloseSubscribeEvents(ht *lntest.HarnessTest) {
	...
	// Crucially: no second CLOSED_CHANNEL event should have arrived
	// between the early one and the FULLY_RESOLVED_CHANNEL. The
	// channel-arbitrator-side suppression drops the duplicate that
	// MarkChannelClosed would otherwise emit. We give the stream a brief
	// quiet window to surface any straggling events.
