...
// readWriteTx holds a reference to an open sqlite transaction.
...
func newReadWriteTx(db *db, readOnly bool) (*readWriteTx, error) {
	...
	// database lock from sqlite. Unfortunately there is no database-level
	// lock in sqlite, meaning that each table would need to be locked
