...
// found within a block, and that Alice can report the status of the new
// channel.
func addFundAndAssert(ctx context.Context, t *harnessTest,
	net *lntest.NetworkHarness, alice, bob *lntest.HarnessNode,
	addAmt btcutil.Amount, fundingChanPoint *lnrpc.ChannelPoint) *lnrpc.ChannelPoint {

	addFundUpdates, err := net.AddFund(ctx, alice, bob, addAmt, fundingChanPoint)
	if err != nil {
		t.Fatalf("unable to add fund to channel", err)
	}
	...
	if err != nil {
		t.Fatalf("error while waiting for channel add fund: %v", err)
	}
...
func testBasicChannelAddFund(net *lntest.NetworkHarness, t *harnessTest){
	...
	for i := 0; i < numChannels; i++ {
		ctx, _ := context.WithTimeout(context.Background(), timeout)
		newChanPoints[i] = addFundAndAssert(
			ctx, t, net, net.Alice, net.Bob, addAmount, chanPoints[i],
		)
	}
