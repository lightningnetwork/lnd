...
func (s *Server) FetchReputation(_ context.Context,
	...
	if mgr == nil {
		return nil, fmt.Errorf("reputation subsystem disabled; " +
			"enable it with --routing.reputation")
	}
...
func marshalChannelReputation(
	...
	return &ChannelReputation{
		...
		General:              marshalBucket(s.General),
		Congestion:           marshalBucket(s.Congestion),
		Protected:            marshalBucket(s.Protected),
	}
}

// marshalBucket converts a bucket occupancy into its RPC form.
func marshalBucket(b reputation.BucketOccupancy) *BucketOccupancy {
	return &BucketOccupancy{
		SlotsUsed:              uint32(b.SlotsUsed),
		SlotsAllocated:         uint32(b.SlotsAllocated),
		LiquidityUsedMsat:      b.LiquidityUsedMsat,
		LiquidityAllocatedMsat: b.LiquidityAllocatedMsat,
	}
}
