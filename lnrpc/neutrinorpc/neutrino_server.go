...
func (s *Server) GetBlockHeader(ctx context.Context,
	...
	if err := chainhash.Decode(&hash, in.Hash); err != nil {
		return &GetBlockHeaderResponse{}, nil
	}
...
func (s *Server) GetCFilter(ctx context.Context,
	...
	filer, err := s.cfg.NeutrinoCS.GetCFilter(hash, wire.GCSFilterRegular)
	if err != nil {
		return &GetCFilterResponse{}, err
	}

	filterlBytes, err := filer.Bytes()
	if err != nil {
		return &GetCFilterResponse{}, err
	}
...
func (s *Server) TxScan(ctx context.Context,
	...
	default:
		return &TxScanResponse{}, fmt.Errorf("no a valid network type")
	}

	var err error
	var addrs = make([]btcutil.Address, len(in.Addrs))
...
func (s *Server) getBlock(hash chainhash.Hash) (*GetBlockResponse, error) {
	...
	// Transaction weight is calculated as
	// base transaction size * 3 + total transaction size.
	weight := int64(len(strippedData)*3 + len(blockData))

	// Convert txids to a string array.
	transactions := block.Transactions()
	var tx = make([]string, len(transactions))
