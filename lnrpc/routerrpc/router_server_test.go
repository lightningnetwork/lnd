...
func TestIsLsp(t *testing.T) {
	...
	lspTestCases := []struct {
		...
		{
			name: "multiple routes, multiple hints, different fees",
			routeHints: [][]zpay32.HopHint{
				{
					bobHopHint,
				},
			...
			expectedLspHop: &bobExpensiveCopy,
		},
		{
	...
	for _, tc := range lspTestCases {
		t.Run(tc.name, func(t *testing.T) {
			isLsp, adjustedRouteHints, lspHint := isLSP(
				tc.routeHints, tc.probeAmtMsat,
			)
			require.Equal(t, tc.isLsp, isLsp)
			require.Equal(t, tc.expectedHints, adjustedRouteHints)
