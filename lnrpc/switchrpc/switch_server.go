...
var (
	...
	// ErrAmbigousPaymentState is an error returned when a payment attempt
	// completes in an ambiguous state: no error and no valid preimage.
	ErrAmbigousPaymentState = errors.New("payment completed in an " +
...
func (s *Server) TrackOnion(ctx context.Context,
	...
	if result.Preimage == (lntypes.Preimage{}) {
		...
		return &TrackOnionResponse{
			...
			ErrorMessage: ErrAmbigousPaymentState.Error(),
		}, nil
	}
