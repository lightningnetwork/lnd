...
func (w *WalletKit) PendingSweeps(ctx context.Context,
	...
	for _, inp := range inputsMap {
		...
		inp.Params.StartingFeeRate.WhenSome(
			func(feerate chainfee.SatPerKWeight) {
				ps.RequestedSatPerVbyte = uint64(
					feerate.FeePerVByte(),
				)
			})
...
func (w *WalletKit) prepareSweepParams(in *BumpFeeRequest,
	...
	var oldBudget btcutil.Amount
	if ok {
		oldBudget = inp.Params.Budget
	}
	...
	// If it's an existing input, we assign it first, then overwrite it if
