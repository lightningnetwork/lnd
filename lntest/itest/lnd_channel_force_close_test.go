...
func testChannelForceClosure(ht *lntest.HarnessTest) {
	...
	for _, channelType := range commitTypes {
		...
		success := ht.Run(testName, func(t *testing.T) {
			...
			st.SendCoins(btcutil.SatoshiPerBitcoin, alice)

			// Also give Carol some coins to allow her to sweep her
			// anchor.
			st.SendCoins(btcutil.SatoshiPerBitcoin, carol)
...
func channelForceClosureTest(ht *lntest.HarnessTest,
	...
	sweepTxns := ht.GetNumTxsFromMempool(expectedTxes)
	...
	assertSweepFound(ht, alice, sweepingTXID.String(), false)
	...
	ht.AssertTxInBlock(block, sweepTx.Hash())
	...
	htlcTxIDs := ht.AssertNumTxsInMempool(expectedTxes)
	...
	assertSweepFound(ht, alice, htlcSweepTx.Hash().String(), true)
	...
	err = wait.NoError(func() error {
		...
		// limbo.
		pendingChanResp := ht.GetPendingChannels(alice)
...
func findCommitAndAnchor(ht *lntest.HarnessTest,
	...
	for _, tx := range sweepTxns {
		txHash := tx.TxHash()
		sweepTx := ht.GetRawTransaction(&txHash)
...
func testFailingChannel(ht *lntest.HarnessTest) {
	...
	ht.AssertNumPendingCloseChannels(alice, 1, 0)
...
func assertSweepFound(ht *lntest.HarnessTest, hn *lntest.HarnessNode,
	...
	sweepResp := ht.ListSweeps(hn, verbose)

	var found bool
	if verbose {
		found = findSweepInDetails(ht, sweep, sweepResp)
	} else {
		found = findSweepInTxids(ht, sweep, sweepResp)
	}

	require.True(ht, found, "sweep: %v not found", sweep)
}

// NOTE: only used in current test file.
func findSweepInTxids(ht *lntest.HarnessTest, sweepTxid string,
...
// NOTE: only used in current test file.
func findSweepInDetails(ht *lntest.HarnessTest, sweepTxid string,
	sweepResp *walletrpc.ListSweepsResponse) bool {
...
//
// NOTE: only used in current test file.
func assertReports(ht *lntest.HarnessTest, hn *lntest.HarnessNode,
	...
	closed := ht.ClosedChannels(hn, false)
