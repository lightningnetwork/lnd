...
func testUpdateChannelPolicy(net *lntest.NetworkHarness, t *harnessTest) {
	...
	req = &lnrpc.PolicyUpdateRequest{
		BaseFeeMsat:   baseFee,
		Fee:           &lnrpc.PolicyUpdateRequest_FeeRate{float64(feeRate)},
		TimeLockDelta: timeLockDelta,
		MaxHtlcMsat:   maxHtlc,
	}
...
func testUpdateChannelPolicyFeeRateAccuracy(net *lntest.NetworkHarness,
	...
	// integer conversion.
	// We'll use a fee rate of 0.031337 which without rounding up would have
	// resulted in a fee rate ppm of 31336.
	feeRate := 0.031337
