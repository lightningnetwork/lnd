...
func testForwardInterceptor(net *lntest.NetworkHarness, t *harnessTest) {
	t.RunTestCase(&testCase{
		name: "test forward interceptor",
		test: testForwardInterceptorBasic,
	})
	t.RunTestCase(&testCase{
		name: "test forward interceptor dedup htlcs",
...
func testForwardInterceptorDedupHtlc(net *lntest.NetworkHarness, t *harnessTest) {
	...
	addResponse, err := tc.carol.AddInvoice(ctxb,
		&lnrpc.Invoice{
			ValueMsat: 1000,
		})
	require.NoError(tc.t.t, err, "unable to add invoice")

	invoice, err := tc.carol.LookupInvoice(ctxb,
		&lnrpc.PaymentHash{
			RHashStr: hex.EncodeToString(addResponse.RHash),
		})
	require.NoError(tc.t.t, err, "unable to find invoice")

	// We start the htlc interceptor with a simple implementation that saves all
	// intercepted packets. These packets are held to simulate a pending payment.
	...
	go func() {
		...
		for {
			...
			if err != nil {
				...
				require.NoError(
					tc.t.t, err, "unexpected error in interceptor.Recv()",
				)
				return
			}
			interceptedPacketstMap.Store(packet.IncomingCircuitKey.HtlcId, packet)
		}
	}()
	...
	// Here we should wait for the channel to contain a pending htlc.
	err = wait.Predicate(func() bool {
		channels, err := tc.bob.ListChannels(ctxt,
		...
		return len(channels.Channels[0].PendingHtlcs) > 0
	}, defaultTimeout)
	require.NoError(tc.t.t, err,
		"alice <> bob channel pending htlc never arrived")
	<-time.After(1 * time.Second)

	// At this point we want to make bob's link send all pending htlcs to the
	// switch again. We force this behavior by disconnecting and connecting to the
	// peer.
	if err := tc.net.DisconnectNodes(tc.bob,
		tc.alice); err != nil {

		tc.t.Fatalf("failed to disconnect alice and bob")
	}
	tc.net.EnsureConnected(tc.t.t, tc.bob,
		tc.alice)
	...
	require.NoError(tc.t.t, err,
		"alice <> bob channel didnt re-activate")

	// Now that the channel is active we make sure the test passes as expected.
	payments, err := tc.alice.ListPayments(ctxb,
	...
	require.Equal(tc.t.t, payments.Payments[0].Status,
	...
	// At this point if we have more than one held htlcs then we should fail.
	// This means we hold the same htlc twice which is a risk we want to
	// eliminate. If we dont have the same htlc twice in theory we can cancel one
	// and settle the other by mistake.
