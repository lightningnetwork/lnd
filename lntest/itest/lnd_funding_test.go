...
func testChannelFundingInputTypes(ht *lntest.HarnessTest) {
	...
	for _, addrType := range addrTypes {
		// We'll send her some confirmed funds.
		ht.SendCoinsOfType(chanAmt*2, carol, addrType, true)
...
func sendAllCoinsConfirm(ht *lntest.HarnessTest, node *lntest.HarnessNode,
	addr string) {
	...
	ht.SendCoinFromNode(node, sweepReq)
