...
func (h *holdSubscription) cancel(ht *lntest.HarnessTest) {
	ht.CancelInvoice(h.recipient, h.hash[:])
	...
	require.Equal(ht, lnrpc.PaymentFailureReason_FAILURE_REASON_INCORRECT_PAYMENT_DETAILS,
...
func acceptHoldInvoice(ht *lntest.HarnessTest, idx int, sender,
	...
	invoice := ht.AddHoldInvoice(req, receiver)

	invStream := ht.SubscribeSingleInvoice(receiver, hash[:])
	...
	payStream := ht.SendPayment(sender, sendReq)
