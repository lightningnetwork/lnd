...
func testMultiHopHtlcLocalChainClaim(ht *lntest.HarnessTest,
	...
	bobForceClose := ht.CloseChannelAndAssertType(
		bob, aliceChanPoint, hasAnchors, true,
	)
	...
	ht.AssertTxInBlock(block, &bobSecondLvlTxid)
	...
	block = ht.MineBlocksAndAssertTx(bobSecondLevelCSV, 1)[0]
	...
	if c == lnrpc.CommitmentType_SCRIPT_ENFORCED_LEASE {
		...
		aliceCommitSweep := ht.AssertTxInMempool(
			aliceCommitOutpoint,
		).TxHash()
		bobCommitOutpoint := wire.OutPoint{Hash: closingTxid, Index: 3}
		bobCommitSweep := ht.AssertTxInMempool(
			bobCommitOutpoint,
		).TxHash()

		// Confirm their sweeps.
		block := ht.MineBlocksAndAssertTx(1, 2)[0]
		ht.AssertTxInBlock(block, &aliceCommitSweep)
		ht.AssertTxInBlock(block, &bobCommitSweep)
	}
