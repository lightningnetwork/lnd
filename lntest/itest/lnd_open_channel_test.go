...
func testOpenChannelAfterReorg(ht *lntest.HarnessTest) {
	...
	require.NoError(ht, err, "failed to create temp miner")
	defer func() {
		require.NoError(ht, tempMiner.Stop(),
	...
	// open.
	block := ht.MineBlocksAndAssertTx(10, 1)[0]
	...
	ht.WaitForNodeBlockHeight(alice, minerHeight)
