...
func testRemoteSigner(net *lntest.NetworkHarness, t *harnessTest) {
	...
	defer shutdownAndAssert(net, t, signer)
	...
	require.NoError(t.t, err)
	defer shutdownAndAssert(net, t, watchOnly)

	resp, err := watchOnly.GetInfo(ctxb, &lnrpc.GetInfoRequest{})
	require.NoError(t.t, err)

	require.Equal(t.t, nodePubKey, resp.IdentityPubkey)

	// Let's connect up the watch-only node to Alice and make sure we can
	// use every operation as we normally would.
	net.EnsureConnected(t.t, watchOnly, net.Alice)
	net.SendCoins(t.t, btcutil.SatoshiPerBitcoin, watchOnly)
	chanPoint1 := openChannelAndAssert(
		t, net, watchOnly, net.Alice, lntest.OpenChannelParams{
			Amt: 1234567,
		},
	)

	// With the channels open, we'll create invoices for Alice that the
	// watch-only node will pay to in order to advance the state of the
	// channel.
	...
	defer cancel()
	require.NoError(
		t.t, watchOnly.WaitForNetworkChannelOpen(ctxt, chanPoint1),
	)

	// Make sure the watch-only wallet is ready for sending payments.
	waitForGraphSync(t, watchOnly)
	channelBalance := getChannelBalance(t, watchOnly)
	require.Greater(
		t.t, channelBalance.LocalBalance.Sat,
		uint64(paymentAmt*numInvoices),
	)
	info, err := getChanInfo(watchOnly)
	require.NoError(t.t, err)
	require.Equal(t.t, true, info.Active)

	// Send payments from watch-only to Alice in an async manner.
	...
	// We're done sending payments. Make sure we can coop close the channel.
	closeChannelAndAssert(t, net, watchOnly, chanPoint1, false)

	// Make sure we can also open and then force-close a channel.
	...
	require.NoError(
		t.t, watchOnly.WaitForNetworkChannelOpen(ctxt, chanPoint2),
	)
	closeChannelAndAssert(t, net, watchOnly, chanPoint2, true)
	cleanupForceClose(t, net, watchOnly, chanPoint2)
}
