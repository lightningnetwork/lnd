...
func testResHandoff(ht *lntest.HarnessTest) {
	...
	ht.CompletePaymentRequests(alice, carolPayReqs, false)
	...
	ht.AssertNumChannelUpdates(carol, chanPointCarol, 1)

	// Before we shutdown Alice, we'll assert that she only has 1 update.
	ht.AssertNumChannelUpdates(alice, chanPointAlice, 1)
	...
	ht.CloseChannel(bob, chanPointCarol, true)
	...
	// message.
	ht.AssertNumChannelUpdates(alice, chanPointAlice, 2)
