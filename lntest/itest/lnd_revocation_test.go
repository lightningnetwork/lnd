...
func testRevokedCloseRetribution(ht *lntest.HarnessTest) {
	...
	bobChan := ht.AssertLocalBalance(bob, chanPoint, 30_000)
	...
	ht.AssertNumUpdates(bob, bobStateNumPreCopy, chanPoint)
	...
	closeUpdates := ht.CloseChannelStreamAndAssert(bob, chanPoint, true)

	// Wait for Bob's breach transaction to show up in the mempool to
	// ensure that Carol's node has started waiting for confirmations.
	...
	breachTXID := ht.WaitForChannelClose(closeUpdates)
...
func testRevokedCloseRetributionZeroValueRemoteOutput(ht *lntest.HarnessTest) {
	...
	carolChan := ht.AssertLocalBalance(carol, chanPoint, 0)
	...
	ht.CompletePaymentRequests(dave, carolPayReqs, false)
	...
	// state number in the past, not the *latest* state.
	ht.AssertNumUpdates(carol, carolStateNumPreCopy, chanPoint)

	// Now force Carol to execute a *force* channel closure by unilaterally
	// broadcasting her current channel state. This is actually the
	// commitment transaction of a prior *revoked* state, so she'll soon
	// feel the wrath of Dave's retribution.
	closeUpdates := ht.CloseChannelStreamAndAssert(carol, chanPoint, true)
...
func testRevokedCloseRetributionRemoteHodl(ht *lntest.HarnessTest) {
	...
	// generated above.
	ht.CompletePaymentRequests(carol, davePayReqs[:numInvoices/2], false)
	...
	carolChan := ht.AssertLocalBalance(
...
func testRevokedCloseRetributionAltruistWatchtowerCase(ht *lntest.HarnessTest,
	...
	// capacity.
	carolChan := ht.AssertLocalBalance(carol, chanPoint, int64(chanAmt/4))
	...
	// shutting down Dave.
	err := wait.NoError(func() error {
		bkpStats := ht.WatchtowerStats(dave)
		if bkpStats == nil {
	...
	willyBalResp := ht.GetWalletBalance(willy)
	...
	err = wait.NoError(func() error {
		daveBalResp := ht.GetChannelBalance(dave)
