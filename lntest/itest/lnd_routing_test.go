		...
		ht.Run(test.name, func(t1 *testing.T) {
			...
			st.RunTestCase(&lntest.TestCase{
				...
				TestFunc: func(tt *lntest.HarnessTest) {
					testSingleHopSendToRouteCase(tt, test)
				},
			})
		})
	}
...
func testSingleHopSendToRouteCase(ht *lntest.HarnessTest,
	...
	defer ht.CloseChannel(carol, chanPointCarol, false)

	// Create invoices for Dave, which expect a payment from Carol.
	payReqs, rHashes, _ := ht.CreatePayReqs(dave, paymentAmtSat, numPayments)

	// Reconstruct payment addresses.
	var payAddrs [][]byte
	for _, payReq := range payReqs {
		resp := ht.DecodePayReq(dave, payReq)
	...
	routes := ht.QueryRoutes(carol, routesReq)
	...
	sendToRouteSync := func() {
		for i, rHash := range rHashes {
			...
			resp := ht.SendToRouteSync(carol, sendReq)
	...
	sendToRouteStream := func() {
		alicePayStream := ht.SendToRoute(carol)
	...
	sendToRouteRouterRPC := func() {
		for i, rHash := range rHashes {
			...
			resp := ht.SendToRouteV2(carol, sendReq)
...
func runMultiHopSendToRoute(ht *lntest.HarnessTest, useGraphCache bool) {
	...
	defer ht.CloseChannel(carol, chanPointBob, false)
	...
	for i, rHash := range rHashes {
		...
		resp := ht.SendToRouteV2(alice, sendReq)
...
func testSendToRouteErrorPropagation(ht *lntest.HarnessTest) {
	...
	chanPointCarol := ht.OpenChannel(
		carol, charlie, lntest.OpenChannelParams{Amt: chanAmt},
	)
	defer ht.CloseChannel(carol, chanPointCarol, false)
	...
	fakeRoute := ht.QueryRoutes(carol, fakeReq)
	...
	resp := ht.AddInvoice(invoice, bob)
	rHash := resp.RHash

	// Using Alice as the source, pay to the 5 invoices from Bob created
	// above.
	...
	if err != nil {
		require.Contains(ht, err.Error(), "edge not found")
	} else {
		require.NoError(ht, err, "payment stream has been closed "+
			"but fake route has consumed")
	}
}
...
func testPrivateChannels(ht *lntest.HarnessTest) {
	...
	defer ht.CloseChannel(dave, chanPointDave, false)
	...
	defer ht.CloseChannel(carol, chanPointPrivate, false)
...
func testMultiHopOverPrivateChannels(ht *lntest.HarnessTest) {
	...
	ht.AssertChannelOpen(dave, chanPointBob)
	...
	ht.CloseChannel(bob, chanPointBob, false)
...
func testQueryRoutes(ht *lntest.HarnessTest) {
	...
	testMissionControlCfg(ht, alice)
...
// tests are affected.
func testMissionControlCfg(ht *lntest.HarnessTest, hn *lntest.HarnessNode) {
	startCfg := ht.GetMissionControlConfig(hn)
	...
	ht.SetMissionControlConfig(hn, cfg)

	resp := ht.GetMissionControlConfig(hn)
	require.True(ht, proto.Equal(cfg, resp.Config))

	ht.SetMissionControlConfig(hn, startCfg.Config)

	resp = ht.GetMissionControlConfig(hn)
	require.True(ht, proto.Equal(startCfg.Config, resp.Config))
}

// testMissionControlImport tests import of mission control results from an
// external source.
func testMissionControlImport(ht *lntest.HarnessTest, hn *lntest.HarnessNode,
	fromNode, toNode []byte) {

	// Reset mission control so that our query will return the default
	// probability for our first request.
	ht.ResetMissionControl(hn)
	...
	resp1 := ht.QueryProbability(hn, probReq)
	...
	ht.XImportMissionControl(hn, req)

	resp2 := ht.QueryProbability(hn, probReq)
	...
	ht.XImportMissionControlAssertErr(hn, req)
}
