...
func testSingleHopInvoice(ht *lntest.HarnessTest) {
	...
	assertAmountPaid := func(expected int64) {
		ht.AssertAmountPaid("alice -> bob", alice, chanPoint, expected, 0)
		ht.AssertAmountPaid("bob <- alice", bob, chanPoint, 0, expected)
	}
	...
	resp := ht.SendPaymentAndAssert(alice, req)
	require.Equal(ht, hex.EncodeToString(preimage), resp.PaymentPreimage,
		"preimage mismatch")

	// Bob's invoice should now be found and marked as settled.
	dbInvoice := ht.LookupInvoice(bob, invoiceResp.RHash)
	...
	// internally.
	keysendInvoice := ht.LookupInvoice(bob, keySendHash[:])
	...
	invoiceResp = ht.AddInvoice(invoice, bob)
	payreq := ht.DecodePayReq(bob, invoiceResp.PaymentRequest)
