...
func testWipeForwardingPackages(ht *lntest.HarnessTest) {
	...
	// and Carol.
	ht.AssertChannelOpen(alice, chanPointBC)
	...
	for i := 0; i < numInvoices; i++ {
		...
		invoiceResp := ht.AddInvoice(invoice, carol)
	...
	ht.CloseChannelStreamAndAssert(bob, chanPointAB, true)
	...
	ht.CloseChannelStreamAndAssert(bob, chanPointBC, false)
