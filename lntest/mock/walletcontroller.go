...
func (w *WalletController) NewAddress(lnwallet.AddressType, bool,
	string) (btcaddr.Address, error) {

	pkh := btcaddr.Hash160(w.RootKey.PubKey().SerializeCompressed())
	addr, _ := btcaddr.NewAddressPubKeyHash(pkh, &chaincfg.MainNetParams)
	return addr, nil
}
...
func (w *WalletController) IsOurAddress(btcaddr.Address) bool {
	return false
}

// AddressInfo currently returns a dummy value.
func (w *WalletController) AddressInfo(
	btcaddr.Address) (waddrmgr.ManagedAddress, error) {
...
func (w *WalletController) ImportAccount(string, *hdkeychain.ExtendedKey,
	...
	[]btcaddr.Address, []btcaddr.Address, error) {
