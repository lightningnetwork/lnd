...
func Bip32DerivationFromAddress(
	...
	*psbt.TaprootBip32Derivation, error) {

	pubKeyAddr, ok := addr.(waddrmgr.ManagedPubKeyAddress)
	if !ok {
		return nil, nil, fmt.Errorf("address is not a pubkey address")
	}

	scope, derivationPath, haveInfo := pubKeyAddr.DerivationInfo()
	if !haveInfo {
		return nil, nil, fmt.Errorf("address is an imported public " +
			"key, can't derive BIP32 path")
	}
	...
	bip32Derivation := &psbt.Bip32Derivation{
		...
		Bip32Path: []uint32{
			...
			derivationPath.Account + hdkeychain.HardenedKeyStart,
