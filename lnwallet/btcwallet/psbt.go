...
func sumInputPubKeys(packet *psbt.Packet) (*btcec.PublicKey, error) {
	...
	for idx := range packet.Inputs {
		...
		if result == nil {
			...
			result = silentpayments.Add(result, pubKey)
		}
	}
...
func sumInputPrivKeys(packet *psbt.Packet,
	...
	// If the result is still nil here, we weren't able to extract any
	// private keys.
	if result == nil {
		return nil, fmt.Errorf("unable to sum input private keys: no " +
			"private keys found")
	}
