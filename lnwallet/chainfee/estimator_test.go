...
func TestRandomFeeUpdateTimeout(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		...
		require.Truef(t, timeout >= minFeeUpdateTimeout, "timeout %v "+
			" is less than minFeeUpdateTimeout %v",
			timeout, minFeeUpdateTimeout)
		require.Truef(t, timeout <= maxFeeUpdateTimeout, "timeout %v "+
			"is greater than maxFeeUpdateTimeout %v",
			timeout, maxFeeUpdateTimeout)
	}
}
