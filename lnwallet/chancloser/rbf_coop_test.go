...
var (
	...
	remoteSigBytes = fromHex("304502210082235e21a2300022738dabb8e1bbd9d19cfb1e" +
		"7ab8c30a23b0afbb8d178abcf3022024bf68e256c534ddfaf966bf908de" +
		"b944305596f7bdcc38d69acad7f9c868724")
...
func newChainHash(s []byte) chainhash.Hash {
...
func assertState[Event any, Env protofsm.Environment](t *testing.T,
	m *protofsm.StateMachine[Event, Env],
...
func assertUnknownEventFail(t *testing.T, startingState ProtocolState) {
	...
	t.Run("unknown_event", func(t *testing.T) {
		...
		closeHarness.expectFaliure(ErrInvalidStateTransition)
...
var failAddrErr = fmt.Errorf("fail")

// failNewAddrFunc causes the newAddrFunc to fail.
func (r *rbfCloserTestHarness) failNewAddrFunc() {
	r.T.Helper()

	r.newAddrErr = failAddrErr
}

// successNewAddrFunc causes the newAddrFunc to succeed.
func (r *rbfCloserTestHarness) successNewAddrFunc() {
	r.T.Helper()
...
func (r *rbfCloserTestHarness) startFSM() {
	r.T.Helper()
...
func (r *rbfCloserTestHarness) assertState(expectedState RbfState) {
	r.T.Helper()

	assertState(r.T, r.chanCloser, expectedState)
}
...
func (r *rbfCloserTestHarness) expectShutdownEvents(expect shutdownExpect) {
	...
	chanPoint := r.env.ChanPoint

	r.daemonAdapters.On("DisableChannel", chanPoint).Return(nil)
}

func (r *rbfCloserTestHarness) expectFinalBalances(b fn.Option[ShutdownBalances]) {
...
func (r *rbfCloserTestHarness) expectFaliure(err error) {
	r.T.Helper()
...
func (r *rbfCloserTestHarness) expectNewCloseSig(
	...
	closeBalance btcutil.Amount, sequence uint32) {
...
func (r *rbfCloserTestHarness) expectRemoteCloseFinalized(
	...
	balanceAfterClose btcutil.Amount, isLocal bool, sequence uint32) {

	r.expectNewCloseSig(
		localScript, remoteScript, fee, balanceAfterClose, sequence,
	)
...
func (r *rbfCloserTestHarness) assertLocalClosePending() {
	...
	// close txn is known.
	closePendingState, ok := currentState.PeerState.LocalState.(*ClosePending)
	require.True(r.T, ok)
...
func (r *rbfCloserTestHarness) expectHalfSignerIteration(
	...
	offerSentState, ok := currentState.PeerState.LocalState.(*LocalOfferSent)
	require.True(r.T, ok)
...
func (r *rbfCloserTestHarness) assertSingleRemoteRbfIteration(
	...
	r.expectRemoteCloseFinalized(
		...
		absoluteFee, balanceAfterClose, false, sequence,
	)
	...
	// From this, we'll assert the resulting peer state.
	pendingState, ok := currentState.PeerState.RemoteState.(*ClosePending)
	require.True(r.T, ok)
...
func noAddr() fn.Option[lnwire.DeliveryAddress] {
...
func TestRbfChannelActiveTransitions(t *testing.T) {
	...
	t.Run("local_initiated_close_addr_fail", func(t *testing.T) {
		...
		closeHarness.expectFaliure(failAddrErr)
...
// a shtudown to be received, or a notification that were were able to send a
// shutdown ourselves.
func TestRbfShutdownPendingTransitions(t *testing.T) {
	...
	t.Run("initiator_shutdown_recv_validate_fail", func(t *testing.T) {
		...
		// expected.
		closeHarness.expectFaliure(ErrUpfrontShutdownScriptMismatch)
...
func TestRbfChannelFlushingTransitions(t *testing.T) {
	...
	for _, isFreshFlush := range []bool{true, false} {
		...
		flushEvent.ShutdownBalances.LocalBalance = lnwire.NewMSatFromSatoshis( //nolint:lll
			absoluteFee * 3,
		)
...
func TestRbfCloseClosingNegotiationLocal(t *testing.T) {
	...
	t.Run("send_offer_too_many_sigs_received", func(t *testing.T) {
		...
		// than one error.
		closeHarness.expectFaliure(ErrTooManySigs)
...
func TestRbfCloseClosingNegotiationRemote(t *testing.T) {
	...
	t.Run("recv_offer_cannot_pay_for_fees", func(t *testing.T) {
		...
		closeHarness.expectFaliure(ErrRemoteCannotPay)
	...
	t.Run("recv_offer_final_sequence", func(t *testing.T) {
		...
		closeHarness.expectFaliure(ErrNonFinalSequence)
	...
	t.Run("recv_offer_err_closer_no_closee", func(t *testing.T) {
		...
		closeHarness.expectFaliure(ErrCloserNoClosee)
		...
		feeOffer := &OfferReceivedEvent{
			SigMsg: lnwire.ClosingComplete{
				FeeSatoshis: absoluteFee,
				ClosingSigs: lnwire.ClosingSigs{
					CloserAndClosee: newSigTlv[tlv.TlvType3]( //nolint:lll
						remoteWireSig,
					),
				},
			},
	...
	t.Run("recv_offer_err_closer_and_closee", func(t *testing.T) {
		...
		// We should fail as they included the wrong sig.
		closeHarness.expectFaliure(ErrCloserAndClosee)
		...
		feeOffer := &OfferReceivedEvent{
			SigMsg: lnwire.ClosingComplete{
				FeeSatoshis: absoluteFee,
				ClosingSigs: lnwire.ClosingSigs{
					CloserNoClosee: newSigTlv[tlv.TlvType1]( //nolint:lll
						remoteWireSig,
					),
				},
			},
	...
	t.Run("recv_offer_rbf_loop_iterations", func(t *testing.T) {
		...
		// aren't yet dust.
		closingTerms := *closeTerms
		closingTerms.ShutdownBalances.LocalBalance = lnwire.NewMSatFromSatoshis( //nolint:lll
			9000,
		)
