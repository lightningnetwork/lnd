...
func (pd *PaymentDescriptor)GetMsgAmt() lnwire.UnitPrec11{
	if pd.AssetID==lnwire.BtcAssetId{
		return lnwire.UnitPrec11(pd.BtcAmount)
	}
	return lnwire.UnitPrec11(pd.AssetAmount)
}
func (pd *PaymentDescriptor)SetAmtFromHtlcMsg(htlc *lnwire.UpdateAddHTLC){
	if htlc.AssetID==lnwire.BtcAssetId{
		pd.BtcAmount=lnwire.MilliSatoshi(htlc.Amount)
	}else{
		pd.AssetAmount=omnicore.Amount(htlc.Amount)
		pd.BtcAmount=lnwire.NewMSatFromSatoshis(lnwire.OmniGas*3)
	}
}
	...
	for i, logUpdate := range logUpdates {
		...
		case *lnwire.UpdateAddHTLC:
			pd = PaymentDescriptor{
				...
				//BtcAmount:    wireMsg.BtcAmount,
				//AssetAmount:    wireMsg.AssetAmount,
				AssetID: 	wireMsg.AssetID,
				EntryType: Add,
...
func locateOutputIndex(p *PaymentDescriptor, tx *wire.MsgTx, ourCommit bool,
	...
	obd add wxf
	*/
	//opreturn output.value is 0
	if tx.TxOut[0].Value ==0{
		cltvs=append([]uint32{0},cltvs...)
	}
	for i, txOut := range tx.TxOut {
		cltv := cltvs[i]

		if bytes.Equal(txOut.PkScript, pkScript) &&
			txOut.Value == int64(p.BtcAmount.ToSatoshis()) &&
			cltv == p.Timeout {
	...
	return 0, fmt.Errorf("unable to find htlc: script=%x, value=%v, "+
		"cltv=%v", pkScript, p.BtcAmount, p.Timeout)
}
...
func (c *commitment) populateHtlcIndexes(chanType channeldb.ChannelType,
	...
	populateIndex := func(htlc *PaymentDescriptor, incoming bool) error {
		...
		//	htlc.BtcAmount.ToSatoshis(), c.dustLimit,
		//)
...
func (c *commitment) toDiskCommit(ourCommit bool) *channeldb.ChannelCommitment {
	...
	commit := &channeldb.ChannelCommitment{
		...
		LocalBtcBalance:    c.ourBtcBalance,
		RemoteBtcBalance:   c.theirBtcBalance,
		LocalAssetBalance:    c.ourAssetBalance,
		RemoteAssetBalance:   c.theirAssetBalance,
		CommitFee:       c.fee,
	...
	for _, htlc := range c.outgoingHTLCs {
		...
		h := channeldb.HTLC{
			RHash:         htlc.RHash,
			BtcAmt:           htlc.BtcAmount,
			AssetAmt:           htlc.AssetAmount,
...
func (lc *LightningChannel) diskHtlcToPayDesc(feeRate chainfee.SatPerKWeight,
	...
	remoteCommitKeys *CommitmentKeyRing, isLocal bool,assetId uint32) (PaymentDescriptor,
	error) {
	...
	isDustLocal := HtlcIsDust(
		...
		htlc.BtcAmt.ToSatoshis(), lc.channelState.LocalChanCfg.DustLimit,
		assetId,
	)
	...
	isDustRemote := HtlcIsDust(
		...
		htlc.BtcAmt.ToSatoshis(),  lc.channelState.RemoteChanCfg.DustLimit,
		assetId,
	)
	...
	pd = PaymentDescriptor{
		...
		BtcAmount:             htlc.BtcAmt,
		AssetAmount:             htlc.AssetAmt,
		EntryType:          Add,
		HtlcIndex:          htlc.HtlcIndex,
		LogIndex:           htlc.LogIndex,
...
func (lc *LightningChannel) diskCommitToMemCommit(isLocal bool,
	...
	incomingHtlcs, outgoingHtlcs, err := lc.extractPayDescs(
		...
		isLocal,lc.channelState.AssetID,
	)
	if err != nil {
		return nil, err
	}
	...
	commit := &commitment{
		...
		isOurs:            isLocal,
		ourBtcBalance:        diskCommit.LocalBtcBalance,
		theirBtcBalance:      diskCommit.RemoteBtcBalance,
		ourAssetBalance:        diskCommit.LocalAssetBalance,
		theirAssetBalance:      diskCommit.RemoteAssetBalance,
...
func NewLightningChannel(signer input.Signer,
	...
	lc := &LightningChannel{
		...
		BtcCapacity:          state.BtcCapacity,
		AssetCapacity:        state.AssetCapacity,
...
func (lc *LightningChannel) createSignDesc() error {
	...
	lc.signDesc = &input.SignDescriptor{
		...
		Output: &wire.TxOut{
			...
			Value:    int64(lc.channelState.BtcCapacity),
		},
...
func (lc *LightningChannel) logUpdateToPayDesc(logUpdate *channeldb.LogUpdate,
	...
	case *lnwire.UpdateAddHTLC:
		...
		pd.SetAmtFromMsgHtlc(wireMsg)
		pd.OnionBlob = make([]byte, len(wireMsg.OnionBlob))
		copy(pd.OnionBlob[:], wireMsg.OnionBlob[:])

		isDustRemote := HtlcIsDust(
			lc.channelState.ChanType, false, false, feeRate,
			lnwire.MilliSatoshi(wireMsg.Amount).ToSatoshis(), remoteDustLimit,wireMsg.AssetID,
		)
		if !isDustRemote {
			theirP2WSH, theirWitnessScript, err := genHtlcScript(
	...
	case *lnwire.UpdateFulfillHTLC:
		...
		pd = &PaymentDescriptor{
			AssetID: 					ogHTLC.AssetID,
			BtcAmount:                   ogHTLC.BtcAmount,
			AssetAmount:                   ogHTLC.AssetAmount,
			RHash:                    ogHTLC.RHash,
			RPreimage:                wireMsg.PaymentPreimage,
			LogIndex:                 logUpdate.LogIndex,
	...
	case *lnwire.UpdateFailMalformedHTLC:
		...
		pd = &PaymentDescriptor{		AssetID: 					ogHTLC.AssetID,
			BtcAmount:                   ogHTLC.BtcAmount,
			AssetAmount:                   ogHTLC.AssetAmount,
			RHash:                    ogHTLC.RHash,
			ParentIndex:              ogHTLC.HtlcIndex,
	...
	case *lnwire.UpdateFee:
		pd = &PaymentDescriptor{
			LogIndex: logUpdate.LogIndex,
			BtcAmount: lnwire.NewMSatFromSatoshis(
...
func NewBreachRetribution(chanState *channeldb.OpenChannel, stateNum uint64,
	...
	for _, htlc := range revokedSnapshot.Htlcs {
		...
		if HtlcIsDust(
			...
			htlc.BtcAmt.ToSatoshis(), chanState.RemoteChanCfg.DustLimit,chanState.AssetID,
		) {
			continue
		}
...
func HtlcIsDust(chanType channeldb.ChannelType,
	...
	htlcAmt , dustLimit btcutil.Amount, assetId uint32) bool {
	/*
		obd update wxf
	*/
	if assetId != lnwire.BtcAssetId {
		htlcAmt = lnwire.OmniGas * 3
	}
	...
	res:=(htlcAmtBtc - htlcFee) < dustLimitBtc
	if res{
		log.Printf("dust htlc assetId %v,htlcAmtBtc %v,htlcFee %v,dustLimitBtc %v",
			assetId,htlcAmtBtc,htlcFee,dustLimitBtc)
	}
	return  res
}
...
func (lc *LightningChannel) fetchCommitmentView(remoteChain bool,
	...
	ourBtcBalance, theirBtcBalance, ourAssetBalance, theirAssetBalance, _, filteredHTLCView, err := lc.computeView(
	...
	commitTx, err := lc.commitBuilder.createUnsignedCommitmentTx(
		ourBtcBalance, theirBtcBalance, ourAssetBalance, theirAssetBalance, !remoteChain, feePerKw, nextHeight,
	...
	fee := lc.channelState.BtcCapacity - totalOut
	...
	/*obd update wxf : assetBalanceUpdate
	todo set ourBtcBalance when close channel
	*/
	ourBtcAmt:=commitTx.ourBtcBalance
	theirBtcAmt:=commitTx.theirBtcBalance
	//if lc.commitBuilder.chanState.AssetID>1 {
	//	ourBtcAmt = lnwire.NewMSatFromSatoshis(lc.commitBuilder.chanState.BtcCapacity)
	//	theirBtcAmt = ourBtcAmt
	//}

	// With the commitment view created, store the resulting balances and
	// transaction with the other parameters for this height.
	c := &commitment{
		ourBtcBalance:        ourBtcAmt,
		theirBtcBalance:      theirBtcAmt,
		ourAssetBalance:        commitTx.ourAssetBalance,
		theirAssetBalance:      commitTx.theirAssetBalance,
		txn:               commitTx.txn,
		fee:               commitTx.fee,
...
// method.
func (lc *LightningChannel) evaluateHTLCView(view *htlcView, ourBtcBalance,
	theirBtcBalance *lnwire.MilliSatoshi,ourAssetBalance,
	theirAssetBalance *omnicore.Amount, nextHeight uint64,
	...
	for _, entry := range view.ourUpdates {
		...
		if mutateState && entry.EntryType == Settle && !remoteChain &&
			...
			lc.channelState.TotalMSatReceived += entry.BtcAmount
			/*
			obd update wxf
			*/
			lc.channelState.TotalAssetReceived += entry.AssetAmount
		}
		...
		processRemoveEntry(entry, ourBtcBalance, theirBtcBalance,
	...
	for _, entry := range view.theirUpdates {
		...
		if mutateState && entry.EntryType == Settle && !remoteChain &&
			entry.removeCommitHeightLocal == 0 {
			lc.channelState.TotalMSatSent += entry.BtcAmount
			lc.channelState.TotalAssetReceived += entry.AssetAmount
		}
	...
	for _, entry := range view.ourUpdates {
		...
		processAddEntry(entry, ourBtcBalance, theirBtcBalance,
			ourAssetBalance, theirAssetBalance, nextHeight,
...
func processAddEntry(htlc *PaymentDescriptor, ourBtcBalance, theirBtcBalance *lnwire.MilliSatoshi,
	ourAssetBalance, theirAssetBalance *omnicore.Amount,
	...
	if isIncoming {
		...
		*theirBtcBalance -= htlc.BtcAmount
		*theirAssetBalance -= htlc.AssetAmount
	} else {
		// Similarly, we need to debit our balance if this is an out
		// going HTLC to reflect the pending balance.
		*ourBtcBalance -= htlc.BtcAmount
		*ourAssetBalance -= htlc.AssetAmount
	}
...
// is skipped.
func processRemoveEntry(htlc *PaymentDescriptor, ourBtcBalance, theirBtcBalance *lnwire.MilliSatoshi,
	ourAssetBalance, theirAssetBalance *omnicore.Amount, nextHeight uint64,
...
func processFeeUpdate(feeUpdate *PaymentDescriptor, nextHeight uint64,
	...
	view.feePerKw = chainfee.SatPerKWeight(feeUpdate.BtcAmount.ToSatoshis())
...
func genRemoteHtlcSigJobs(keyRing *CommitmentKeyRing,
	...
	for _, htlc := range remoteCommitView.incomingHTLCs {
		...
		htlcFee := HtlcTimeoutFee(chanType, feePerKw)
		outputAmt := htlc.BtcAmount.ToSatoshis() - htlcFee
...
func (pd *PaymentDescriptor)SetAmtFromMsgHtlc( htlc *lnwire.UpdateAddHTLC){
	...
	if htlc.AssetID==lnwire.BtcAssetId{//btc
		pd.BtcAmount = lnwire.MilliSatoshi(htlc.Amount)
	}else{//asset
		pd.AssetAmount=omnicore.Amount(htlc.Amount)
		pd.BtcAmount=lnwire.NewMSatFromSatoshis(lnwire.OmniGas*3)
...
func (pd *PaymentDescriptor)GetHtlcAmt() lnwire.UnitPrec11{
	if pd.AssetID==lnwire.BtcAssetId{
		return lnwire.UnitPrec11(pd.BtcAmount)
	}else{
		return lnwire.UnitPrec11(pd.AssetAmount)
	...
	for e := lc.localUpdateLog.Front(); e != nil; e = e.Next() {
		...
		case Add:
			htlc := &lnwire.UpdateAddHTLC{
				...
				ID:          pd.HtlcIndex,
				AssetID: pd.AssetID,
				Amount: pd.GetHtlcAmt(),
				//AssetAmount: pd.AssetAmount,
				//BtcAmount:      pd.BtcAmount,
				Expiry:      pd.Timeout,
		...
		case FeeUpdate:
			...
			logUpdate.UpdateMsg = &lnwire.UpdateFee{
				ChanID:   chanID,
				AssetID:   pd.AssetID,
				FeePerKw: uint32(pd.BtcAmount.ToSatoshis()),
			}
		}
...
func (lc *LightningChannel) validateCommitmentSanity(theirLogCounter,
	...
	if predictOurAdd != nil && predictOurAdd.AssetAmount > 0 && predictOurAdd.AssetID == lnwire.BtcAssetId {
	...
	/*
		...
		todo check Asset
	*/
	if predictOurAdd != nil && predictOurAdd.AssetID !=lnwire.BtcAssetId {
		return nil
	}
	if predictTheirAdd != nil && predictTheirAdd.AssetID !=lnwire.BtcAssetId {
		return nil
	}

	if predictTheirAdd != nil && predictTheirAdd.AssetAmount > 0 && predictTheirAdd.AssetID == lnwire.BtcAssetId {
	...
	ourInitialBalance := lnwire.UnitPrec11(commitChain.tip().ourAssetBalance)
	theirInitialBalance := lnwire.UnitPrec11(commitChain.tip().theirAssetBalance)
	if lc.channelState.AssetID == lnwire.BtcAssetId {
		ourInitialBalance = lnwire.UnitPrec11(commitChain.tip().ourBtcBalance)
		theirInitialBalance = lnwire.UnitPrec11(commitChain.tip().theirBtcBalance)
	}

	/*
	bod update wxf
	todo check  ourAssetBalance, theirAssetBalance
	*/
	...
	switch {
	case int64(ourBtcBalance) < 0:
		return ErrBelowChanReserve
	case int64(theirBtcBalance) < 0:
		return ErrBelowChanReserve
	}
	...
	ourAmt:=lnwire.UnitPrec11(ourAssetBalance)
	if lc.channelState.AssetID==lnwire.BtcAssetId{
		ourAmt=lnwire.UnitPrec11(ourBtcBalance)
	}
	thireAmt:=lnwire.UnitPrec11(theirAssetBalance)
	ourReserve:=lnwire.UnitPrec11(lc.channelState.LocalChanCfg.ChanReserve)
	theirReserve:=lnwire.UnitPrec11(lc.channelState.RemoteChanCfg.ChanReserve)
	if lc.channelState.AssetID==lnwire.BtcAssetId{
		thireAmt=lnwire.UnitPrec11(theirBtcBalance)
		ourReserve=lnwire.UnitPrec11(lc.channelState.LocalChanCfg.ChanReserve.ToMsat())
		theirReserve=lnwire.UnitPrec11(lc.channelState.RemoteChanCfg.ChanReserve.ToMsat())
	}
	...
	validateUpdates := func(updates []*PaymentDescriptor,
		...
		var amtInFlight lnwire.UnitPrec11
...
func (lc *LightningChannel) SignNextCommitment() (lnwire.Sig, []lnwire.Sig, []channeldb.HTLC, error) {
	...
	obd  update wxf
	*/
	lc.log.Tracef("remote chain: our_btc_balance=%v, their_btc_balance=%v," +
		" our_asset_balance=%v, their_asset_balance=%v, commit_tx: %v",
		newCommitView.ourBtcBalance,
		newCommitView.theirBtcBalance,
		newCommitView.ourAssetBalance,
		newCommitView.theirAssetBalance,
		newLogClosure(func() string {
...
func (lc *LightningChannel) computeView(view *htlcView, remoteChain bool,
	updateState bool) (lnwire.MilliSatoshi, lnwire.MilliSatoshi ,
     omnicore.Amount, omnicore.Amount, int64, *htlcView, error) {
	...
	// state.
	ourBtcBalance := commitChain.tip().ourBtcBalance
	theirBtcBalance := commitChain.tip().theirBtcBalance
	/*
	obd add wxf
	*/
	ourAssetBalance := commitChain.tip().ourAssetBalance
	theirAssetBalance := commitChain.tip().theirAssetBalance
	...
	filteredHTLCView, err := lc.evaluateHTLCView(view, &ourBtcBalance,
		&theirBtcBalance,&ourAssetBalance,&theirAssetBalance, nextHeight, remoteChain, updateState)
	if err != nil {
	...
	for _, htlc := range filteredHTLCView.ourUpdates {
		if HtlcIsDust(
			...
			feePerKw, htlc.BtcAmount.ToSatoshis(), dustLimit, htlc.AssetID,
		) {
			continue
		}
	...
	totalCommitWeight := CommitWeight(lc.channelState.ChanType) +
		totalHtlcWeight
	return ourBtcBalance, theirBtcBalance, ourAssetBalance, theirAssetBalance,
	totalCommitWeight, filteredHTLCView, nil
}
		...
		case localCommitmentView.incomingHTLCIndex[outputIndex] != nil:
			...
			sigHash = func() ([]byte, error) {
				...
				successTx, err := CreateHtlcSuccessTx(
					...
					outputAmt,htlc.AssetAmount,htlc.AssetID, uint32(localChanCfg.CsvDelay),
				...
				sigHash, err := txscript.CalcWitnessSigHash(
					...
					int64(htlc.BtcAmount.ToSatoshis()),
				)
				if err != nil {
					return nil, err
				}
		...
		case localCommitmentView.outgoingHTLCIndex[outputIndex] != nil:
			...
			sigHash = func() ([]byte, error) {
				...
				timeoutTx, err := CreateHtlcTimeoutTx(
					chanType, isLocalInitiator, op,
					outputAmt, htlc.AssetAmount, htlc.AssetID, htlc.Timeout,
...
func (lc *LightningChannel) ReceiveNewCommitment(commitSig lnwire.Sig,
	...
	lc.log.Tracef("local chain: our_btc_balance=%v, their_btc_balance=%v, our_asset_balance=%v, " +
		"their_asset_balance=%v, commit_tx: %v",
		localCommitmentView.ourBtcBalance, localCommitmentView.theirBtcBalance,
		localCommitmentView.ourAssetBalance, localCommitmentView.theirAssetBalance,
		newLogClosure(func() string {
	...
	sigHash, err := txscript.CalcWitnessSigHash(
		...
		localCommitTx, 0, int64(lc.channelState.BtcCapacity),
	)
	if err != nil {
...
func (lc *LightningChannel) RevokeCurrentCommitment() (*lnwire.RevokeAndAck, []channeldb.HTLC, error) {
	...
	lc.log.Tracef("state transition accepted: our_btc_balance=%v, their_btc_balance=%v," +
		"our_asset_balance=%v, their_asset_balance=%v, unsigned_acked_updates=%v",
		chainTail.ourBtcBalance, chainTail.theirBtcBalance,
		chainTail.ourAssetBalance,	chainTail.theirAssetBalance,
		len(unsignedAckedUpdates))
...
func (lc *LightningChannel) GetDustSum(remote bool) lnwire.MilliSatoshi {
	...
	for e := lc.localUpdateLog.Front(); e != nil; e = e.Next() {
		...
		amt := pd.BtcAmount.ToSatoshis()
		// If the satoshi amount is under the dust limit, add the msat
		// amount to the dust sum.
		if HtlcIsDust(
			chanType, false, !remote, feeRate, amt, dustLimit, pd.AssetID,
		) {
			dustSum += pd.BtcAmount
		}
	}
	...
	for e := lc.remoteUpdateLog.Front(); e != nil; e = e.Next() {
		...
		// amount to the dust sum.
		if HtlcIsDust(
			chanType, true, !remote, feeRate, amt, dustLimit, pd.AssetID,
		) {
			dustSum += pd.BtcAmount
		}
	}
...
func (lc *LightningChannel) MayAddOutgoingHtlc(amt lnwire.UnitPrec11) error {
	lc.Lock()
	defer lc.Unlock()

	var mockHtlcAmt lnwire.UnitPrec11
	switch {
...
func NewUnilateralCloseSummary(chanState *channeldb.OpenChannel, signer input.Signer,
	...
	htlcResolutions, err := extractHtlcResolutions(
		...
		chanState.ChanType,chanState.AssetID, isRemoteInitiator, leaseExpiry,
	)
	if err != nil {
...
func newOutgoingHtlcResolution(signer input.Signer,
	...
	chanType channeldb.ChannelType,assetId uint32 ) (*OutgoingHtlcResolution, error) {
	...
	htlcFee := HtlcTimeoutFee(chanType, feePerKw)
	secondLevelOutputAmt := htlc.BtcAmt.ToSatoshis() - htlcFee

	// With the fee calculated, re-construct the second level timeout
	// transaction.
	timeoutTx, err := CreateHtlcTimeoutTx(
		chanType, isCommitFromInitiator, op, secondLevelOutputAmt, htlc.AssetAmt,assetId,
...
func extractHtlcResolutions(feePerKw chainfee.SatPerKWeight, ourCommit bool,
	...
	commitTx *wire.MsgTx, chanType channeldb.ChannelType, assetId uint32,
	...
	for _, htlc := range htlcs {
		...
		if HtlcIsDust(
			...
			htlc.BtcAmt.ToSatoshis(),  dustLimit, assetId,
		) {
			continue
		}
		...
		if htlc.Incoming {
			...
			ihr, err := newIncomingHtlcResolution(
				...
				ourCommit, isCommitFromInitiator, chanType, assetId,
			)
			if err != nil {
				return nil, err
			}
		...
		ohr, err := newOutgoingHtlcResolution(
			...
			isCommitFromInitiator, chanType, assetId,
		)
		if err != nil {
			return nil, err
		}
...
func NewLocalForceCloseSummary(chanState *channeldb.OpenChannel,
	...
	*LocalForceCloseSummary, error) {
	...
	htlcResolutions, err := extractHtlcResolutions(
		...
		&chanState.RemoteChanCfg, commitTx, chanState.ChanType, chanState.AssetID,
...
func (lc *LightningChannel) CreateCloseProposal(proposedFee btcutil.Amount,
	...
	ourBalance, theirBalance,_,_, err := CoopCloseBalance(
		lc.channelState.ChanType, lc.channelState.IsInitiator,
		proposedFee, lc.channelState.LocalCommitment,
	)
	/*
	obd update  wxf
	todo add ourAssetBalance
	*/
	//ourBalance, theirBalance,ourAssetBalance, theirAssetBalance, err := CoopCloseBalance(
	//	lc.channelState.ChanType, lc.channelState.IsInitiator,
	//	proposedFee, lc.channelState.LocalCommitment,
	//)
	if err != nil {
...
func (lc *LightningChannel) CompleteCooperativeClose(
	...
	/*
	obd update wxf
	todo add ourAssetBalance, theirAssetBalance
	*/
	//ourBalance, theirBalance,ourAssetBalance, theirAssetBalance, err := CoopCloseBalance(
	//	lc.channelState.ChanType, lc.channelState.IsInitiator,
	//	proposedFee, lc.channelState.LocalCommitment,
...
func (lc *LightningChannel) AvailableBalance() lnwire.UnitPrec11  {
	lc.RLock()
	defer lc.RUnlock()
...
func (lc *LightningChannel) availableBalance() (lnwire.UnitPrec11, int64) {
...
func (lc *LightningChannel) availableCommitmentBalance(view *htlcView,
	remoteChain bool) (lnwire.UnitPrec11, int64) {
	...
	//)
	ourAmt:=lnwire.UnitPrec11(ourAssetBalance)
	theirAmt:=lnwire.UnitPrec11(theirAssetBalance)
	ourReserve:=lnwire.UnitPrec11(lc.channelState.LocalChanCfg.ChanReserve)
	if lc.channelState.AssetID==lnwire.BtcAssetId{
		ourReserve=lnwire.UnitPrec11(lc.channelState.LocalChanCfg.ChanReserve.ToMsat())
		ourAmt=lnwire.UnitPrec11(ourBalance)
		theirAmt=lnwire.UnitPrec11(theirBalance)
	}
	...
	if lc.channelState.IsInitiator {
		...
		// add HTLCs.
		if lc.channelState.AssetID==lnwire.BtcAssetId {
			if lnwire.MilliSatoshi(ourAmt) < htlcCommitFee {
				return 0, commitWeight
			}
			return lnwire.UnitPrec11(lnwire.MilliSatoshi(ourAmt) - htlcCommitFee), commitWeight
		}else {
	...
	if lc.channelState.AssetID==lnwire.BtcAssetId{
		...
		nonDustHtlcAmt :=dustlimitBtc + htlcFee
		...
		if  lnwire.MilliSatoshi(theirAmt) < htlcCommitFee &&  lnwire.MilliSatoshi(ourAmt) >=nonDustHtlcAmt {
			ourAmt =lnwire.UnitPrec11(nonDustHtlcAmt - 1)
		}
	}
...
func (lc *LightningChannel) validateFeeRate(feePerKw chainfee.SatPerKWeight) error {
	...
	availableBalanceBtc:=lnwire.MilliSatoshi(availableBalance)
	...
	baseBalance := availableBalanceBtc + oldFee
...
func (lc *LightningChannel) UpdateFee(feePerKw chainfee.SatPerKWeight) error {
	...
	pd := &PaymentDescriptor{
		LogIndex:  lc.localUpdateLog.logIndex,
		BtcAmount:    lnwire.NewMSatFromSatoshis(btcutil.Amount(feePerKw)),
		EntryType: FeeUpdate,
		AssetID: lc.channelState.AssetID,
	}
...
func (lc *LightningChannel) MaxFeeRate(maxAllocation float64) chainfee.SatPerKWeight {
	...
	baseBalance := lnwire.MilliSatoshi(availableBalance).ToSatoshis() + oldFee
...
func (lc *LightningChannel) LocalChanReserve() lnwire.UnitPrec8 {
...
func (lc *LightningChannel) FwdMinHtlc() lnwire.UnitPrec11 {
