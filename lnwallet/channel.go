...
func (lc *LightningChannel) diskCommitToMemCommit(isLocal bool,
	...
	if localCommitPoint != nil {
		localCommitKeys = lc.commitType.DeriveCommitmentKeys(
			localCommitPoint, true, lc.localChanCfg,
			lc.remoteChanCfg,
		)
	}
	if remoteCommitPoint != nil {
		remoteCommitKeys = lc.commitType.DeriveCommitmentKeys(
...
func (lc *LightningChannel) logUpdateToPayDesc(logUpdate *channeldb.LogUpdate,
	...
	feeRate chainfee.SatPerKWeight, remoteCommitKeys *commitmenttx.KeyRing,
...
func (lc *LightningChannel) restoreCommitState(
	...
	var (
		...
		pendingRemoteKeyChain   *commitmenttx.KeyRing
	)
	...
	if pendingRemoteCommitDiff != nil {
		...
		pendingRemoteKeyChain = lc.commitType.DeriveCommitmentKeys(
...
func (lc *LightningChannel) restoreStateLogs(
	...
	pendingRemoteCommitDiff *channeldb.CommitDiff,
	pendingRemoteKeys *commitmenttx.KeyRing) error {
...
func (lc *LightningChannel) fetchCommitmentView(remoteChain bool,
	...
	keyRing *commitmenttx.KeyRing) (*commitment, error) {
	...
	if err := lc.commitType.createCommitmentTx(c, filteredHTLCView, keyRing); err != nil {
		return nil, err
	}
...
func genRemoteHtlcSigJobs(keyRing *commitmenttx.KeyRing,
	...
	for _, htlc := range remoteCommitView.incomingHTLCs {
		...
		sigJob.Tx, err = createHtlcTimeoutTx(
			...
			keyRing.RevocationKey, keyRing.LocalKey,
		)
		if err != nil {
			return nil, nil, err
		}
...
func (lc *LightningChannel) SignNextCommitment() (lnwire.Sig, []lnwire.Sig, []channeldb.HTLC, error) {
	...
	keyRing := lc.commitType.DeriveCommitmentKeys(
...
func (lc *LightningChannel) computeView(view *htlcView, remoteChain bool,
	...
	*htlcView) {
	...
	totalCommitWeight := lc.commitType.CommitWeight() + totalHtlcWeight
...
func genHtlcSigValidationJobs(localCommitmentView *commitment,
	keyRing *commitmenttx.KeyRing, htlcSigs []lnwire.Sig,
...
func newOutgoingHtlcResolution(signer input.Signer,
	...
	htlc *channeldb.HTLC, keyRing *commitmenttx.KeyRing,
	...
	htlcSweepScript, err := input.SecondLevelHtlcScript(
		keyRing.RevocationKey, keyRing.LocalKey, csvDelay,
	)
	if err != nil {
		return nil, err
	}
...
func newIncomingHtlcResolution(signer input.Signer, localChanCfg *channeldb.ChannelConfig,
	commitHash chainhash.Hash, htlc *channeldb.HTLC, keyRing *commitmenttx.KeyRing,
	feePerKw chainfee.SatPerKWeight, csvDelay uint32,
...
func extractHtlcResolutions(feePerKw chainfee.SatPerKWeight, ourCommit bool,
	signer input.Signer, htlcs []channeldb.HTLC, keyRing *commitmenttx.KeyRing,
	localChanCfg, remoteChanCfg *channeldb.ChannelConfig,
...
func NewLocalForceCloseSummary(chanState *channeldb.OpenChannel, signer input.Signer,
	...
	*LocalForceCloseSummary, error) {
	...
	selfScript, err := input.CommitScriptToSelf(
		csvTimeout, keyRing.LocalKey, keyRing.RevocationKey,
	)
	if err != nil {
		return nil, err
	}
...
func (lc *LightningChannel) CalcFee(feeRate chainfee.SatPerKWeight) btcutil.Amount {
	return feeRate.FeeForWeight(lc.commitType.CommitWeight())
}
