...
func NewUnilateralCloseSummary(chanState *channeldb.OpenChannel, //nolint:funlen
	...
	htlcResolutions, err := extractHtlcResolutions(
		...
		commitTxHeight, remoteCommit.CommitHeight, chanState.ChanType,
	...
	if selfPoint != nil {
		...
		resolveReq := ResolutionReq{
			...
			KeyRing:             keyRing,
			CommitHeight:        fn.Some(remoteCommit.CommitHeight),
...
func newOutgoingHtlcResolution(signer input.Signer,
	...
	commitTxHeight uint32, stateNum uint64, htlc *channeldb.HTLC,
	keyRing *CommitmentKeyRing,
...
func extractHtlcResolutions(feePerKw chainfee.SatPerKWeight,
	...
	commitTx *wire.MsgTx, commitTxHeight uint32, stateNum uint64,
	...
	for _, htlc := range htlcs {
		...
		if htlc.Incoming {
			...
			ihr, err := newIncomingHtlcResolution(
				...
				stateNum, &htlc, keyRing, feePerKw,
				uint32(csvDelay), leaseExpiry, whoseCommit,
				isCommitFromInitiator, chanType, chanState,
		...
		ohr, err := newOutgoingHtlcResolution(
			signer, localChanCfg, commitTx, commitTxHeight,
			stateNum, &htlc, keyRing, feePerKw, uint32(csvDelay),
...
func NewLocalForceCloseSummary(chanState *channeldb.OpenChannel,
	...
	htlcResolutions, err := extractHtlcResolutions(
		...
		&chanState.RemoteChanCfg, commitTx, commitTxHeight, stateNum,
