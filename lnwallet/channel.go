...
func NewBreachRetribution(chanState *channeldb.OpenChannel, stateNum uint64,
	...
	if theirAmt >= int64(chanState.RemoteChanCfg.DustLimit) {
		...
		// can be septn using a single key spend, now that we know the
		// revocation key.
		if isTaproot {
...
func (lc *LightningChannel) ReceiveNewCommitment(commitSigs *CommitSigs) error {
	...
	if lc.channelState.ChanType.IsTaproot() {
		...
		localCtrNonce := WithLocalCounterNonce(
			nextHeight+1, lc.channelState.RevocationProducer,
		)
...
func (lc *LightningChannel) getSignedCommitTx() (*wire.MsgTx, error) {
	...
	case lc.channelState.ChanType.IsTaproot():
		...
		// the remote party to create this musig session. For the
		// target height we pass in 1 minus the current height, as
		// NewMusigVerificationNonce is used to create the nonce for
		// the _next_ height.
		//
		// TODO(roasbeef): make into func for unit tests
		localNonce, err := NewMusigVerificationNonce(
			ourKey.PubKey, lc.currentHeight,
			lc.channelState.RevocationProducer, true,
		)
		if err != nil {
			return nil, err
		}
	...
	if selfPoint != nil {
		...
		if chanState.ChanType.IsTaproot() {
			...
			ctrlBlock := input.MakeTaprootSuccessCtrlBlock(
				...
				keyRing.CombinedFundingKey, selfScript.ScriptTree,
			)
...
func NewAnchorResolution(chanState *channeldb.OpenChannel,
	...
	if chanState.ChanType.IsTaproot() {
		...
		// Finally, as this is a keyspned method, we'll need to also
