...
func TestHTLCSigNumber(t *testing.T) {
	...
	if len(aliceNewCommit.PendingHTLCs) != 1 {
		t.Fatalf("expected 1 htlc sig, instead got %v",
			len(aliceNewCommit.PendingHTLCs))
	}
...
func TestInvalidCommitSigError(t *testing.T) {
	...
	sigCopy := aliceNewCommit.CommitSig.Copy()
	copyBytes := sigCopy.RawBytes()
	copyBytes[0] ^= 80

	aliceNewCommit.CommitSig, err = lnwire.NewSigFromSchnorrRawSignature(
		copyBytes,
	)
	require.NoError(t, err)
