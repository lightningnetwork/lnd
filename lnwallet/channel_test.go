...
// TestCooperativeCloseOpReturnOmit tests that if either party's script is an
// OP_RETURN script, then their output will be omitted from the co-op close
// transaction.
func TestCooperativeCloseOpReturnOmit(t *testing.T) {
	t.Parallel()
	...
	// A Bob's output should be omitted from the close transaction, we
	// should only have a single output in the closeTx.
	require.Len(t, closeTxAlice.TxOut, 1)
