...
func TestProcessFeeUpdate(t *testing.T) {
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			h := update.addCommitHeights.GetParty(
				test.whoseCommitChain,
			)
			...
			if h == 0 {
				...
				if test.mutate {
					update.addCommitHeights.SetParty(
						test.whoseCommitChain,
						nextHeight,
					)

					update.removeCommitHeights.SetParty(
						test.whoseCommitChain,
						nextHeight,
					)
				}
...
func TestProcessAddRemoveEntry(t *testing.T) {
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if heightDual.GetParty(test.whoseCommitChain) == 0 {
				process(
					update, &ourBalance, &theirBalance,
					nextHeight, test.whoseCommitChain,
					test.isIncoming,
				)
