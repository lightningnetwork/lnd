...
func TestCreateBreachRetributionLegacy(t *testing.T) {
	...
	br, ourAmt, theirAmt, err := createBreachRetributionLegacy(
		...
		fn.None[AuxContractResolver](), fn.None[AuxSigner](),
	)
	require.NoError(t, err)
...
func TestHtlcFeesSigHashDefault(t *testing.T) {
	...
	// With sigHashDefault=true, taproot channels use baked-in fees
	// at sigHashDefaultFeeRate regardless of the passed feePerKw.
	timeoutFeeOn := HtlcTimeoutFee(taprootChanType, 0, true)
	successFeeOn := HtlcSuccessFee(taprootChanType, 0, true)
	...
	// The fee should be deterministic and match the expected weight
	// calculation at 1.1x the floor rate (covers aux-leaf weight not
	// counted by the LND HTLC weight constants and integer-rounding
	// margin; CPFP on the anchor handles any further fee bumping).
	// Weight includes the appended anchor output.
	anchorWeight := lntypes.WeightUnit(
		input.TaprootCommitmentAnchorOutput * 4,
	)
	feeRate := chainfee.FeePerKwFloor * 11 / 10
	expectedTimeout := feeRate.FeeForWeight(
		lntypes.WeightUnit(input.TaprootHtlcTimeoutWeight) +
			anchorWeight,
	)
	expectedSuccess := feeRate.FeeForWeight(
		lntypes.WeightUnit(input.TaprootHtlcSuccessWeight) +
			anchorWeight,
	)
	require.Equal(t, expectedTimeout, timeoutFeeOn)
	require.Equal(t, expectedSuccess, successFeeOn)

	// The fee should be independent of the passed feePerKw.
	...
	require.Equal(t, timeoutFeeOn, timeoutFeeHigh,
		"sigHashDefault fee should not depend on feePerKw")
