...
func genValidAmount(t *rapid.T) btcutil.Amount {
	return btcutil.Amount(
		rapid.Int64Range(0, 21_000_000*100_000_000).Draw(t, "amount"),
	)
}
...
func genCommitFee(t *rapid.T) btcutil.Amount {
	...
	return btcutil.Amount(rapid.Int64Range(100, 5_000).Draw(t, "commit_fee"))
}
...
func TestCoopCloseBalance(tt *testing.T) {
	...
	rapid.Check(tt, func(t *rapid.T) {
		...
		ourBalance := genValidAmount(t)
		theirBalance := genValidAmount(t)
		...
		if feePayer.IsSome() {
			...
			if payer == lntypes.Local {
				require.LessOrEqual(
					...
					btcutil.Amount(0),
					"local balance reduced by more than fee", //nolint:lll
				)
			} else {
				require.LessOrEqual(
					theirBalance-(theirFinal+coopCloseFee),
					btcutil.Amount(0),
					"remote balance reduced by more than fee", //nolint:lll
				)
			}
		}
