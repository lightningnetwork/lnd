...
func CommitScriptToSelf(chanType channeldb.ChannelType, initiator bool,
	...
	*ScriptInfo, error) {
	...
	// where the top level key is the revocation case, with our CSV timeout
...
func CommitScriptAnchors(chanType channeldb.ChannelType,
	...
	var (
		...
		keySelector  func(*channeldb.ChannelConfig, bool) *btcec.PublicKey
	)
	...
	//
	// TODO(roasbeef): need to re-examine the assumption of what's
	// revealed? otherwise then have two levels of tweaks...
	case chanType.IsTaproot():
		anchorScript = func(key *btcec.PublicKey) (*ScriptInfo, error) {
			...
			anchorPkScript, err := input.PayToTaprootScript(anchorKey)
			if err != nil {
				return nil, err
			}
	...
	default:
		...
		keySelector = func(cfg *channeldb.ChannelConfig, _ bool) *btcec.PublicKey {
...
// genTaprootHtlcScript....
func genTaprootHtlcScript(chanType channeldb.ChannelType, isIncoming,
	ourCommit bool, timeout uint32, rHash [32]byte,
	keyRing *CommitmentKeyRing) (*ScriptInfo, error) {
