...
type WalletController interface {
	...
	NewAddress(addrType AddressType, change bool,
		account string) (btcaddr.Address, error)
	...
	IsOurAddress(a btcaddr.Address) bool

	// AddressInfo returns the information about an address, if it's known
	// to this wallet.
	AddressInfo(a btcaddr.Address) (waddrmgr.ManagedAddress, error)
...
func InternalKeyForAddr(wallet WalletController, netParams *chaincfg.Params,
	...
	// internal key.
	_, isTaproot := addr.(*btcaddr.AddressTaproot)
	if !isTaproot {
		return none, nil
	}
