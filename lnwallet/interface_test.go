...
func runTest(t *testing.T, walletDriver *lnwallet.WalletDriver,
	...
	case "btcwallet":
		...
		var aliceTempSeed [32]byte
		var bobTempSeed [32]byte

		if _, err := rand.Read(aliceTempSeed[:]); err != nil {
			t.Fatalf("unable to create alice seed: %v", err)
		}

		if _, err := rand.Read(bobTempSeed[:]); err != nil {
			t.Fatalf("unable to create bob seed: %v", err)
		}

		aliceSeed := sha256.New()
		aliceSeed.Write([]byte(backEnd))
		if _, err := aliceSeed.Write(aliceTempSeed[:]); err != nil {
			t.Fatalf("unable to write alice seed: %v", err)
		}
		...
		if _, err := bobSeed.Write(bobTempSeed[:]); err != nil {
			t.Fatalf("unable to write bob seed: %v", err)
		}
