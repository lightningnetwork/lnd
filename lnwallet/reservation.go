...
func NewChannelReservation(localFundingAmt, remoteFundingAmt btcutil.Amount,
	...
	return &ChannelReservation{
		...
		partialState: &channeldb.OpenChannel{
			...
			LocalShutdownScript:  req.LocalShutdownScript,
...
func (r *ChannelReservation) ProcessChannelParams(p *ChannelParams) error {
	...
	// If we have a remote shutdown script, validate it.
	shutdown := p.RemoteShutdownScript
	if len(shutdown) > 0 {
		if !ValidateUpfrontShutdown(shutdown, &r.wallet.Cfg.NetParams) {
			return fmt.Errorf("invalid shutdown script")
		}
	}
