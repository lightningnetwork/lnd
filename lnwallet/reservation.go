...
func (r *ChannelReservation) CommitConstraints(
	...
	err := VerifyConstraints(
		bounds, rParams, maxLocalCSVDelay, r.partialState.Capacity,
	)
	if err != nil {
		return err
	}
	...
	r.ourContribution.CsvDelay = rParams.CsvDelay
...
func VerifyConstraints(bounds *channeldb.ChannelStateSpaceBounds,
	rParams *channeldb.CommitmentRenderingParams, maxLocalCSVDelay uint16,
	channelCapacity btcutil.Amount) error {

	// Fail if the csv delay for our funds exceeds our maximum.
	if rParams.CsvDelay > maxLocalCSVDelay {
		return ErrCsvDelayTooLarge(rParams.CsvDelay, maxLocalCSVDelay)
	}
	...
	if rParams.DustLimit > bounds.ChanReserve {
		return ErrChanReserveTooSmall(
			bounds.ChanReserve, rParams.DustLimit,
		)
	}
	...
	if rParams.DustLimit < maxWitnessLimit ||
		rParams.DustLimit > 3*maxWitnessLimit {

		return ErrInvalidDustLimit(rParams.DustLimit)
	}
