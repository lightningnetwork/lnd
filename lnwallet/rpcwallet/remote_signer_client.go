...
func (s *StreamFeeder) GetStream(streamCtx context.Context) (
	...
	// should not connect.
	if !s.shouldConnect {
		return nil, nil, errors.New("config not correctly set to be " +
...
// requests from the watch-only node, which are sent over a stream between the
// node and a watch-only node.
//
...
func NewRemoteSignerClient(subServers []lnrpc.SubServer,
	...
	if walletServer == nil {
		log.Debugf("Disabling the remote signer client as no " +
			"walletrpc.WalletKitServer exists")
	}

	if signerServer == nil {
		log.Debugf("Disabling the remote signer client as no " +
			"signrpc.SignerServer exists")
	}

	shouldRun := walletServer != nil && signerServer != nil && cfg != nil &&
		cfg.SignerType == lncfg.SignerClientType &&
...
func (r *RemoteSignerClient) processSignRequests(
	streamCtx context.Context) error {
