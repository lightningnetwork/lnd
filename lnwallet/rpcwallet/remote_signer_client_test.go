...
func (msf *mockStreamFeeder) GetStream(ctxc context.Context) (
	...
	stream, cancel := newMockStream(ctxc)
...
// stream.
func newMockStream(ctxc context.Context) (*mockStream, func()) {
	// Wrap the context in a cancel function, so that the stream will be
	...
	// If the wrapped context is cancelled, the remote signer client has
	// cancelled the stream.
	ctxc, cancel := context.WithCancel(ctxc)
...
func (ms *mockStream) handleHandshake(t *testing.T) error {
	...
	rType := &walletrpc.SignCoordinatorRequest_RegistrationComplete{
...
// newTestRemoteSignerClient creates a new remote signer client instance for
// testing purposes, and inserts the passed streamFeeder together with a mock
// sub-servers into the created instance.
func newTestRemoteSignerClient(t *testing.T,
	streamFeeder *mockStreamFeeder) *RemoteSignerClient {

	client := NewRemoteSignerClient(
...
func TestStreamCreationBackoff(t *testing.T) {
	...
	// creation. The backoff time is therefore expected to be the
