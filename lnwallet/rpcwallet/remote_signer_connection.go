...
// AddConnection feeds the inbound connection handler with the incoming stream set
// up by an outbound remote signer and then blocks until the stream is closed.
// Lnd can then send any requests to the remote signer through the stream.
//
