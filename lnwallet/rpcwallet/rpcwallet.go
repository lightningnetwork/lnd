...
func NewRPCKeyRing(watchOnlyKeyRing keychain.SecretKeyRing,
	...
	return &RPCKeyRing{
		...
		lnClient:         lnrpc.NewLightningClient(rpcConn),
...
func (r *RPCKeyRing) ImportAccount(name string, accountPubKey *hdkeychain.ExtendedKey,
...
// error will be returned. The actual digest signed is the double SHA-256 of the
// passed message.
//
...
func (r *RPCKeyRing) SignMessageCompact(keyLoc keychain.KeyLocator,
	...
	resp, err := r.lnClient.SignMessage(ctxt, &lnrpc.SignMessageRequest{
		Msg: msg,
	})
	if err != nil {
		return nil, err
	...
	return zbase32.DecodeString(resp.Signature)
}
