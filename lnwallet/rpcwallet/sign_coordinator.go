	...
	for {
		...
		if ok {
			...
			case <-time.After(s.requestTimeout):
				// If we don't pick up the response within the
				// request timeout, we just ignore the response
				// as the caller has most likely timed out.
			}
...
func (s *SignCoordinator) createResponseChannel(requestID uint64) func() {
	...
	respChan := make(chan *walletrpc.SignCoordinatorResponse)
