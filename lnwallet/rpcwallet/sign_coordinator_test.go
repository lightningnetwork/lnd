...
func TestPingRequests(t *testing.T) {
	...
	go func() {
		...
		// response.
		success, err := coordinator.Ping(ctx, 2*time.Second)
...
func TestPingTimeout(t *testing.T) {
	...
	_, err := coordinator.Ping(ctx, 1*time.Second)
...
func TestConcurrentPingTimeout(t *testing.T) {
	...
	go func() {
		...
		success, err := coordinator.Ping(ctx, 1*time.Second)
		require.Equal(t, context.DeadlineExceeded, err)
...
func TestIncorrectResponseRequestId(t *testing.T) {
	...
	go func() {
		...
		success, err := coordinator.Ping(ctx, pingTimeout)
