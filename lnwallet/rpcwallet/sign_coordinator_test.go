...
func setupSignCoordinator(t *testing.T) (*SignCoordinator, *mockSCStream,
	chan error, func() map[uint64]struct{}) {
	...
	// A helper function to get the keys of the coordinators responses map
	// in a thread-safe way.
	getResponsesKeys := func() map[uint64]struct{} {
		coordinator.mu.Lock()
		defer coordinator.mu.Unlock()

		responsesKeys := make(map[uint64]struct{})
		for key := range coordinator.responses {
			responsesKeys[key] = struct{}{}
		}

		return responsesKeys
	}

	return coordinator, stream, errChan, getResponsesKeys
}
...
func TestPingRequests(t *testing.T) {
	...
	coordinator, stream, _, getResponses := setupSignCoordinator(t)
...
func TestIncorrectResponseRequestId(t *testing.T) {
	...
	// Send a Ping request that times out in 2 seconds.
	go func() {
		defer wg.Done()
...
func TestStopCoordinator(t *testing.T) {
	...
	coordinator, stream, runErrChan, getResponses := setupSignCoordinator(t)
