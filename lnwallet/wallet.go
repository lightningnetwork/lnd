...
type InitFundingReserveMsg struct {
	...
	// FilterUtxo enables the channel funding workflow to filter for
	// explicit utxos when selecting the inputs for the channel
	...
	// inputs to make sure stable non-replaceable inputs to the transaction
	// are used.
	FilterUtxo FilterUtxoFn
...
func (l *LightningWallet) handleFundingReserveRequest(req *InitFundingReserveMsg) {
	...
	if req.ChanFunder == nil {
		...
		cfg := chanfunding.WalletConfig{
			CoinSource:       &CoinSource{l, req.FilterUtxo},
			CoinSelectLocker: l,
			CoinLeaser:       l,
			Signer:           l.Cfg.Signer,
...
// FilterUtxoFn is a function type which enables the filtering of utxos.
type FilterUtxoFn func(Utxo) bool

// NewCoinSource creates a new instance of the CoinSource wrapper struct.
func NewCoinSource(w *LightningWallet, filter FilterUtxoFn) *CoinSource {
	return &CoinSource{
		wallet:     w,
		filterUtxo: filter,
	}
...
//
// NOTE: Unconfirmed Utxos which are marked by the sweeper subsystem are
// excluded from the coin selection because they are not final and can be RBFed
// by the sweeper subsystem.
func (c *CoinSource) ListCoins(minConfs int32,
	...
	for _, utxo := range utxos {
		...
		if c.filterUtxo != nil && !c.filterUtxo(*utxo) {
