...
func (l *LightningWallet) handleContributionMsg(req *addContributionMsg) {
	...
	var stateObfuscator [input.StateHintSize]byte
...
func DeriveStateHintObfuscator(key1, key2 *btcec.PublicKey) [input.StateHintSize]byte {
	h := sha256.New()
...
func initStateHints(commit1, commit2 *wire.MsgTx,
	obfuscator [input.StateHintSize]byte) error {

	if err := input.SetStateNumHint(commit1, 0, obfuscator); err != nil {
		return err
	}
	if err := input.SetStateNumHint(commit2, 0, obfuscator); err != nil {
		return err
	}
