...
// TestClosingCompleteEncodeDecode tests that a raw byte stream can be
// decoded, then re-encoded to the same exact byte stream.
func TestClosingCompleteEncodeDecode(t *testing.T) {
	t.Parallel()

	// Create a sample ClosingComplete message.
	var rawBytes []byte
