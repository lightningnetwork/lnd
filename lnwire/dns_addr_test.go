...
func TestDNSHostnameAddressValidate(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "first label ends with hyphen",
			hostname:  "example-.com",
			port:      80,
			expectErr: true,
			errMsg:    "starts or ends with a hyphen",
		...
		{
			name:      "middle label ends with hyphen",
		...
		{
			name:      "last label ends with hyphen",
			hostname:  "example.com-",
			port:      80,
			expectErr: true,
			errMsg:    "starts or ends with a hyphen",
		...
		{
			name:      "IP-like hostname",
			hostname:  "127.0.0.1",
			port:      9735,
			expectErr: true,
			errMsg: "resembles an IPv4 address and cannot be " +
				"used as a valid DNS hostname",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := NewDNSAddr(tt.hostname, tt.port)
			if tt.expectErr {
