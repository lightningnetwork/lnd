...
func TestNodeAnn2AddrDecodeTruncated(t *testing.T) {
	...
	for _, family := range families {
		...
		// record is needed to reach the address read as well. A record
		// that stops between two addresses makes the next read return
		// no bytes at all.
		cases := []struct {
			name     string
			length   uint64
			...
			{
				name:     "address boundary",
				length:   2 * family.size,
