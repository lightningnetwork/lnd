...
In addition to those three, five sub servers bake a macaroon of their own on
first startup when they are compiled in, scoped to the permissions that sub
server needs. The other sub servers write no file of their own:
...
creates all of these files. Four of them can be pointed elsewhere with a
command line option (`--routermacaroonpath`, `--signermacaroonpath`,
`--walletkitmacaroonpath` and `--notifiermacaroonpath`). `invoices.macaroon`
has no such option and is always read from the network directory.
