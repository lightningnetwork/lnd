...
// TestMultiMutexConcurrency ensures proper handling of concurrent access.
func TestMultiMutexConcurrency(t *testing.T) {
	...
	for i := 0; i < numOps; i++ {
		...
		go func() {
			...
			mtx.Lock("shared")
			current := counter
			// Simulate some work and ensure atomicity.
...
// TestMultiMutexMultipleIDs ensures independent locking per ID.
...
// TestMultiMutexReuse ensures mutex reuse after unlocking.
...
// TestMultiMutexPanic ensures proper panic behavior on invalid unlocks.
...
// TestMultiMutexCustomType ensures compatibility with custom types.
