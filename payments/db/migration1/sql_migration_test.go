	...
	err := kvdb.Update(kvDB, func(tx kvdb.RwTx) error {
		...
		attemptInfo := &HTLCAttemptInfo{
			...
			Route: Route{
				...
				Hops: []*Hop{
					{
						PubKeyBytes:      NewVertex(hopKey.PubKey()),
						ChannelID:        12345,
...
// runPaymentsMigration executes the payment migration from KV to SQL within
// a SQL transaction.
func runPaymentsMigration(ctx context.Context, kvDB kvdb.Backend,
...
// paymentTestConfig holds configuration for creating test payments with various
// features.
...
// writes it to the bucket.
func serializeAndPut(bucket kvdb.RwBucket, key []byte,
	serializer func(io.Writer) error) error {

	var b bytes.Buffer
	if err := serializer(&b); err != nil {
		return err
	}
...
func generateSessionKey(t *testing.T) (*btcec.PrivateKey, Vertex,
	[32]byte, error) {
	...
	return sessionKey, sourcePubKey, sessionKeyBytes, nil
}

// createTestHops creates the specified number of test hops with the given
// parameters. The configurator function is called for each hop to allow
...
func createTestHops(t *testing.T, numHops int, baseAmount lnwire.MilliSatoshi,
	...
	for i := 0; i < numHops; i++ {
		...
		if configurator != nil {
			configurator(hop, i, i == numHops-1)
		}
	...
	return hops, currentAmt, nil
}

// writeHTLCAttempt writes the HTLC attempt info to the bucket.
func writeHTLCAttempt(bucket kvdb.RwBucket, attemptID uint64,
	info *HTLCAttemptInfo) error {

	attemptKey := make([]byte, len(htlcAttemptInfoKey)+8)
	copy(attemptKey, htlcAttemptInfoKey)
	byteOrder.PutUint64(attemptKey[len(htlcAttemptInfoKey):], attemptID)

	return serializeAndPut(bucket, attemptKey, func(w io.Writer) error {
		return serializeHTLCAttemptInfo(w, info)
	})
}

// writeHTLCSettle writes the HTLC settle info to the bucket.
func writeHTLCSettle(bucket kvdb.RwBucket, attemptID uint64,
	info *HTLCSettleInfo) error {

	settleKey := make([]byte, len(htlcSettleInfoKey)+8)
	copy(settleKey, htlcSettleInfoKey)
	byteOrder.PutUint64(settleKey[len(htlcSettleInfoKey):], attemptID)

	return serializeAndPut(bucket, settleKey, func(w io.Writer) error {
		return serializeHTLCSettleInfo(w, info)
	})
}

// createIndexEntry creates a payment index entry in the index bucket.
func createIndexEntry(indexBucket kvdb.RwBucket, seqBytes []byte,
	hash [32]byte) error {

	var idx bytes.Buffer
	if err := WriteElements(
		&idx, paymentIndexTypeHash, hash[:],
	); err != nil {
		return err
	}
...
// createTestPayment creates a test payment with the specified configuration,
// handling all common boilerplate code.
func createTestPayment(t *testing.T, paymentsBucket, indexBucket kvdb.RwBucket,
	cfg paymentTestConfig) error {
	...
	// Create payment ID.
	var paymentID lntypes.Hash
	copy(paymentID[:], cfg.hash[:])

	// Create and serialize payment creation info.
	creationInfo := &PaymentCreationInfo{
		PaymentIdentifier:     paymentID,
		Value:                 cfg.value,
		CreationTime:          cfg.creationTime,
		PaymentRequest:        []byte(cfg.paymentRequest),
		FirstHopCustomRecords: cfg.paymentCustomRecs,
	}
	...
	// Store sequence number.
	seqBytes := make([]byte, 8)
	byteOrder.PutUint64(seqBytes, cfg.seqNum)
	...
	// Create route with hops.
	hops, totalAmount, err := createTestHops(
		t, cfg.numHops, cfg.value, cfg.baseChannelID,
		cfg.baseTimeLock, cfg.hopConfigurator,
	)
	if err != nil {
		return err
	}
	...
	if err = writeHTLCAttempt(
		htlcBucket, cfg.attemptID, attemptInfo,
	); err != nil {
		return err
	}
	...
	// Add settlement.
	settleInfo := &HTLCSettleInfo{
		Preimage:   lntypes.Preimage(cfg.hash),
		SettleTime: cfg.creationTime.Add(2 * time.Minute),
	}

	if err := writeHTLCSettle(
		htlcBucket, cfg.attemptID, settleInfo,
	); err != nil {
		return err
	}

	// Create index entry.
	return createIndexEntry(indexBucket, seqBytes, cfg.hash)
}
	...
	return createTestPayment(
		...
		paymentTestConfig{
			...
			creationTime: time.Date(
				2024, 1, 1, 12, 0, 0, 0, time.UTC,
			),
			paymentRequest: "lnbc500n1test_mpp",
			attemptID:      1,
			numHops:        3,
...
func createPaymentWithAMPChildIndex(t *testing.T, paymentsBucket,
	...
	return createTestPayment(
		...
		paymentTestConfig{
			...
			value:  75000,
			creationTime: time.Date(
				2024, 2, 1, 10, 0, 0, 0, time.UTC,
			),
			paymentRequest: "lnbc750n1test_amp",
			attemptID:      1,
			numHops:        2,
...
func createPaymentWithCustomRecords(t *testing.T, paymentsBucket,
	...
	return createTestPayment(
		...
		paymentTestConfig{
			...
			value:  100000,
			creationTime: time.Date(
				2024, 3, 1, 14, 0, 0, 0, time.UTC,
			),
			paymentRequest: "lnbc1m1test_custom",
			attemptID:      1,
			numHops:        3,
			baseChannelID:  300000,
			baseTimeLock:   700000,
			paymentCustomRecs: lnwire.CustomRecords{
...
func createPaymentWithBlindedRoute(t *testing.T, paymentsBucket,
	...
	return createTestPayment(
		...
		paymentTestConfig{
			...
			value:  120000,
			creationTime: time.Date(
				2024, 4, 1, 16, 0, 0, 0, time.UTC,
			),
			paymentRequest: "lnbc1200n1test_blinded",
			attemptID:      1,
			numHops:        4,
			...
			hopConfigurator: func(hop *Hop, index int,
				...
				if isFinal {
					...
					hop.EncryptedData = []byte(
						"encrypted_blinded_route_" +
							"data_test_value_12345",
					)
...
func createPaymentWithMetadata(t *testing.T, paymentsBucket,
	...
	return createTestPayment(
		...
		paymentTestConfig{
			...
			value:  80000,
			creationTime: time.Date(
				2024, 5, 1, 18, 0, 0, 0, time.UTC,
			),
			paymentRequest: "lnbc800n1test_metadata",
			attemptID:      1,
			numHops:        3,
...
func createPaymentWithAllFeatures(t *testing.T, paymentsBucket,
	...
	return createTestPayment(
		...
		paymentTestConfig{
			...
			value:  150000,
			creationTime: time.Date(
				2024, 6, 1, 20, 0, 0, 0, time.UTC,
			),
			paymentRequest: "lnbc1500n1test_all_features",
			attemptID:      1,
			numHops:        4,
			baseChannelID:  600000,
			baseTimeLock:   1000000,
			paymentCustomRecs: lnwire.CustomRecords{
...
// paymentFeatureSet defines a combination of optional payment features for
