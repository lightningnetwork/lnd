...
func assertRouteEqual(a, b *route.Route) error {
	if !reflect.DeepEqual(a, b) {
		return fmt.Errorf("attempt routes don't match: %v vs %v",
...
// generateNewSessionKey generates a new session key for a given
// HTLCAttemptInfo because we have a unique constraint on the session key
// in the sql database.
func generateNewSessionKey(t *testing.T,
	htlc *HTLCAttemptInfo) error {

	priv, err := btcec.NewPrivateKey()
	require.NoError(t, err)
	var cachedSessionKey [btcec.PrivKeyBytesLen]byte
	copy(cachedSessionKey[:], priv.Serialize())

	htlc.SetSessionKey(cachedSessionKey)
	htlc.cachedSessionKey = priv

	return nil
}

func genInfo(t *testing.T) (*PaymentCreationInfo,
	*HTLCAttemptInfo, lntypes.Preimage, error) {
...
// TestSwitchFail checks that payment status returns to Failed
...
func TestSwitchFail(t *testing.T) {
	...
	// Add a specific test for the kv-store.
	...
	// Always generate a new session key for the attempt because every
	// attempt has a unique session key constraint in the sql database.
	err = generateNewSessionKey(t, attempt)
	require.NoError(t, err)
...
// TestSuccessesWithoutInFlight checks that the payment
...
// TestFailsWithoutInFlight checks that a strict payment
...
// TestDeletePayments tests that DeletePayments correctly deletes
...
// TestMultiShard checks the ability of payment control to
...
func TestMPPRecordValidation(t *testing.T) {
	...
	// Increment the attempt ID because the attempt ID needs to be unique
...
func assertPaymentStatus(t *testing.T, p PaymentDB,
...
// expected values.
func assertPaymentInfo(t *testing.T, p PaymentDB, hash lntypes.Hash,
	...
	if err := assertRouteEqual(&htlc.Route, &a.Route); err != nil {
		t.Fatal("routes do not match with: ", err)
	}
...
func createTestPayments(t *testing.T, p PaymentDB, payments []*payment) {
	attemptID := uint64(0)
	...
	for i := 0; i < len(payments); i++ {
		...
		attemptID++
		// Always generate a new session key for the attempt because
		// every attempt has a unique session key constraint in the sql
		// database.
		err = generateNewSessionKey(t, attempt)
		require.NoError(t, err)
...
// database.
func assertPayments(t *testing.T, paymentDB PaymentDB,
	payments []*payment) {
