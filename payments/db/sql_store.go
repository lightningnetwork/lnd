...
func fetchPaymentWithCompleteData(ctx context.Context,
	...
	batchData, err := batchLoadPaymentsRelatedData(
...
func batchLoadPayments(ctx context.Context, cfg *sqldb.QueryConfig,
	db SQLQueries, paymentIDs []int64) (*paymentsAggregate, error) {

	coreData, err := batchLoadPaymentsCoreData(ctx, cfg, db, paymentIDs)
	if err != nil {
		return nil, fmt.Errorf("failed to load payment core data: %w",
			err)
	}

	batchData, err := batchLoadPaymentsRelatedData(ctx, cfg, db, paymentIDs)
	if err != nil {
	...
	return &paymentsAggregate{
		paymentsCoreData:    coreData,
		paymentsRelatedData: batchData,
	}, nil
}
...
// batchLoadPaymentsCoreData loads the core payment and payment intent data for
...
func batchLoadPaymentsCoreData(ctx context.Context,
	cfg *sqldb.QueryConfig, db SQLQueries,
	paymentIDs []int64) (*paymentsCoreData, error) {

	coreData := &paymentsCoreData{
	...
	err := sqldb.ExecuteBatchQuery(
		...
		func(ctx context.Context,
			...
			coreData.paymentsAndIntents[payment.ID] = payment

			return nil
		},
	)
	if err != nil {
		return nil, fmt.Errorf("failed to fetch payment core "+
			"data: %w", err)
	}
...
func batchLoadPaymentCustomRecords(ctx context.Context,
	...
	batchData *paymentsRelatedData) error {
...
func batchLoadHtlcAttempts(ctx context.Context, cfg *sqldb.QueryConfig,
	...
	batchData *paymentsRelatedData) ([]int64, error) {
...
func batchLoadHopCustomRecords(ctx context.Context, cfg *sqldb.QueryConfig,
	db SQLQueries, hopIDs []int64, batchData *paymentsRelatedData) error {
...
// batchLoadPaymentsRelatedData loads all related data for multiple payments in
// batch. It uses a batch queries to fetch all data for the given payment IDs.
func batchLoadPaymentsRelatedData(ctx context.Context, cfg *sqldb.QueryConfig,
	db SQLQueries, paymentIDs []int64) (*paymentsRelatedData, error) {

	batchData := &paymentsRelatedData{
...
func buildPaymentFromBatchData(dbPayment sqlc.PaymentAndIntent,
	batchData *paymentsRelatedData) (*MPPayment, error) {
...
func (s *SQLStore) QueryPayments(ctx context.Context, query Query) (Response,
	...
	err := s.db.ExecTx(ctx, sqldb.ReadTxOpt(), func(db SQLQueries) error {
		...
		batchDataFunc := func(ctx context.Context, paymentIDs []int64) (
			*paymentsRelatedData, error) {

			return batchLoadPaymentsRelatedData(
...
func (s *SQLStore) FetchInFlightPayments(ctx context.Context) ([]*MPPayment,
	...
	err := s.db.ExecTx(ctx, sqldb.ReadTxOpt(), func(db SQLQueries) error {
		...
		batchDataFunc := func(ctx context.Context,
			paymentIDs []int64) (*paymentsAggregate, error) {
		...
		processAttempt := func(ctx context.Context,
			...
			batchData *paymentsAggregate) error {
			...
			mpPayment, err := buildPaymentFromBatchData(
				dbPayment, batchData.paymentsRelatedData,
			)
			if err != nil {
