...
// paymentStatusBatchData holds lightweight resolution data for computing
// payment status efficiently during deletion operations.
type paymentStatusBatchData struct {
...
func (s *SQLStore) loadPaymentResolutionsBatchData(ctx context.Context,
	db SQLQueries, paymentIDs []int64) (*paymentStatusBatchData, error) {

	batchData := &paymentStatusBatchData{
...
func (s *SQLStore) loadPaymentResolutions(ctx context.Context, db SQLQueries,
	...
	batchData, err := s.loadPaymentResolutionsBatchData(
...
// computePaymentStatus determines the payment status from resolution types
// and failure reason without building the complete MPPayment structure.
// This is a lightweight version that builds minimal HTLCAttempt structures
// and delegates to decidePaymentStatus for consistency.
func computePaymentStatus(resolutionTypes []sql.NullInt32,
...
func (s *SQLStore) computePaymentStatusFromDB(ctx context.Context,
	...
	resolutionTypes, err := s.loadPaymentResolutions(ctx, db, payment.ID)
	if err != nil {
	...
	status, err := computePaymentStatus(resolutionTypes, payment.FailReason)
	if err != nil {
...
func (s *SQLStore) RegisterAttempt(ctx context.Context,
	...
	err := s.db.ExecTx(ctx, sqldb.WriteTxOpt(), func(db SQLQueries) error {
		...
		_, err = db.InsertHtlcAttempt(ctx, sqlc.InsertHtlcAttemptParams{
			PaymentID:    dbPayment.GetPayment().ID,
...
func (s *SQLStore) DeletePayments(ctx context.Context, failedOnly,
	...
	err := s.db.ExecTx(ctx, sqldb.WriteTxOpt(), func(db SQLQueries) error {
		...
		batchDataFunc := func(ctx context.Context, paymentIDs []int64) (
			*paymentStatusBatchData, error) {

			return s.loadPaymentResolutionsBatchData(
				ctx, db, paymentIDs,
			)
		}
		...
		processPayment := func(ctx context.Context,
			dbPayment sqlc.FilterPaymentsRow,
			batchData *paymentStatusBatchData) error {
			...
			status, err := computePaymentStatus(
