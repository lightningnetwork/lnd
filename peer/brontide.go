...
func newDiscMsgStream(p *Brontide) *msgStream {
	apply := func(msg lnwire.Message) {
		...
		p.log.Debugf("Processing remote msg %T from peer=%x", msg,
			p.IdentityKey())
		...
		go func() {
			...
			case err := <-errChan:
				if err != nil {
					p.log.Errorf("Error processing remote "+
						"msg %T from peer=%x: %v", msg,
						p.IdentityKey(), err)
				}
			}

			p.log.Debugf("Processed remote msg %T from peer=%x",
				msg, p.IdentityKey())
		}()
	}
