...
// TestPeerPingFloodDisconnects verifies one-token Ping accounting precedes a
// generic router that would otherwise consume the message.
func TestPeerPingFloodDisconnects(t *testing.T) {
	...
	// Arrange: Encode one valid Pong request, then register the read loop
	// with the control group for joinable teardown.
	var b bytes.Buffer
	...
	// Act: Send two valid Pings. The first spends the sole token and
...
func TestPeerSendMessageQueueBounds(t *testing.T) {
	...
	// limits.
	// A 64 KiB charge lets 256 payloads fill the 16 MiB byte budget
	// without approaching the message-count cap.
	limits := defaultQueueLimits()
	failureReasonSize := (1 << 16) - limits.msgOverhead
	messageCost := limits.msgOverhead + failureReasonSize
	require.Zero(t, limits.maxBytes%messageCost)
	tests := []struct {
		name        string
		...
		{
			name:        "message bytes",
			msgType:     lnwire.MsgUpdateFailHTLC,
			payloadSize: failureReasonSize,
			numAtLimit:  limits.maxBytes / messageCost,
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			// fresh peer-controlled payload for every message.
			params := createTestPeer(t)
			peer := params.peer
			...
			// declarative, including one distinct payload per send.
			newMsg := func() lnwire.Message {
				...
				case lnwire.MsgUpdateFailHTLC:
					reason := make([]byte, test.payloadSize)
