...
type MsgEndpoint interface {
	...
	// CanRouteTo returns true if the target message can be routed to this
	// endpoint.
	CanRouteTo(msg lnwire.Message) bool
...
type MsgRouter interface {
	...
	// endpoint. If ANY endpoint could handle the message, then true is
	// returned.
...
// EndpointsList is a slice of all registered endpoints.
type EndpointsList []MsgEndpoint
...
type PeerMsgRouter struct {
	...
	// will be sent to.
	endpointQueries chan queryMsg[MsgEndpoint, EndpointsList]

	wg   sync.WaitGroup
	quit chan struct{}
}

// NewPeerMsgRouter creates a new instance of a peer message router.
func NewPeerMsgRouter() *PeerMsgRouter {
	return &PeerMsgRouter{
		...
		endpointQueries: make(chan queryMsg[MsgEndpoint, EndpointsList]),
		quit:            make(chan struct{}),
	}
}

// Start starts the peer message router.
func (p *PeerMsgRouter) Start() {
	p.startOnce.Do(func() {
		p.wg.Add(1)
		go p.msgRouter()
	})
...
func (p *PeerMsgRouter) RegisterEndpoint(endpoint MsgEndpoint) error {
	return sendQueryErr(p.registerChan, endpoint, p.quit)
}

// UnregisterEndpoint unregisters the target endpoint from the router.
func (p *PeerMsgRouter) UnregisterEndpoint(name EndPointName) error {
...
func (p *PeerMsgRouter) msgRouter() {
	defer p.wg.Done()
	...
	for {
		...
		case msgQuery := <-p.msgChan:
			...
			for _, endpoint := range endpoints {
				if endpoint.CanRouteTo(msg) {
					couldSend = endpoint.SendMessage(msg)
				}
			}
		...
		case endpointQuery := <-p.endpointQueries:
			endpointsCopy := make(EndpointsList, len(endpoints))
			copy(endpointsCopy, maps.Values(endpoints))

			endpointQuery.respChan <- fn.NewLeft[EndpointsList, error](
				endpointsCopy,
			)
...
// A compile time check to ensure PeerMsgRouter implements the MsgRouter
// interface.
var _ MsgRouter = (*PeerMsgRouter)(nil)
