...
// EmitedEvent is a special type that can be emitted by a state transition.
...
func (s *StateMachine[Event, Env]) CurrentState() (State[Event, Env], error) {
	...
	// TODO(roasbeef): channel sent on for each state transition?
	// subscribers?
...
func (s *StateMachine[Event, Env]) applyEvents(newEvent Event) (State[Event, Env], error) {
	...
	for nextEvent := eventQueue.Dequeue(); nextEvent.IsSome(); nextEvent = eventQueue.Dequeue() {
		err := fn.MapOptionZero(nextEvent, func(event Event) error {
			...
			err = fn.MapOptionZero(newEvents, func(events EmittedEvent[Event]) error {
