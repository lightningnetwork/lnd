...
func TestStateMachineTerminateCleanup(t *testing.T) {
	...
	//
	// As a result of this call, we expects the clean up method on the env
	// to be called.
...
func TestStateMachineMsgMapper(t *testing.T) {
	...
	// First, we'll verify that the CanRouteTo method works as expected.
	require.True(t, stateMachine.CanRouteTo(wireError))
	require.False(t, stateMachine.CanRouteTo(&lnwire.Init{}))
