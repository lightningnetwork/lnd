...
func (m *queueMachine[T]) Enqueue(t *rapid.T) {
	...
	if errors.Is(err, ErrQueueFullAndDropped) {
		...
		// ErrQueueFullAndDropped, it's unexpected.
...
func NewIntqueueMachine(rt *rapid.T) *intQueueMachine {
	...
	predicate := AsDropPredicate[int](RandomEarlyDrop(
...
func TestBackpressureQueueDequeueCancellation(t *testing.T) {
	rapid.Check(t, func(rt *rapid.T) {
		...
		q := NewBackpressureQueue[int](
			capacity, AsDropPredicate[int](
				RandomEarlyDrop(0, capacity),
			),
		)
		...
		require.True(
			rt, result.IsErr(), "dequeue should have "+
				"returned an error for cancelled context",
		)
		require.ErrorIs(
			rt, result.Err(),
			context.Canceled, "error should be context.Canceled",
		)
	})
}
...
func TestBackpressureQueueTryEnqueue(t *testing.T) {
	...
	q := NewBackpressureQueue[int](capacity, alwaysDropAboveThreshold)
	...
	q2 := NewBackpressureQueue[int](capacity,
		func(_ int, _ int) bool { return false },
	)
	for i := 0; i < capacity; i++ {
		ok := q2.TryEnqueue(i)
...
func TestBackpressureQueueLenAndReceiveChan(t *testing.T) {
	...
	q := NewBackpressureQueue[int](10, neverDrop)
