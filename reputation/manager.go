...
// fee the node advertised on that link for this forward, height is the current
// best block height, and accountable is the outgoing accountable signal.
func (m *Manager) OnForward(incoming models.CircuitKey,
	...
	d, err := m.addHTLC(
		...
		incomingCltv, height, accountable, at,
	)
	if err != nil {
...
func (m *Manager) OnSettle(incoming models.CircuitKey,
	...
	m.resolve(incoming, outgoing, true)
}
...
func (m *Manager) OnFail(incoming models.CircuitKey,
	...
	m.resolve(incoming, outgoing, false)
}

// resolve applies an HTLC resolution under the lock.
func (m *Manager) resolve(incoming models.CircuitKey,
	outgoing lnwire.ShortChannelID, settled bool) {
	...
	if err := m.resolveHTLC(incoming, outgoing, settled, at); err != nil {
...
func (m *Manager) addHTLC(incoming models.CircuitKey,
	outgoing lnwire.ShortChannelID, incomingAmt, outgoingAmt,
	advertisedFee lnwire.MilliSatoshi, incomingCltv, height uint32,
	accountable bool, at time.Time) (decision, error) {

	if outgoingAmt > incomingAmt {
		return decision{}, fmt.Errorf("outgoing amount %v exceeds "+
	...
	if _, ok := outChan.pendingHTLCs[ref]; ok {
		return decision{}, fmt.Errorf("duplicate htlc %v", ref)
	}
	...
	htlcRisk := satFromUint(m.cfg.inFlightRisk(fee, incomingCltv, height))
	...
	totalRisk := htlcRisk.Add(outChan.inFlightRisk(m.cfg))
...
// resolveHTLC applies an HTLC resolution to reputation and revenue. Caller must
// hold mu.
func (m *Manager) resolveHTLC(incoming models.CircuitKey,
	outgoing lnwire.ShortChannelID, settled bool, at time.Time) error {
