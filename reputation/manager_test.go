...
// TestInFlightRiskDecision checks that withInFlight, the verdict BOLT #1280
// defines, also charges the risk of the accountable HTLCs already pending on
// the outgoing channel, while unaccountable ones add none.
func TestInFlightRiskDecision(t *testing.T) {
	t.Parallel()

	// A cltv delta of 1 block gives a 600s hold, so each HTLC carries a
	// risk of round((600-90)/90 * 1000) = 5667. The outgoing reputation of
	// 12000 covers one HTLC's risk, and two of them, but not three.
	const (
		fee        = 1000
		cltv       = testHeight + 1
		risk       = 5667
	...
	tests := []struct {
		...
		// prior holds the accountable bit of each HTLC already in
		// flight on the outgoing channel.
		prior []bool

		wantTotalRisk    int64
		wantWithInFlight bool
	}{{
		name:             "nothing in flight",
		wantTotalRisk:    risk,
		wantWithInFlight: true,
	}, {
		name:             "accountable in flight",
		prior:            []bool{true, true},
		wantTotalRisk:    3 * risk,
		wantWithInFlight: false,
	}, {
		// Counting the unaccountable HTLC would make it 3 * risk and
		// flip the verdict.
		name:             "unaccountable in flight adds no risk",
		prior:            []bool{false, true},
		wantTotalRisk:    2 * risk,
		wantWithInFlight: true,
	}}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			outChan := m.getOrCreateChannel(2, at)
			_, err := outChan.outgoingReputation.add(reputation, at)
			require.NoError(t, err, "seed reputation")

			for i, accountable := range test.prior {
				_, err := m.addHTLC(
					circuit(1, uint64(i)), scid(2), fee,
					cltv, testHeight, accountable, at,
				)
				require.NoError(t, err, "add prior htlc")
			}

			d, err := m.addHTLC(
				circuit(1, 99), scid(2), fee, cltv, testHeight,
				true, at,
			)
			require.NoError(t, err, "addHTLC")

			require.True(t, d.inIsolation, "in isolation: %s", d)
			require.Equal(t, test.wantTotalRisk, d.totalRisk)
			require.Equal(
				t, test.wantWithInFlight, d.withInFlight,
				"with in flight: %s", d,
			)
		})
	}
...
// TestDuplicateForwardRejected checks that a second forward for an incoming
// circuit already pending is rejected without touching the first. Otherwise the
// index would move to the new channel and orphan the original pending HTLC,
// which would then never resolve and keep charging its channel's in-flight
// risk.
func TestDuplicateForwardRejected(t *testing.T) {
	...
	m.OnForward(in, scid(2), 2000, 1000, 1000, 200, testHeight, true)
	m.OnForward(in, scid(3), 2000, 1000, 1000, 200, testHeight, true)

	require.EqualValues(t, 2, m.htlcIndex[in], "index moved")
	require.Len(t, m.channels[2].pendingHTLCs, 1, "original pending lost")
	require.Empty(t, m.channels[3].pendingHTLCs, "duplicate recorded")
}
