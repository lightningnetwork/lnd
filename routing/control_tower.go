...
type DBMPPayment interface {
	...
	GetHTLCs() []pmt.HTLCAttempt

	// InFlightHTLCs returns all HTLCs that are in flight.
	InFlightHTLCs() []pmt.HTLCAttempt
	...
	// failure reason.
	TerminalInfo() (*pmt.HTLCAttempt, *pmt.FailureReason)
}
...
type ControlTower interface {
	...
	// hash.
	InitPayment(lntypes.Hash, *pmt.PaymentCreationInfo) error
	...
	RegisterAttempt(lntypes.Hash, *pmt.HTLCAttemptInfo) error
	...
	// for record keeping.
	SettleAttempt(lntypes.Hash, uint64, *pmt.HTLCSettleInfo) (
		*pmt.HTLCAttempt, error)

	// FailAttempt marks the given payment attempt failed.
	FailAttempt(lntypes.Hash, uint64, *pmt.HTLCFailInfo) (
		*pmt.HTLCAttempt, error)
	...
	// payment.
	FailPayment(lntypes.Hash, pmt.FailureReason) error

	// FetchInFlightPayments returns all payments with status InFlight.
	FetchInFlightPayments() ([]*pmt.MPPayment, error)
...
func (p *controlTower) RegisterAttempt(paymentHash lntypes.Hash,
	attempt *pmt.HTLCAttemptInfo) error {
...
func (p *controlTower) SettleAttempt(paymentHash lntypes.Hash,
	attemptID uint64, settleInfo *pmt.HTLCSettleInfo) (
	*pmt.HTLCAttempt, error) {
...
func (p *controlTower) FailAttempt(paymentHash lntypes.Hash,
	attemptID uint64, failInfo *pmt.HTLCFailInfo) (
	*pmt.HTLCAttempt, error) {
...
// FetchInFlightPayments returns all payments with status InFlight.
func (p *controlTower) FetchInFlightPayments() ([]*pmt.MPPayment, error) {
