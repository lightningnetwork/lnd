...
type DBMPPayment interface {
	...
	GetHTLCs() []pymtpkgDB.HTLCAttempt

	// InFlightHTLCs returns all HTLCs that are in flight.
	InFlightHTLCs() []pymtpkgDB.HTLCAttempt
	...
	// failure reason.
	TerminalInfo() (*pymtpkgDB.HTLCAttempt, *pymtpkgDB.FailureReason)
}
...
type ControlTower interface {
	...
	// hash.
	InitPayment(lntypes.Hash, *pymtpkgDB.PaymentCreationInfo) error
	...
	RegisterAttempt(lntypes.Hash, *pymtpkgDB.HTLCAttemptInfo) error
	...
	// for record keeping.
	SettleAttempt(lntypes.Hash, uint64, *pymtpkgDB.HTLCSettleInfo) (
		*pymtpkgDB.HTLCAttempt, error)

	// FailAttempt marks the given payment attempt failed.
	FailAttempt(lntypes.Hash, uint64, *pymtpkgDB.HTLCFailInfo) (
		*pymtpkgDB.HTLCAttempt, error)
	...
	// payment.
	FailPayment(lntypes.Hash, pymtpkgDB.FailureReason) error

	// FetchInFlightPayments returns all payments with status InFlight.
	FetchInFlightPayments() ([]*pymtpkgDB.MPPayment, error)
...
type ControlTowerSubscriber interface {
	// Updates is the channel over which *pymtpkgDB.MPPayment updates can be
	// received.
...
func NewControlTower(db pymtpkgDB.PaymentDB) ControlTower {
	return &controlTower{
		db: db,
...
func (p *controlTower) InitPayment(paymentHash lntypes.Hash,
	info *pymtpkgDB.PaymentCreationInfo) error {
...
func (p *controlTower) RegisterAttempt(paymentHash lntypes.Hash,
	attempt *pymtpkgDB.HTLCAttemptInfo) error {
...
func (p *controlTower) SettleAttempt(paymentHash lntypes.Hash,
	attemptID uint64, settleInfo *pymtpkgDB.HTLCSettleInfo) (
	*pymtpkgDB.HTLCAttempt, error) {
...
func (p *controlTower) FailAttempt(paymentHash lntypes.Hash,
	attemptID uint64, failInfo *pymtpkgDB.HTLCFailInfo) (
	*pymtpkgDB.HTLCAttempt, error) {
...
// FetchInFlightPayments returns all payments with status InFlight.
func (p *controlTower) FetchInFlightPayments() ([]*pymtpkgDB.MPPayment, error) {
