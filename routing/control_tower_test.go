...
func TestControlTowerSubscribeUnknown(t *testing.T) {
	...
	paymentDB := pymtpkgDB.NewTestDB(t)
	...
	require.ErrorIs(t, err, pymtpkgDB.ErrPaymentNotInitiated)
}
...
func TestControlTowerSubscribeSuccess(t *testing.T) {
	...
	settleInfo := pymtpkgDB.HTLCSettleInfo{
		Preimage: preimg,
	}
	...
	for i, s := range subscribers {
		...
		require.Equalf(t, pymtpkgDB.StatusSucceeded, result.GetStatus(),
			"subscriber %v failed, want %s, got %s", i,
			pymtpkgDB.StatusSucceeded, result.GetStatus())
...
func TestPaymentControlSubscribeAllSuccess(t *testing.T) {
	...
	paymentDB := pymtpkgDB.NewTestDB(
		t, pymtpkgDB.WithKeepFailedPaymentAttempts(true),
	)
	pControl := NewControlTower(paymentDB)
	...
	settleInfo1 := pymtpkgDB.HTLCSettleInfo{
		Preimage: preimg1,
	}
	...
	settleInfo2 := pymtpkgDB.HTLCSettleInfo{
		Preimage: preimg2,
	}
	...
	require.Equal(
		t, pymtpkgDB.StatusSucceeded, result1.GetStatus(),
	...
	require.Equal(
		t, pymtpkgDB.StatusSucceeded, result2.GetStatus(),
...
func TestPaymentControlSubscribeAllImmediate(t *testing.T) {
	...
	case update := <-subscription.Updates():
		...
		payment, ok := update.(*pymtpkgDB.MPPayment)
		require.True(t, ok, "unexpected type in "+
...
func testPaymentControlSubscribeFail(t *testing.T, registerAttempt,
	...
	opts := []pymtpkgDB.OptionModifier{
		pymtpkgDB.WithKeepFailedPaymentAttempts(
			keepFailedPaymentAttempts,
		),
	}
	paymentDB := pymtpkgDB.NewTestDB(t, opts...)
	pControl := NewControlTower(paymentDB)
	...
	err = pControl.FailPayment(
		info.PaymentIdentifier, pymtpkgDB.FailureReasonTimeout,
	)
	if err != nil {
		t.Fatal(err)
	}
	...
	for i, s := range subscribers {
		...
		if result.GetStatus() == pymtpkgDB.StatusSucceeded {
		...
		require.Equalf(t, pymtpkgDB.StatusFailed, result.GetStatus(),
			"subscriber %v failed, want %s, got %s", i,
			pymtpkgDB.StatusFailed, result.GetStatus())

		if *result.FailureReason != pymtpkgDB.FailureReasonTimeout {
...
func genInfo() (*pymtpkgDB.PaymentCreationInfo, *pymtpkgDB.HTLCAttemptInfo,
	...
	attempt, err := pymtpkgDB.NewHtlcAttempt(
	...
	return &pymtpkgDB.PaymentCreationInfo{
