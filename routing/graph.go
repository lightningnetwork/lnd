...
// GraphSessionFactory can be used to gain access to a graphdb.CachedGraph
...
type GraphSessionFactory interface {
	...
	// graphdb.CachedGraph instance which can be used to perform queries
	// against the channel graph.
	GraphSession(cb func(graph graphdb.CachedGraph) error) error
}
