...
type MissionControl struct {
	...
	lastPairResult map[DirectedNodePair]pairHistory
...
type pairHistory struct {
	// timestamp is the time when this last result was obtained.
	timestamp time.Time
...
func NewMissionControl(db *bbolt.DB, cfg *MissionControlConfig) (
	*MissionControl, error) {
	...
	mc := &MissionControl{
		lastPairResult:   make(map[DirectedNodePair]pairHistory),
...
func (m *MissionControl) ResetHistory() error {
	...
	m.lastPairResult = make(map[DirectedNodePair]pairHistory)
...
func (m *MissionControl) reportEdgePolicyFailure(timestamp time.Time,
	...
	m.lastNodeFailure[failedEdge.from] = timestamp
}
...
func (m *MissionControl) reportEdgeFailure(timestamp time.Time, failedEdge edge,
	...
	pair := NewDirectedNodePair(failedEdge.from, failedEdge.to)
