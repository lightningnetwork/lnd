...
func (m *MissionControl) GetHistorySnapshot() *MissionControlSnapshot {
	...
	for v, h := range m.history {
		channelSnapshot := make([]*MissionControlChannelSnapshot,
			0, len(h.channelLastFail))

		for id, lastFail := range h.channelLastFail {
			prob := m.getEdgeProbabilityForNode(h, id)
		...
		otherProb := m.getEdgeProbabilityForNode(h, 0)
	...
	log.Debugf("History snapshot created")
