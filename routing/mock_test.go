...
func (m *mockMissionControlOld) ReportPaymentFail(
	...
	*pymtpkgDB.FailureReason, error) {
	...
	if failure.Code() == lnwire.CodeIncorrectOrUnknownPaymentDetails {
		reason := pymtpkgDB.FailureReasonPaymentDetails
		return &reason, nil
	}
...
type initArgs struct {
	c *pymtpkgDB.PaymentCreationInfo
}

type registerAttemptArgs struct {
	a *pymtpkgDB.HTLCAttemptInfo
}
...
func makeMockControlTower() *mockControlTowerOld {
	return &mockControlTowerOld{
		...
		failed:     make(map[lntypes.Hash]pymtpkgDB.FailureReason),
	}
}

func (m *mockControlTowerOld) InitPayment(phash lntypes.Hash,
	c *pymtpkgDB.PaymentCreationInfo) error {
	...
	if _, ok := m.successful[phash]; ok {
		return pymtpkgDB.ErrAlreadyPaid
	}
	...
	if ok && !failed {
		return pymtpkgDB.ErrPaymentInFlight
	}
...
func (m *mockControlTowerOld) DeleteFailedAttempts(phash lntypes.Hash) error {
	...
	if !ok {
		return pymtpkgDB.ErrPaymentNotInitiated
	}
...
func (m *mockControlTowerOld) RegisterAttempt(phash lntypes.Hash,
	a *pymtpkgDB.HTLCAttemptInfo) error {
	...
	if settled || failed {
		return pymtpkgDB.ErrPaymentTerminal
	}

	if settled && !inFlight {
		return pymtpkgDB.ErrPaymentAlreadySucceeded
	}

	if failed && !inFlight {
		return pymtpkgDB.ErrPaymentAlreadyFailed
	}

	// Add attempt to payment.
	p.attempts = append(p.attempts, pymtpkgDB.HTLCAttempt{
		HTLCAttemptInfo: *a,
	})
	m.payments[phash] = p
...
func (m *mockControlTowerOld) SettleAttempt(phash lntypes.Hash,
	pid uint64, settleInfo *pymtpkgDB.HTLCSettleInfo) (
	*pymtpkgDB.HTLCAttempt, error) {
	...
	p, ok := m.payments[phash]
	if !ok {
		return nil, pymtpkgDB.ErrPaymentNotInitiated
	}
	...
	for i, a := range p.attempts {
		...
		if a.Settle != nil {
			return nil, pymtpkgDB.ErrAttemptAlreadySettled
		}
		if a.Failure != nil {
			return nil, pymtpkgDB.ErrAttemptAlreadyFailed
		}
...
func (m *mockControlTowerOld) FailAttempt(phash lntypes.Hash, pid uint64,
	failInfo *pymtpkgDB.HTLCFailInfo) (*pymtpkgDB.HTLCAttempt, error) {
...
func (m *mockMissionControl) ReportPaymentFail(
	...
	*pymtpkgDB.FailureReason, error) {
	...
	return args.Get(0).(*pymtpkgDB.FailureReason), args.Error(1)
}
...
func (m *mockControlTower) SettleAttempt(phash lntypes.Hash,
	...
	*pymtpkgDB.HTLCAttempt, error) {
	...
	return attempt.(*pymtpkgDB.HTLCAttempt), args.Error(1)
}
...
func (m *mockControlTower) FailAttempt(phash lntypes.Hash, pid uint64,
	...
	return args.Get(0).(*pymtpkgDB.HTLCAttempt), args.Error(1)
}
...
func (m *mockControlTower) FetchInFlightPayments() (
	...
	args := m.Called()
	return args.Get(0).([]*pymtpkgDB.MPPayment), args.Error(1)
}
...
func (m *mockMPPayment) GetState() *pymtpkgDB.MPPaymentState {
	args := m.Called()
	return args.Get(0).(*pymtpkgDB.MPPaymentState)
}

func (m *mockMPPayment) GetStatus() pymtpkgDB.PaymentStatus {
	args := m.Called()
	return args.Get(0).(pymtpkgDB.PaymentStatus)
}
...
func (m *mockMPPayment) GetHTLCs() []pymtpkgDB.HTLCAttempt {
	args := m.Called()
	return args.Get(0).([]pymtpkgDB.HTLCAttempt)
}

func (m *mockMPPayment) InFlightHTLCs() []pymtpkgDB.HTLCAttempt {
	args := m.Called()
	return args.Get(0).([]pymtpkgDB.HTLCAttempt)
}
...
func (m *mockMPPayment) TerminalInfo() (*pymtpkgDB.HTLCAttempt,
	*pymtpkgDB.FailureReason) {

	args := m.Called()

	var (
		settleInfo  *pymtpkgDB.HTLCAttempt
		failureInfo *pymtpkgDB.FailureReason
	)

	settle := args.Get(0)
	if settle != nil {
		settleInfo = settle.(*pymtpkgDB.HTLCAttempt)
	}

	reason := args.Get(1)
	if reason != nil {
		failureInfo = reason.(*pymtpkgDB.FailureReason)
	}
