...
func (p *paymentLifecycle) decideNextStep(
	...
	if !allow {
		...
		// If we are not allowed to make new HTLC attempts and
		// no need to wait, the lifecycle is done and we can
		// exit.
		if !wait {
			return stepExit, nil
		}
...
lifecycle:
	for {
		...
		// We may not be able to find a route. In that case, we
		// continue the loop and move straight to the next iteration.
		if rt == nil {
			continue lifecycle
		}
