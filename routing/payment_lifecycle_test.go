...
// newTestPaymentLifecycle creates a `paymentLifecycle` using the mocks.
...
func TestResumePaymentFailOnRequestRouteErr(t *testing.T) {
	...
	m.paySession.On("RequestRoute",
		ps.RemainingAmt, p.feeLimit, uint32(ps.NumAttemptsInFlight),
...
// an error is returned from `sendPaymentAttempt`.
//
// NOTE: No parallel test because it overwrites global variables.
//
//nolint:paralleltest
func TestResumePaymentFailOnSendAttemptErr(t *testing.T) {
	...
	// 6. mock `sendPaymentAttempt` to return an error.
	m.payer.On("SendHTLC",
...
// TestResumePaymentSuccess checks that a normal payment flow that succeeds.
//
// NOTE: No parallel test because it overwrites global variables.
//
//nolint:paralleltest
func TestResumePaymentSuccess(t *testing.T) {
	...
	// 1.6. mock `sendPaymentAttempt` to succeed, which brings us into the
	// next iteration of the lifecycle.
	m.payer.On("SendHTLC",
		mock.Anything, attemptID, mock.Anything,
	).Return(nil).Once()
...
// TestResumePaymentRetry checks a payment flow with a failure on the first
// attempt, but that succeeds on the second attempt.
//
// NOTE: No parallel test because it overwrites global variables.
//
//nolint:paralleltest
func TestResumePaymentRetry(t *testing.T) {
	// Create a test paymentLifecycle with the initial two calls mocked.
	p, m := setupTestPaymentLifecycle(t)
	...
	m.shardTracker.On("NewShard",
		attemptID, false,
	).Return(m.shard, nil).Run(func(args mock.Arguments) {
		// Once this is called, increment the attemptID for the next
		// call.
		attemptID++
	}).Once()
	...
	// 1.6. mock `sendPaymentAttempt` to succeed, which brings us into the
	// next iteration of the lifecycle where we mock a temp failure.
	m.payer.On("SendHTLC",
		mock.Anything, attemptID, mock.Anything,
	).Return(nil).Once()
	...
	// 2.6. mock `sendPaymentAttempt` to succeed, which brings us into the
	// next iteration of the lifecycle.
	m.payer.On("SendHTLC",
		mock.Anything, attemptID, mock.Anything,
	).Return(nil).Once()
...
func TestCollectResultAsyncSuccess(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		...
		return true
	}, testTimeout, 10*time.Millisecond, "lifecycle not quit")
