...
func newTestPaymentLifecycle(t *testing.T) (*paymentLifecycle, *mockers) {
	...
	resultCollector := func(attempt *pymtpkgDB.HTLCAttempt) {
...
func makeSettledAttempt(t *testing.T, total int,
	preimage lntypes.Preimage) *pymtpkgDB.HTLCAttempt {

	a := &pymtpkgDB.HTLCAttempt{
		HTLCAttemptInfo: makeAttemptInfo(t, total),
		Settle:          &pymtpkgDB.HTLCSettleInfo{Preimage: preimage},
	}
...
func makeFailedAttempt(t *testing.T, total int) *pymtpkgDB.HTLCAttempt {
	return &pymtpkgDB.HTLCAttempt{
		HTLCAttemptInfo: makeAttemptInfo(t, total),
		Failure: &pymtpkgDB.HTLCFailInfo{
			Reason: pymtpkgDB.HTLCFailInternal,
		},
	}
}

func makeAttemptInfo(t *testing.T, amt int) pymtpkgDB.HTLCAttemptInfo {
	rt := createDummyRoute(t, lnwire.MilliSatoshi(amt))
	return pymtpkgDB.HTLCAttemptInfo{
		Route: *rt,
...
func TestCheckTimeoutTimedOut(t *testing.T) {
	...
	ct := &mockControlTower{}
	ct.On("FailPayment",
		p.identifier, pymtpkgDB.FailureReasonTimeout).Return(nil)
	...
	ct = &mockControlTower{}
	ct.On("FailPayment",
		p.identifier, pymtpkgDB.FailureReasonTimeout).Return(errDummy)
...
func TestRequestRouteHandleNoRouteErr(t *testing.T) {
	...
	m.control.On("FailPayment",
		p.identifier, pymtpkgDB.FailureReasonNoRoute,
	).Return(nil).Once()
...
func TestResumePaymentFailOnTimeout(t *testing.T) {
	...
	m.payment.On("GetStatus").Return(pymtpkgDB.StatusInFlight)
	...
	m.control.On("FailPayment",
		p.identifier, pymtpkgDB.FailureReasonTimeout,
	).Return(nil).Once()
	...
	reason := pymtpkgDB.FailureReasonTimeout
...
func TestResumePaymentFailContextCancel(t *testing.T) {
	...
	m.control.On(
		"FailPayment", p.identifier, pymtpkgDB.FailureReasonCanceled,
	).Return(nil).Once()
...
func TestReloadInflightAttemptsLegacy(t *testing.T) {
	...
	attempts := []pymtpkgDB.HTLCAttempt{*attempt}
