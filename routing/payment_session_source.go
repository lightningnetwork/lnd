...
type SessionSource struct {
	// GraphSessionFactory can be used to start a new GraphSession which
	// will give read only access to a Graph. If the backing DB allows it,
	// this will mean that a read transaction is being held during the use
	// of the session. After use of the session is complete, `Close` must
	// be called on the GraphSession.
