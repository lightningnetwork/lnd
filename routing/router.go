...
func (r *ChannelRouter) resumePayments() error {
	...
	launchPayment := func(payment *channeldb.MPPayment) {
		...
		// We pass in a zero timeout value, to indicate we don't need
...
func (r *ChannelRouter) failStaleAttempt(a channeldb.HTLCAttempt,
	...
	// First, check if we've already had a network result for this attmept.
	...
	// never saved, causing the payments to stuck inflight. We now check
	...
	if len(a.Route.Hops) == 0 {
		...
		if r.IsKnownEdge(scid) {
			return
		}

		// The channel is unknown to us, we now check whether this
