...
func TestNodeEdgeUnifier(t *testing.T) {
	...
	unifierFilled.addPolicy(fromNode, &p1, inboundFee1, c1)
	unifierFilled.addPolicy(fromNode, &p2, inboundFee2, c2)

	unifierNoCapacity := newNodeEdgeUnifier(source, toNode, false, nil)
	unifierNoCapacity.addPolicy(fromNode, &p1, inboundFee1, 0)
	unifierNoCapacity.addPolicy(fromNode, &p2, inboundFee2, 0)

	unifierNoInfo := newNodeEdgeUnifier(source, toNode, false, nil)
	unifierNoInfo.addPolicy(
		fromNode, &models.CachedEdgePolicy{},
		models.InboundFee{}, 0,
	)

	unifierInboundFee := newNodeEdgeUnifier(source, toNode, true, nil)
	unifierInboundFee.addPolicy(fromNode, &p1, inboundFee1, c1)
	unifierInboundFee.addPolicy(fromNode, &p2, inboundFee2, c2)

	unifierLocal := newNodeEdgeUnifier(fromNode, toNode, true, nil)
	unifierLocal.addPolicy(fromNode, &p1, inboundFee1, c1)
	...
	unifierNegInboundFee.addPolicy(fromNode, &p1, inboundFeeZero, c1)
	unifierNegInboundFee.addPolicy(fromNode, &p2, inboundFeeNegative, c2)
