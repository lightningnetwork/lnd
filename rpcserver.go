...
func (r *rpcServer) UpdateChannelPolicy(ctx context.Context,
	...
	switch fee := req.Fee.(type) {
	// If the request is using fee_rate.
	case *lnrpc.PolicyUpdateRequest_FeeRate:
		...
		if fee.FeeRate != 0 && fee.FeeRate < minFeeRate {
			return nil, fmt.Errorf("fee rate of %v is too small, min fee "+
				"rate is %v", fee.FeeRate, minFeeRate)
		}
		...
		feeRateFixed = uint32(math.Round(fee.FeeRate * feeBase))
	// Otherwise, we use the fee_rate_ppm parameter.
	case *lnrpc.PolicyUpdateRequest_FeeRatePpm:
		feeRateFixed = fee.FeeRatePpm
	}
