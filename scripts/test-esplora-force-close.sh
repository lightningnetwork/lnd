#!/bin/bash
#
# Force Close E2E Test Script for LND Esplora Backend
#
# This script specifically tests force close scenarios to verify sweep
# transaction creation for time-locked outputs works correctly with
# the Esplora HTTP API backend.
#
# Prerequisites:
# - Bitcoin Core running (native or in Docker)
# - Esplora API server (electrs/mempool-electrs) running
# - LND built (go build -o lnd-esplora ./cmd/lnd)
#
# Usage:
...
TEST_DIR="./test-esplora-force-close"
...
cleanup() {
    ...
    pkill -f "lnd-esplora.*test-esplora-force-close" 2>/dev/null || true
...
[Application Options]
noseedbackup=true
debuglevel=debug,SWPR=trace,CNCT=trace,NTFN=trace,ESPN=trace
...
EOF
    ...
    log_info "Created configs with trace logging for SWPR, CNCT, NTFN, ESPN"
}
...
wait_for_channel_balance() {
    local expected_balance=$1
    local max_attempts=30
    local attempt=0

    log_info "Waiting for channel to have balance >= $expected_balance sats..."
    while [ $attempt -lt $max_attempts ]; do
        local balance=$(alice_cli listchannels 2>/dev/null | jq -r '.channels[0].local_balance // "0"')
        if [ "$balance" != "0" ] && [ "$balance" != "null" ] && [ "$balance" -ge "$expected_balance" ] 2>/dev/null; then
            log_info "Channel local balance: $balance sats"
            return 0
        fi
        sleep 1
        attempt=$((attempt + 1))
    ...
    log_warn "Channel balance not detected after $max_attempts attempts"
    alice_cli listchannels | jq '.channels[0] | {local_balance, remote_balance, active}'
    return 1
}
...
run_force_close_test() {
    ...
    log_step "Opening channel (100k sats) for force close test..."
    alice_cli openchannel --node_key="$bob_pubkey" --local_amt=100000
    mine_blocks 6
    sleep 5
    wait_for_channel

    # Wait for channel to be fully ready with balance
    wait_for_channel_balance 90000

    log_step "Making payment so Bob has balance..."
    local invoice=$(bob_cli addinvoice --amt=20000 | jq -r '.payment_request')
    if ! alice_cli payinvoice --force "$invoice" > /dev/null 2>&1; then
        log_warn "Payment failed, retrying after delay..."
        sleep 5
        alice_cli payinvoice --force "$invoice" > /dev/null 2>&1
    fi
    log_info "Payment complete - Bob now has 20000 sats in channel"
    ...
    if [ "$pending_force" = "0" ]; then
        ...
        log_info "  - Any errors from SWPR, CNCT, or ESPN"
    fi
    echo ""
}
...
main() {
    echo -e "${GREEN}"
    echo "============================================"
    echo "  LND Esplora Force Close Test Script"
    echo "============================================"
    echo -e "${NC}"
    echo ""
