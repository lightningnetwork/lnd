...
func newServer(_ context.Context, cfg *Config, listenAddrs []net.Addr,
	...
	// the network.
	color, alias, addresses, err := getNodeAnnouncementFields(
...
// getNodeAnnouncementFields populates the alias, color, and addresses. This
// is used during startup to construct the initial node announcement.
func getNodeAnnouncementFields(cfg *Config, s *server,
	...
	// Try to parse the color from config. We will update this later if the
	// config color is not changed from default (#3399FF).
	...
	// data for our initial node announcement else we'll use the default
	// config values.
	...
	if err != nil {
		...
		return color, alias, addr, nil
	}

	// If the color was not changed from default, it means that we didn't
	...
	// If an alias was not specified in the config, we'll use the source
	// node's alias.
	if alias == "" {
		alias = sourceNode.Alias
	}

	// Get the previously set addresses from the source node.
