...
func (b *BudgetAggregator) sortInputs(inputs []SweeperInput) []SweeperInput {
	...
	sort.Slice(sortedInputs, func(i, j int) bool {
		...
		switch {
		// If both are forced inputs, we sort based on their budgets.
		case leftForce && rightForce:
			return left > right

		// If the left input is forced, it should be put in the front.
		case leftForce:
			return true

		// If the right input is forced, it should be put in the front.
		case rightForce:
			return false
