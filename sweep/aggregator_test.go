...
func TestBudgetAggregatorFilterInputsAuxBudget(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			// An unexpectedly negative own budget must be clamped
			// to zero before computing the headroom for the
			// saturating add; otherwise math.MaxInt64-budget would
			// overflow and the input would be falsely promoted to
			// MaxInt64. With the clamp, a negative own budget plus
			// a positive aux contribution that doesn't cover the
			// min fee correctly drops the input.
			name:       "negative own budget clamps to zero",
			ownBudget:  -shortfall,
			auxResult:  fn.Ok(minFee - shortfall),
			expectKept: false,
		},
	}
