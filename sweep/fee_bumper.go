...
func (t *TxPublisher) createAndCheckTx(req *BumpRequest, f FeeFunction) (
	*wire.MsgTx, btcutil.Amount, error) {
	...
	// If the backend version is too low, we'll skip this check.
	if errors.Is(err, rpcclient.ErrBackendVersion) {
		log.Warnf("Skipping mempool acceptance check: %v", err)
		return tx, fee, nil
	}
...
func (t *TxPublisher) processRecords() {
	...
	for requestID, r := range feeBumpRecords {
		...
		t.wg.Add(1)
		go t.handleFeeBumpTx(requestID, rec)
	}
}
...
func (t *TxPublisher) createSweepTx(inputs []input.Input, changePkScript []byte,
	...
	sweepTx.LockTime = uint32(t.currentHeight)
	locktimeOpt.WhenSome(func(locktime int32) {
