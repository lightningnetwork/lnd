...
func (s *UtxoSweeper) removeExclusiveGroup(group uint64) {
	for outpoint, input := range s.pendingInputs {
		...
		s.signalResult(&outpoint, Result{
...
func (s *UtxoSweeper) signalResult(outpoint *wire.OutPoint, result Result) {
	pi, exist := s.pendingInputs[*outpoint]
	if !exist {
		log.Debugf("Input %v doesn't exist", outpoint)
		return
	}
...
func (s *UtxoSweeper) markInputsPendingPublish(tr *TxRecord,
	...
	for _, input := range tx.TxIn {
		...
		if pi.publishAttempts >= s.cfg.MaxSweepAttempts {
			...
			s.signalResult(&input.PreviousOutPoint, Result{
...
func (s *UtxoSweeper) handleNewInput(input *sweepInputMessage) {
	...
	if err != nil {
		...
		s.signalResult(&outpoint, Result{Err: err})
...
// maybeAttachRBFInfo queries the mempool to see whether the given input has
// already been spent. If so, it will query the sweeper store to fetch the fee
// info of the spending transction, hence preparing for possible RBF.
func (s *UtxoSweeper) maybeAttachRBFInfo(pi *pendingInput) *pendingInput {
	...
	//
	// NOTE: this is not accurate for neutrino backend.
	if !spent {
		return pi
	}
...
func (s *UtxoSweeper) updateSweeperInputs() pendingInputs {
	...
	for op, input := range s.pendingInputs {
		...
		if input.state == StatePublished {
			// TODO(yy): handle the RBF case here. Let it be
