...
const (
	...
	// sweeping transactions confirmed, the rest two will be excluded.
	Excluded
...
func (s *UtxoSweeper) handlePendingSweepsReq(
	...
	for _, SweeperInput := range s.inputs {
		...
		resps[op] = &PendingInputResponse{
			...
			Amount: btcutil.Amount(
				SweeperInput.SignDesc().Output.Value,
			),
			LastFeeRate:       SweeperInput.lastFeeRate,
...
// publish.
func (s *UtxoSweeper) handleBumpEventTxFailed(r *BumpResult) {
	tx, err := r.Tx, r.Err
