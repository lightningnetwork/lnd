...
func TestFeeRateForPreference(t *testing.T) {
	...
	// Set the max fee rate to be 1000 sat/kw.
	...
	largeFeeFunc := func(_ chainfee.Estimator, _ FeePreference) (
		...
		return s.cfg.MaxFeeRate + 1, nil
	}

	// validFeeRate is used to test the success case.
	validFeeRate := (s.cfg.MaxFeeRate + s.relayFeeRate) / 2
	...
	testCases := []struct {
		...
		},
		{
			...
			expectedErr:       ErrFeePreferenceTooHigh,
		},
		{
...
func TestClusterByLockTime(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			// Use a fee rate below the max value so we don't hit
			...
			testFeeRate: s.cfg.MaxFeeRate - 1,
			expectedClusters: []inputCluster{
				{
					lockTime:     &lockTime1,
					sweepFeeRate: s.cfg.MaxFeeRate - 1,
					inputs:       cluster1,
				},
				{
					lockTime:     &lockTime2,
		...
		},
		{
			...
			// Use a fee rate above the max value so we hit an
