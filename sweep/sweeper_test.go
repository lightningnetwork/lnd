...
// TestMaybeAttachRBFInfo checks that the RBF info is attached to the pending
// input, along with the state being marked as published, when this input can
// be found both in mempool and the sweeper store.
func TestMaybeAttachRBFInfo(t *testing.T) {
	t.Parallel()
	...
	result := s.maybeAttachRBFInfo(pi)
	require.Nil(result.rbf)
