...
type nurseryTestContext struct {
	...
	receiveSweepTx func() wire.MsgTx
...
func createNurseryTestContext(t *testing.T,
	...
	ctx.receiveSweepTx = func() wire.MsgTx {
		tickCount := 0
		ticker := time.NewTicker(defaultTestTimeout)
		for {
			select {
			case tx := <-ctx.publishChan:
				utxnLog.Tracef("tx received after %v ticks",
					tickCount)
				return tx
			case <-ticker.C:
				pprof.Lookup("goroutine").WriteTo(os.Stdout, 1)
...
func (ctx *nurseryTestContext) tick(ticks int) {
	select {
