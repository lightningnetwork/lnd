...
func extractChanBackups(chanBackups *lnrpc.ChanBackupSnapshot) *ChannelsToRecover {
	...
	if len(chanBackups.SingleChanBackups.ChanBackups) == 0 &&
		len(chanBackups.MultiChanBackup.MultiChanBackup) == 0 {
	...
	backups := ChannelsToRecover{
		PackedMultiChanBackup: chanbackup.PackedMulti(
			multiBackup.MultiChanBackup,
		),
	}

	// Finally, we can extract all the single chan back ups as well.
