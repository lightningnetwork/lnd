...
func TestDecodeEncode(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			valid:          false,
			wantErr:        ErrPaymentSecretNotFound,
		},
		{
		...
		{
			// Reject fields with unknown lengths.
		...
		{
			// A second payment hash with an unknown length is
			// rejected by the length check before the duplicate
			// check is reached.
		...
		},
		{
			...
			// and a payment secret, with the tagged fields in the
			// order LND encodes them.
...
// TestDecodeSkipPaymentSecretCheck checks that an invoice without a payment
// secret is rejected by default, and accepted when the payment secret check is
// skipped.
func TestDecodeSkipPaymentSecretCheck(t *testing.T) {
	t.Parallel()

	encodedInvoice := "lnbc20m1pvjluezpp5qqqsyqcyq5rqwzqfqqqsyqcyq5" +
	...
	_, err := Decode(encodedInvoice, &chaincfg.MainNetParams)
	require.ErrorIs(t, err, ErrPaymentSecretNotFound)

	invoice, err := Decode(
		encodedInvoice, &chaincfg.MainNetParams,
		WithSkipPaymentSecretCheck(),
	)
	require.NoError(t, err)
	require.Equal(t, testPaymentHash, *invoice.PaymentHash)
	require.True(t, invoice.PaymentAddr.IsNone())
}
...
func TestInvoiceChecksumMalleability(t *testing.T) {
	// The key is chosen so that the checksum of the encoded invoice
	// ends in "p", which the test below relies on.
